...
"""like average_secondary_structureII.py, but secondary structure
prediction from scratchdir+'/spliceVariants/out/jnets.For every
...
cmd='grep "00$" '+scratchdir+'/spliceVariants/out/jnets/success.list'
for ref in chomp(os.popen(cmd).readlines()):
    #obtain secondary structure and dump it in a string
    ss=translateSec(scratchdir+'/spliceVariants/out/jnets/'+ref+'/seq.dat')
