...
c
c     pgf77 -Mextend -O3 -Bstatic -o spk_rp spicker_report_unaligned.f
c     pgf77 -Mextend -Mbounds -Mchkfpstk  -o spk_rp.debug spicker_report_unaligned.f
...
c 345678901234567890123456789012345678901234567890123456789012345678901234567
     ...
     $     i_str_cl_ex,rmsd_cl_cut,n_str_desired
      ...
      integer n_str_desired      !desired size of biggest cluster
...
ccc   
     ...
     $     str_selected
      ...
      character*512 fnam
********************************************************************
c     initialize optional command line arguments
      n_str_desired=-1
c     input command line arguments
      call getarg(1,fnam)
      if(fnam.eq.'?'.or.fnam.eq.'-h')then
         call system('clear')
         write(*,*)'spicker [arguments]'
         write(*,*)'Optional'
         write(*,*)' -s size, desired size of biggest cluster ',
     $        '(def: -1, for no desired size)' 
         call exit(1)
      endif
      narg=iargc()
      i=1
...
c     determine the cluster with maximum density
      ...
      if(n_str_desired.lt.0)then !we do not require cluster of certain size
         do i=2,nc
...
c     cluster centroid conformation of cluster with maximum density
...
c     to most dense cluster 
      ...
      do iloop=1,nloop
         ...
         write(0,*)'iloop=',iloop,' lextent=',lextent(iloop),' lbegin=',lbegin(iloop)
         write(0,*)'do clustering of loop local rmsd values'
         call clusterize(iloop,amat,.true.) !do clustering of loop local rmsd values
         write(0,*)'do combo structures for the loop'
         call make_combo(iloop,.true.) !do combo structures for the loop
         write(0,*)'do rst_iloop.dat-like file'
         call report_clustering(iloop,.true.) !do rst_iloop.dat-like file
         write(0,*)'do clustering of loop global rmsd values'
...
c     read chain.dat file, initializing isalig,lbegin,lextent,nloop,lextents
     ...
     &ntemp,itemp,ncov,i,j,k,b,e,lextents(ndim),lresid(ndim,ndim)
...
c     find covered residues and their coordinates
      itemp=1 !by default, select the first template
      do i=1,ncov
         read(7,*) j,x0,y0,z0
...
c     next add the stems on both sides, totalling another lextent(i)
c     residues, at most. Stems only included aligned residues
 ...
 10      if(lextents(i).lt.2*lextent(i) .and. 
     $        (b.gt.0 .or. e.le.Lch) )then
 15         if(b.gt.0)then      !extend on the left
 ...
 20         if(e.le.Lch)then    !extend on the right
...
c     print loops
c      do i=1,nloop
c         write(0,'3i3')lbegin(i),lextent(i),lextents(i)
c         do j=1,lextents(i)
c            write(0,'i3x') lresid(j,i)
c         enddo
c      enddo
...
c     find out the biggest cluster and decide the RMSD_cut_min
     ...
     $           end_to_end_dist(iloop),.false.) !expected random rmsd for loop global
         ...
         ratio2=0.15
         nc_max=10
      else !obtain global (full sequence) clusters
         rmsd_cut_ini=8.0
         rmsd_cut_max=12.0
         rmsd_cut_min=3.5
         ...
         ratio2=0.15
         nc_max=10
         if(n_str_desired.gt.0) rmsd_cut=0.2 !make sure initial biggest cluster is very small
      endif


 140  do j=1,nstr_selected
         jj=str_selected(j)
         mark(jj)=1              !initialization
...
c     check the size of the cluster
      if(iloop.lt.0.and.n_str_desired.gt.0)then !we want global first cluster of certain size
         if(n_str_cl_max.lt.n_str_desired.and.
     $        n_str_cl_max.lt.nstr_selected)then
...
c     calculate nc(j), number of neighboring structures ----------------->
         do j=1,nstr_selected
            jj=str_selected(j)
            n_str_near(jj)=0     !initialize number of structure in cluster
            do k=1,nstr_selected
               kk=str_selected(k)
               if(mark(jj).eq.1.and.mark(kk).eq.1)then !not used up in a previous cluster
                  if(amat(jj,kk).lt.rmsd_cut)then
                     n_str_near(jj)=n_str_near(jj)+1
                  endif
               endif
...
c     check the size of the cluster 
         if(n_str_cl_max.lt.1) goto 41 !no remaining conformations.
c     remove the structures in the cluster
         ...
         do j=1,nstr_selected
            ...
            if(amat(jj,i_cl(i)).lt.rmsd_cut)then
               ...
               mark(jj)=0 !remove conformation from pool of structures
...
c               write(*,'f5.2') armsd
         ...
         d_rc_in(i)=float(n_str_cl(i))/Rc_in(i) !related density
      enddo
...
c     $           rmsd_model,rmsd_closc,rmsd_piece
     ...
     $           rmsd_model,rmsd_closc,rmsd_piece,'\n'            
         ...
         bl=lnblnk(b)
         write(b,'a')b(1:bl)//'\n'
      endif !end of comparison to native structure
...
c     section summary of clusters'
     ...
     $        density,'\n'
      enddo
 132  format(a,i2,i6,f6.2,f7.0,a)
