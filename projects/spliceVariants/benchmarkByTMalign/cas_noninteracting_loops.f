...
*     pgf77 -g -Mbounds -Mchkfpstk -Mextend  -Wl,-static -o cas_noninteracting_loops cas_noninteracting_loops.f       *
*     To compile for production, use:
*     pgf77 -Mextend -O3 -Bstatic -o cas_noninteracting_loops cas_noninteracting_loops.f *
*                                                                           *
*     Version as in tasser1.0/learning/cas.f with the following changes:    *
*     regions not aligned in the template do not participate in the following
*     potentials: hydrogen-bond, contact-pair, contacts-retraints, 
*     distance-restraints. Implemented only for switch=1 option
*     We keep track of which template is assigned to which replica at all times
*****************************************************************************
...
c     number of vectors on lattice
      ...
      common/seqe/seq(ndim),sec(ndim),ialigned(ndim,nrep),ialig(ndim)
...
c     produce initial structure randomly.
...
c     find maximum values of apar1(i,j) and apar2(i,j) to normalize apar1 to apar2
      apar1_max=-100
      apar2_max=-100
      do i=1,Lch
         do j=1,Lch
...
c     Number of templates in 'chain.dat'
      ...
      if(n_thr.eq.0)then !create random chains if no templates
         call random_initial
         return
      endif
...
ccccc record the initial conformation of current replica
         ...
         do i=1,Lch
            ...
            yrep(i,i_rep)=y(i)
            zrep(i,i_rep)=z(i)
            ialigned(i,i_rep)=q(i) !marks aligned residues
         enddo
...
c     load aligned residues marks
      do i=1,Lch
         ialig(i)=ialigned(i,itemp)
      enddo
...
c     compute ellipticities E(i) and principial axis of SG's T(3,3)
      call get_center
...
c     backup of contact-order
      ...
      enddo
      codevsum=conew            !backup  penalty for total deviation of comb
      didevsum=dinew            !backup  penalty for total deviation of dist
...
ccccc go through every residue of the extended fragment [jj,kk]
      do 1002 k=jj,kk
         ialigk=ialig(k) !ialigk==1 if residue was template-aligned
...
ccccc find interactions of amino acid "k" with rest of the protein
         do 1001 i=1,Lch
            iend=max(k+1,kk)
            ialigik=ialig(i)*ialigk !ialigik==1 if both residues were aligned by the template
...
ccccc pair-wise potential of SG-SG
               ...
               if(Gr2.lt.concut2(iseq,kseq))then
                  if(ialigik.eq.1) EHB1c=EHB1c+apar(i,k)
               endif
               ...
               IF(cc.gt.0.5)THEN !c_i//c_k
                  ...
                  if(Gr2.lt.alp(iseq,kseq))then
                     ...
                     if(Gr2.gt.arlp(iseq,kseq))then
                        if(ialigik.eq.1) EHB2=EHB2+app(i,k) !quarsi3
                     else
                  ...
                  IF(cc.lt.-0.5) THEN !antiparallel pair-wise of (i,k)
                     if(Gr2.lt.ala(iseq,kseq))then
                        ...
                        if(Gr2.gt.arla(iseq,kseq))then
                           if(ialigik.eq.1) EHB2=EHB2+apa(i,k)
                        else
                           EHB1b=EHB1b+1 !excluded volume
                        endif
                     endif
                  ELSE          !neither parallel nor antiparallel
                     if(Gr2.lt.alm(iseq,kseq)) then
                        ...
                        if(Gr2.gt.arlm(iseq,kseq))then
                            if(ialigik.eq.1) EHB2=EHB2+apm(i,k)
                        else
                           EHB1b=EHB1b+1 !excluded volume
                        endif
                     endif
...
c^^^^^^^^^^^^^ interaction of pair-wise finished ^^^^^^^^^^^^^^^^^
               ...
               if(ialigik.eq.0) goto 1001 !no hydrogen bond for unaligned residues
...
************************************************************************
               ...
               if(idist.eq.3)then !putative interacting residues are idist residues apart
***   alpha-helix:
                  if(sec(k).ne.4.and.sec(i).ne.4)then !both not assigned as helixes
                     if(Cr2.lt.Cr2a ! |r_i-r_k|^2 < Cr2a=36Angstroms^2
     $                    .and.cc.gt.acut_cc)then  !c_i*c_k < acut_cc=0.1, c_i:bisector vector
                        bb=bxi*bxk+byi*byk+bzi*bzk !cos(Hbi.Hbk) dot prod between donor and accetor
                        if(bb.gt.acut_bb)then !acut_bb=0.45
                           av11=avv( !dot product between bond vector (i-1,i) and (k-1,k)
     $                          aax(i-1),aay(i-1),aaz(i-1),
     $                          axi,ayi,azi,
     $                          aax(k-1),aay(k-1),aaz(k-1),
     $                          axk,ayk,azk
     $                          )
                           if(av11.gt.acut_vv)then !acut_vv=0.0
                              av22=avv( !dot product between bond vector (i,i+1) and (k,k+1)
     $                             axi,ayi,azi,
     $                             aax(i+1),aay(i+1),aaz(i+1),
     $                             axk,ayk,azk,
     $                             aax(k+1),aay(k+1),aaz(k+1)
     ...
     $                                energyHBa(
     $                                i,k,fact,
     $                                axki,ayki,azki, !relative vector from i to k
     $                                bxi,byi,bzi,    !hydrogen bond unit vector for i
     $                                bxk,byk,bzk     !hydrogen bond unit vector for k
     $                                )
                              endif
                           endif
                        endif
               ...
               elseif(idist.gt.4.and.idist.lt.20)then !antiparallel-sheet, short range
                  if(sec(k).ne.2.and.sec(i).ne.2)then
                     if(Cr2.lt.Cr2b               !Cr2b=38Angstroms^2
     $                    .and.cc.gt.bcut_cc)then !bcut_cc=0.4
                        bb=bxi*bxk+byi*byk+bzi*bzk !cos(Hbi.Hbk)
                        if(bb.lt.-bcut_bb)then !antiparallel, bcut_bb=0.25
                           av12=avv(
     $                          aax(i-1),aay(i-1),aaz(i-1),
     $                          axi,ayi,azi,
     $                          axk,ayk,azk,
     $                          aax(k+1),aay(k+1),aaz(k+1)
     $                          )
                           if(av12.lt.-bcut_vv)then !bcut_vv=0.35
                              av21=avv(
     $                             axi,ayi,azi,
     $                             aax(i+1),aay(i+1),aaz(i+1),
     $                             aax(k-1),aay(k-1),aaz(k-1),
     ...
     $                                )
               ...
               elseif(idist.ge.20)then !long range
                  if(sec(k).ne.2.and.sec(i).ne.2)then
                     if(Cr2.lt.Cr2b.and.cc.gt.bcut_cc)then
                        bb=bxi*bxk+byi*byk+bzi*bzk !cos(Hbi.Hbk) 
                        if(bb.lt.-bcut_bb)then !antiparallel-sheet, now long-range
                           av12=avv(
     $                          aax(i-1),aay(i-1),aaz(i-1),
     $                          axi,ayi,azi,
     $                          axk,ayk,azk,
     ...
     $                                )
                        ...
                        elseif(bb.gt.bcut_bb)then !parallel-sheet, now long-range
                           av11=avv(
     $                          aax(i-1),aay(i-1),aaz(i-1),
     $                          axi,ayi,azi,
     $                          aax(k-1),aay(k-1),aaz(k-1),
...
c     reverse direction as v(i---k), if (k<i);
      ...
      byi=5.7*dyi
      bzi=5.7*dzi
      if(k.gt.i)then            !bki,bxk,axki are in same direction
      ...
      else                      !bki,bxk, and axki are in opposite direction
         br=(bxk+axki)**2+(byk+ayki)**2+(bzk+azki)**2
         if(br.lt.acut_hh)then
            b=1/(1+abs(br-3.2))
            energyHBa=energyHBa-EHBIJ(i,k)*a*b
...
c     4, bias to (predicted) protein-like structure, panality on crumpling.
c     5, E13,E15,E15.
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      dimension axj(ndim),ayj(ndim),azj(ndim)
      common/center/cex,cey,cez
      common/eigen/AA(3,3),EE(3),HH(3,3)
...
c      ESHORT=0.0
      ...
      if(istat.lt.0)THEN       !Eold 
         ...
         conew=0.0   !penalty for incorrect contact (from contact restrain)
         dinew=0.0   !penalty for incorrect distance (from distant restain)
      endif

      do i=1,Lch !init axj,ayj,azj
         if(mv(i).gt.0)then
            axj(i)=x(i)
            ayj(i)=y(i)
            axj(i)=z(i)
...
c     area(i)<0, propensity to bury, >0, prop. to be exposed
            if(ialig(i).eq.1) ESHORT2=ESHORT2-aaa*area(i)
         endif
               ...
               if(ialig(j).eq.1) then
                  if(j.lt.i.OR.j.gt.jjjj)then
                     ...
                     if(aij2.gt.concut2(iseq,jseq))then
                        ...
                        if(istat.lt.0)then
                           coold=coold+sqrt(aij2)-concut(iseq,jseq) !penalty
                        else
                           conew=conew+sqrt(aij2)-concut(iseq,jseq) !penalty
                        endif
                     endif
                  endif
               endif
...
ccc   Contact restrain from 'combCA.dat' ------------------------------>
         ...
         if(N_com.ge.1)then
            do k=1,N_com
               ...
               if(ialig(j).eq.1)then
                  if(j.lt.i.OR.j.gt.jjjj)then
                     aij2=(axi-axj(j))**2+(ayi-ayj(j))**2+(azi-azj(j))**2
...
*********E13 --------------------------------------->
      ...
      i2=min(jjjj-1,Lch-2)
      do i=i1,i2
         ar13=(axj(i)-axj(i+2))**2+(ayj(i)-ayj(i+2))**2+
     $        (azj(i)-azj(i+2))**2
...
ccccccE14:
         ...
         if(ialig(i).eq.1) ESHORT7=ESHORT7+asr(i,ibin(r14)) !asr(i,dis(i,i+4)) from r14*.dat

cccccccccE15:
         ar15=(ax1-ax5)**2+(ay1-ay5)**2+(az1-az5)**2
         r15=nint(ar15)
         if(r15.gt.500) r15=500
         if(ialig(i).eq.1) ESHORT8=ESHORT8+bsr(i,jbin(r15))
...
cccccccccbias1: encourage helix/sheet-like structure
     ...
     $                       +(ay5-ay4)*(ay2-ay1)+(az5-az4)*(az2-az1)
                        if(dot14.gt.0)then
                           ...
                           if(ialig(i).eq.1) ESHORT5c=ESHORT5c-2-ff
                        endif
                     endif
                  endif
               endif
...
c                     ESHORT5c=ESHORT5c-2-ff
                     if(ialig(i).eq.1) ESHORT5c=ESHORT5c-2-ff*2
                  endif
               endif
            endif
         endif
...
****  Protein-like bias2: b(i), b(i+4) parallel------->
      ...
      do i=i1,i2
         ...
         if(sec(i).eq.2.and.sec(i+2).eq.2.and.sec(i+4).eq.2)then
            if(b15.gt.0.9)then
               if(ialig(i).eq.1) ESHORT5=ESHORT5-ff !alpha
            endif
         else
            if(b15.gt.0.5.or.b15.lt.-0.3)then !beta or turn
c               ESHORT5=ESHORT5-ff
               if(ialig(i).eq.1) ESHORT5=ESHORT5-ff*2
            endif
         endif
      enddo
c^^^^^^^^^^^^^^^^^^ bias2 finished ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
ccccccbias4a: to predicted alpha fragment --------------->
     ...
     $           (aay(i)-aay(i+7))**2+(aaz(i)-aaz(i+7))**2)
            if(ialig(i).eq.1) ESHORT5b=ESHORT5b+abs(dis-frga(i))
         endif
      enddo
...
c            ESHORT5b=ESHORT5b+abs(dis-frgb(i))
            if(ialig(i).eq.1) ESHORT5b=ESHORT5b+abs(dis-frgb(i))*2
         endif
      enddo
c^^^^^^^^^^^^^^^ predicted alpha/beta bias finished ^^^^^^^^^^^^^^^

ccc   penalty for crumpling structures ---------------------------->
      i1=max(iiii-11,1)
      ...
      enddo
c^^^^^^^^^^^^^ penalty of bizarre structure finished (ESC1) ^^^^^^^^^^^^
...
cc         if(conew.gt.colim) ESHORT=ESHORT+er3*(conew-colim)
         if(conew.gt.colim) ESHORT4a=conew-colim
      endif
...
c     prepare new path and move to new path -------------->
 ...
 111     iv=int(aranzy()*anvec)+1
      ...
      if(look(1,m+1))then       ! check excluded volume for passage of [2,m+1]
c     calculate E_new--------------->
         do pp=1,Lch
            nop(pp)=nopp(pp)    !prepare to calculate energy
            nom(pp)=nomm(pp)
...
***   swap identity of aligned residues
      do i=1,Lch
         j=ialigned(i,i1)
         ialigned(i,i1)=ialigned(i,i2)
         ialigned(i,i2)=j
      enddo
      end
...
c     following is for next movement ---------->
      ...
      do i=1,Lch
         ...
         nomm(i)=nom(i)         !backup of number of contact-pair
      enddo
      codevsum=conew            !backup of penalty for total deviation of comb
      didevsum=dinew            !backup of penalty for total deviation of dist
