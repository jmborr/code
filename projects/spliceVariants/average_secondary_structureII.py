...
"""like average_secondary_structure.py, but secondary structure
prediction from scratchdir+'/spliceVariants/out/psipreds.For every
reference sequence, find constitutive and variant fragments. For each
fragment, calculate alpha,beta,coil content prediction or each and
store in appropriate array. Then do the average."""
...
cmd='grep "00$" '+scratchdir+'/spliceVariants/out/psipreds/success.list'
for ref in chomp(os.popen(cmd).readlines()):
    #obtain secondary structure and dump it in a string
    ss=translateSec(scratchdir+'/spliceVariants/out/psipreds/'+ref+'/seq.dat')
