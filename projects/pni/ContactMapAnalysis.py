...
class csr_ContactMap(csr_matrix):
    ...
    def saveToFile(self,filename,mode='a',fmt='HDF5',index=0):
        ...
        if fmt=='HDF5':
            ...
            sys.stderr.write('ERROR: format '+fmt+' not yet implemented\n')
    def asString(self,stringFormat='CMVIEW'):
        """give a string representation of the contact map"""
        if not self.data.any(): 
            sys.stderr.write('No contacts found!\n')
            return None
        N = max(self.shape)
        sequence = 'X'*N #Unknown residue
        cutOff = self.cutOff
        cm = self.tocoo()   #COOrdinate representation
        M = len( cm.row )
    
        if stringFormat == 'CASP_RR':
            cmString  = 'PFRMAT RR\n'
            cmString += 'TARGET T0000\n'
            cmString += 'MODEL 0\n'
            seq = sequence[0:50]+'\n'
        ...
        elif stringFormat == 'CMVIEW':
            cmString  = '#CMVIEW GRAPH FILE ver: 1.0\n'
            cmString += '#SEQUENCE: ' + sequence + '\n'
            cmString += '#PDB: \n'
            cmString += '#PDB CHAIN CODE: \n'
            cmString += '#CHAIN: \n'
            cmString += '#MODEL: 1\n'
            cmString += '#CT: Ca\n'
            cmString += '#CUTOFF: ' + str( cutOff ) + '\n'
            for i in range( M ):
                cmString += "%d\t%d\t%f\n"%(cm.row[i], cm.col[i], cm.data[i] )
