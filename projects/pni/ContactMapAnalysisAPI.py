...
def GetContactMapAsString(file,fileFormat='HDF5',index=0,stringFormat='CASP_RR'):
    """Retrieve a contact map as a string, suitable to be saved to a file
    file: containis one or more contact maps
    fileFormat: format of the file containing the contact map
    index: which contact map (default the first)
    StringFormat: contact map format. Default is CASP Residue-Residue contact predictions
    """
    cm = CMA.load_csr_ContactMapFromFile(file,fmt=fileFormat,index=index) #a csr_matrix
    return cm.asString(stringFormat=stringFormat)
