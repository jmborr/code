c
c       pgf77 -Mextend -O -s -fast -Wl,-static -o linker42sum linker42sum.f
	...
	data aa/ 'GLY','ALA','SER','CYS','VAL','THR','ILE','PRO',
     ...
     &		     'HIS','PHE','TYR','TRP'/
        ...
        if(i .ge. 56)			ibb(i)=4
					...
					ibb(0)=0
	OPEN(unit=20,file='/gpfs1/scratch/jose/structural_neighbors/pdb/LIST.structures')
...
1       format(a255)
	nsize=5
	a1='_'
        open(unit=10,file='/gpfs1/scratch/jose/structural_neighbors/pdb/'//name(1:nsize))
...
c	write(6,*)k,'length=',ncheck
	   if(lstate(k) .eq.2)then
	      ncheck=15 !maximum length of helix
...
c	write(23,233)nres,name
	write(23,'a,a')'>',name(1:5)
 233	format(i4,1x,a9)
c	write(23,53)(jsec(i),i=1,nres)
	write(23,'400a') (struct1(jsec(i)),i=1,nres)
 53	format(50(i1,1x))
