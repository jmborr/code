...
*****************************************************************************
*     To compile, use:
*     pgf77 -Mextend -O -s -fast -Wl,-static -o casB_gene5 casB_gene5.f *
*     pgf77 -g -Mbounds -Mchkfpstk -Mextend  -Wl,-static -o casB_gene5 casB_gene5.f
*     To debug, type in fec02: pgdbg casB_gene4.2
...
*  				!!WAIT!!
*
* Modifications to previous release:                                        *
* Simulations for poly-ala only. Some energy terms are modified             *
* hydrogen bond energy for beta is stronger if long range                   *
* hydrogen bonds only between alpha-alpha and beta-beta                     *
* new hydrogen bond list prevents a residue making more than two bonds      *
* new energy term for second neighbors in the beta-type HB                  *
* quarsi3 potential modified such that two amino acids have no interactions *
* if both are in coils, 1/4 if one is in coil and not the other             *
*****************************************************************************
...
c 3456789012345678901234567890123456789012345678901234567890123456789012345678
      ...
      common/ehbenergy1/EHB5a,EHB5b,EHB6b
...
ccccc variables needed for hydrogen bond network
      common/hbnet/haccept,hdonate,nhaccept,nhdonate,ahacc,ahdon,anhacc,
     $     anhdon
      integer haccept(ndim),hdonate(ndim) !hydrogen acceptor/donator recipient
      integer nhaccept(ndim),nhdonate(ndim) !new hydrogen acceptor/donator recipient
      real ahacc(ndim),ahdon(ndim) !energies of the hydrogen network
      real anhacc(ndim),anhdon(ndim) !energies of thenew  hydrogen network
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      read(19,*) eh6b !second neighbor beta-type HB
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      do 1111 icycle=1,ncycle
         do 2222 itemp=1,N_rep  !iterate for all the replicas
            ...
            call initial_move   !update center, axis, energy, hbnet
...
c
      ...
      do i=1,Lch
         ...
         do j=1,Lch
            js=sec(j)
            EHBIJ(i,j)=0 !by default, no hydrogen bond strength
            if(iabs(i-j).eq.3.and.is.eq.2.and.js.eq.2)then !both helix
               EHBIJ(i,j)=1.5 
            elseif(is.eq.4.and.js.eq.4) then !both beta
               idist=iabs(i-j)
               if(idist.gt.longrange)then !longrange
                  EHBIJ(i,j)=1.5*arfavor
...
c     H-bond (unit vector) of the acceptor oxygen
         hbx(i,j)=bx
         hby(i,j)=by
         hbz(i,j)=bz
...
c     find 2-bond-move position -------------->
      ...
      enddo
      if(nfl2.eq.0)then
         write(0,'a')'ERROR: no 2-bond-moves allowed!'
         call exit(1)
      endif
...
cTTTTTTTTTTccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      do 1111 icycle=1,ncycle
         do 2222 itemp=1,N_rep  !iterate for all the replicas
            ...
            call initial_move   !update center, axis, energy, hbnet
c            if(itemp.eq.1) write(6,'ai4ai3')'icycle=',icycle,
c     $           ' number HB=',nhbonds()
            do 3333 iphot=1,phot !N_swap, iterate at fixed temperature
               do 4444 i_nfl=1,nfl
                  fff=aranzy()
                  ...
                  if(i_fra.ge.n_fra.and.nfr.gt.0)then !1 chunk move in nfr move
                     ...
                     if(switch.eq.2)then !rotation+translation
                        if(nfr_i(ifr).eq.1)then
                           moved=trot_N() !translate+rotate N-terminal
                        elseif(nfr_f(ifr).eq.Lch)then
                           moved=trot_C() !translate+rotate C-terminal
                        else
                           moved=trot_M() !translate+rotate Middle-fragment
                        endif
                     elseif(switch.eq.4)then !translation
                        if(nfr_i(ifr).eq.1)then
                           moved=tran_N() !translate N-terminal
                        elseif(nfr_f(ifr).eq.Lch)then
                           moved=tran_C() !translate C-terminal
                        else
                           moved=tran_M() !translate Middle-fragment
                        endif
                     elseif(switch.eq.5)then !rotation+translation+deformation
                        ...
                        if(nfr_i(ifr).eq.1)then
                           if(aran_num.gt.0.5)then
                              ...
                              i_E_defo=1
                              moved=idefo_N() !translate+deform N-terminal
                           endif
                        elseif(nfr_f(ifr).eq.Lch)then
                           if(aran_num.gt.0.5)then
                              i_E_defo=3
                              ...
                              i_E_defo=1
                              moved=idefo_C() !translate+deform C-terminal
                           endif
                        else
                           if(aran_num.gt.0.5)then
                              i_E_defo=3
                              ...
                              i_E_defo=1
                              moved=idefo_M() !translate+deform Middle-fragment
                           endif
                        endif
                     endif
                  endif
...
cccccccc prepare all the initial parameters of this replica ccccccccc
      ...
      call zerohbnet() !reset hydrogen bond network
      energ=EHB(1,Lch,1)+ESHORT(1,Lch,10) !initialize all below parameters
      call updhbnet(1,Lch) !update hydrogen bond network
c      if(itemp.eq.1)then
c         call printsnapshot(1)
c         call printhbnet()
c         call printhbsecondn()
c      endif
...
ccccc variables needed for hydrogen bond network
     ...
     $     anhdon
      ...
      EHB6B=0                   !H-bond second neighbors beta-type

      if(ISTAT.gt.0)THEN        !Enew
         ICNT=ICNTO
         SUMCT=SUMCTO
c     reset new HB network
         do i=1,Lch
            nhaccept(i)=0
            nhdonate(i)=0            
         enddo
...
************************************************************************
               ...
               if(istat.lt.0) goto 1003 !we already know bonds for old configuration
               nacceptor=0 !signal if a bond may be formed
               if(idist.eq.3)then ! alpha-helix:
                  if(sec(k).ne.4.and.sec(i).ne.4)then
                     if(Cr2.lt.Cr2a.and.cc.gt.acut_cc)then
                        ...
                        if(bb.gt.acut_bb)then
                           av11=avv(aax(i-1),aay(i-1),aaz(i-1),
     $                          axi,ayi,azi,aax(k-1),aay(k-1),
     $                          aaz(k-1),axk,ayk,azk)
     ...
     $                             aay(k+1),aaz(k+1))
                              if(av22.gt.acut_vv)then
                                 fact=(1-abs(cc-0.4))*
     $                                (1-abs(bb-0.815))
                                 EHB5ax=energyHBa(i,k,fact,axki,
     $                                ayki,azki,bxi,byi,bzi,
     $                                bxk,byk,bzk)
                                 if(bxi*axki+byi*ayki+bzi*azki.gt.0)then
                                    nacceptor=i
                                 else
                                    nacceptor=k
                                 endif
               ...
               elseif(idist.gt.4.and.idist.lt.longrange)then !antiparallel-sheet, medium-range
     ...
     $                             aax(k-1),aay(k-1),aaz(k-1),axk,ayk,azk)
                              if(av21.lt.-bcut_vv)then
                                 ...
                                 EHB5bx=energyHBb(i,k,fact,
     $                                axki,ayki,azki,bxi,byi,bzi,bxk,
     $                                byk,bzk)
                                 if(i.lt.k)then !assign acceptor to smaller of i and k is a...
                                    nacceptor=i !...purely arbitrary convention
                                 else
                                    nacceptor=k
                                 endif
                              endif
     ...
     $                             axk,ayk,azk)
                              if(av21.lt.-bcut_vv)then
                                 fact=abs(bb)*cc !bb->1,cc->1
                                 EHB5bx=energyHBb(i,k,fact,axki,
     $                                ayki,azki,bxi,byi,bzi,bxk,byk,bzk)
                                 if(i.lt.k)then !assign acceptor to smaller of i and k is a...
                                    nacceptor=i !...arbitrary convention
                                 else
                                    nacceptor=k
                                 endif
                              endif
                           endif
                        elseif(bb.gt.bcut_bb)then !parallel-sheet:
     ...
     $                                ayki,azki,bxi,byi,bzi,bxk,byk,bzk)
               ...
               if(nacceptor.gt.0)then !update new HB network, add energy
                  if(nacceptor.eq.i)then
                     j=i !j allways plays role of acceptor
                     l=k !l allways plays role of donor
c     It may be that "i" is already acceptor in a previous old bond. If
c     so, and if both "i" and its old donor are outside the chunk, it
c     means we cannot dissolve the previous bond in order to form the
c     new bond involving "i"
                     if(haccept(i).gt.0)then
                        if(i.lt.jj.or.i.gt.kk)then
                           ll=haccept(i)
                           if(ll.lt.jj.or.ll.gt.kk) goto 1003
                        endif
                     endif
                  else
                     j=k
                     l=i
c     same check as before but now "i" is donor in a previous old bond.
                     if(hdonate(i).gt.0)then
                        if(i.lt.jj.or.i.gt.kk)then
                           ll=hdonate(i)
                           if(ll.lt.jj.or.ll.gt.kk) goto 1003
                        endif
...
c     check if acceptor "j" was already acceptor of a previous new
c     bond. If so, check if the previous bond is more stable (smaller
c     energy value). If so, leave the previous new bond. Otherwise,
c     dissolve the previous new bond and form the current new bond
c     (greedy algorithm)
                  if(nhaccept(j).gt.0)then !j already is acceptor for other new bond
                     if(idist.eq.3)then
                        if(anhacc(j).lt.EHB5ax) goto 1003 !previous bond is more stable
                     else
                        if(anhacc(j).lt.EHB5bx) goto 1003
                     endif
                     ll=nhaccept(j) !dissolve previous bond, which is less stable
                     nhdonate(ll)=0
                  endif
c     same for donor "l"
                  if(nhdonate(l).gt.0)then !l already is donor for other bond
                     if(idist.eq.3)then
                        if(anhdon(l).lt.EHB5ax) goto 1003 !previous bond is more stable
                     else
                        if(anhdon(l).lt.EHB5bx) goto 1003
                     endif
                     ll=nhdonate(l) !dissolve previous bond, which is less stable
                     nhaccept(ll)=0
                  endif
                  nhaccept(j)=l !update the new hydrogen bond network
                  nhdonate(l)=j
                  if(idist.eq.3)then !update the new hydrogen bond energies
                     EHB5a=EHB5a+EHB5ax
                     anhacc(j)=EHB5ax
                     anhdon(l)=EHB5ax
                  else
...
c     hydrogen bond energy calculation for old configuration
         if(istat.lt.0)then 
            if(haccept(k).gt.0)then !k is acceptor in a bond
               i=haccept(k) !partner in the bond
               if(iabs(i-k).eq.3)then !helical bond
                  if(i.lt.jj.or.i.gt.kk)then !partner outside the chunk
                     EHB5a=EHB5a+ahacc(k)
                  else
                     EHB5a=EHB5a+0.5*ahacc(k) !0.5 to prevent counting twice the pair interaction
                  endif
               else !beta bond
                  if(i.lt.jj.or.i.gt.kk)then
                     EHB5b=EHB5b+ahacc(k)
                  else
                     EHB5b=EHB5b+0.5*ahacc(k)
                  endif
               endif               
            endif
            if(hdonate(k).gt.0)then !k is donor in a bond
               i=hdonate(k)
               if(iabs(i-k).eq.3)then
                  if(i.lt.jj.or.i.gt.kk)then
                     EHB5a=EHB5a+ahdon(k)
                  else
                     EHB5a=EHB5a+0.5*ahdon(k)
                  endif
               else
                  if(i.lt.jj.or.i.gt.kk)then
                     EHB5b=EHB5b+ahdon(k)
                  else
                     EHB5b=EHB5b+0.5*ahdon(k)
                  endif
               endif               
            endif
         endif
c     second neighbor contribution for old conformation
         ih=haccept(k) !H-donor in the k--ih bond
         id=hdonate(k) !H-acceptor in the k--id bond         
         if(ih.gt.0.and.id.gt.0)then   !ih--k--id situation
            if(iabs(ih-k).ne.3.and.iabs(id-k).ne.3)then
               overcount=0
               EHB6bx=1
               if(ih.ge.jj.and.ih.le.kk) overcount=overcount+1
               if(id.ge.jj.and.id.le.kk) overcount=overcount+1
               if(overcount.gt.0) EHB6bx=EHB6bx/overcount
               EHB6b=EHB6b+EHB6bx
         endif
         endif
         if(ih.gt.0.and.haccept(ih).gt.0)then !ihh-ih-k situation
            ihh=haccept(ih)
            if(iabs(ih-k).ne.3.and.iabs(ihh-k).ne.3)then
               overcount=0
               EHB6bx=1
               if(ih.ge.jj.and.ih.le.kk) overcount=overcount+1
               if(ihh.ge.jj.and.ihh.le.kk) overcount=overcount+1
               if(overcount.gt.0) EHB6bx=EHB6bx/overcount
               EHB6b=EHB6b+EHB6bx
            endif            
         endif
         if(id.gt.0.and.hdonate(id).gt.0)then !idd-id-k situation
            idd=hdonate(id)
            if(iabs(id-k).ne.3.and.iabs(idd-k).ne.3)then
               overcount=0
               EHB6bx=1
               if(id.ge.jj.and.id.le.kk) overcount=overcount+1
               if(idd.ge.jj.and.idd.le.kk) overcount=overcount+1
               if(overcount.gt.0) EHB6bx=EHB6bx/overcount
               EHB6b=EHB6b+EHB6bx
            endif
         endif
...
c     second neighbor contribution for new conformation. We do it after
c     the new network has been set up
      if(istat.gt.0)then
         do k=jj,kk
            ih=nhaccept(k)       !H-donor in the k--ih bond
            id=nhdonate(k)       !H-acceptor in the k--id bond
            if(ih.gt.0.and.id.gt.0)then !ih--k--id situation
               if(iabs(ih-k).ne.3.and.iabs(id-k).ne.3)then
                  overcount=0
                  EHB6bx=1
            ...
            if(ih.gt.0.and.iabs(ih-k).ne.3)then !not a helix HB type
               overcounting=0
               if(ih.ge.jj.and.ih.le.kk)then !first neighbor within chunk
                  overcounting=overcounting+1
                  ihh=nhaccept(ih) !use new network
               else
                  ihh=haccept(ih)  !use old network
               endif
               if(ihh.gt.0.and.iabs(ihh-k).ne.3)then  !ihh-ih-k situation
                  EHB6bx=1
                  if(ihh.ge.jj.and.ihh.le.kk)overcounting=overcounting+1 !second neig within chunk
                  if(overcounting.gt.0) EHB6bx=EHB6bx/overcounting
                  EHB6b=EHB6b+EHB6bx
               endif
            endif
            if(id.gt.0.and.iabs(id-k).ne.3)then 
               overcounting=0
               if(id.ge.jj.and.id.le.kk)then !first neighbor within chunk
                  overcounting=overcounting+1
                  idd=nhdonate(id) !use new network
               else
                  idd=hdonate(id)  !use old network
               endif
               if(idd.gt.0.and.iabs(idd-k).ne.3)then  !ihh-ih-k situation
                  EHB6bx=1
                  if(idd.ge.jj.and.idd.le.kk)overcounting=overcounting+1 !second neig within chunk
                  if(overcounting.gt.0) EHB6bx=EHB6bx/overcounting
                  EHB6b=EHB6b+EHB6bx
               endif
            endif
...
c     $     +eh1c*EHB1c          !pair-wise potential of SC-SC
     ...
     $     +eh6b*EHB6b          !H-bond second neighbor (beta)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      bxk=5.3*dxk !5.3*0.87=4.61Angstroms canonical distance
      byk=5.3*dyk
      bzk=5.3*dzk
      bxi=5.3*dxi
      byi=5.3*dyi
...
***   panelty for the deviation from template---------------->
      if(switch.eq.5)then
         if(ISTAT.eq.10)then    !total energy
            ...
            if(i_E_defo.eq.1)then !for idefo_M but not trot_M
               if(iiii.eq.1)then
                  i=i_chunk(1)
...
c     #############################################
c     update hydrogen bond network and related energies
...
c     remove old hydrogen bonds
      do k=jj,kk !cycle through chunk residues
         i=haccept(k)
         if(i.gt.0)then
            haccept(k)=0
            hdonate(i)=0
      ...
      enddo
c     add new hydrogen bonds and related energies
      do k=jj,kk
         i=nhaccept(k)
         if(i.gt.0)then
            haccept(k)=i
         ...
         !if(i.gt.0) write(6,'ai3ai3')'i=',i,' k=',k
         i=nhdonate(k)
         if(i.gt.0)then
            hdonate(k)=i
            haccept(i)=k
...
c     #############################################
c     reset hydrogen bond network
      subroutine zerohbnet
      implicit integer(i-z)
      parameter(ndim=1000)
      common/lengths/Lch,Lch1,Lch2
ccccc variables needed for hydrogen bond network
     ...
     $     anhdon
      ...
      do i=1,Lch
         ...
         hdonate(i)=0
      enddo
      end !end of subroutine zerohbnet
c     #############################################


c     #############################################
      subroutine printhbnet
      implicit integer(i-z)
      parameter(ndim=1000)
      common/lengths/Lch,Lch1,Lch2
ccccc variables needed for hydrogen bond network
     ...
     $     anhdon
      ...
      write(6,'a')'hydrogen bond network'
      do i=1,Lch
         if(haccept(i).gt.0) write(6,'i3xi3') i,haccept(i)
      enddo
      
      end !end of subroutine printhbnet
c     #############################################


c     #############################################
      subroutine printhbsecondn
c     print second neighbor interactions
      implicit integer(i-z)
      parameter(ndim=1000)
      common/lengths/Lch,Lch1,Lch2
ccccc variables needed for hydrogen bond network
     ...
     $     anhdon
      ...
      do i=1,Lch
         if(haccept(i).gt.0.and.hdonate(i).gt.0) then
            if(iabs(haccept(i)-i).ne.3) then
               if(iabs(hdonate(i)-i).ne.3) then
                  write(6,'3(xi3)') haccept(i),i,hdonate(i)
               endif
            endif
         endif
      enddo
...
ccccc variables needed for hydrogen bond network
     ...
     $     anhdon
      ...
      nhbonds=0
      do i=1,Lch
         if(haccept(i).gt.0) nhbonds=nhbonds+1
      enddo
      end !end of subroutine zerohbnet
c     #############################################
