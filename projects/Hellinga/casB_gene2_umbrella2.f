...
*****************************************************************************
*     To compile, use:
*     pgf77 -Mextend -O -s -fast -Bstatic -o casB_gene2_umbrella.x casB_gene2_umbrella.f  inputArgs.f umbrella.f
*
...
* THE OTHER
* THIS CODE IS MODIFIED TO RUN UMBRELLA SAMPLING ON GLOBAL RMSD, ALL REPLICAS
* RUNNING AT SAME TEMPERATURE
*****************************************************************************
...
c 3456789012345678901234567890123456789012345678901234567890123456789012345678
      ...
      common/debugg/xd(ndim),yd(ndim),zd(ndim)
      common/debugd/exd(ndim),eyd(ndim),ezd(ndim)
      ...
      common/nana1/nana
      common/run_id/runID
      character runID,natf*255 !number in between 0 and 1 for simulation ID
      
      call read_command_line(runID,natf) !read input arguments
...
ccccccccccccccc sequence specified input files cccccccccccccccccccccccccccc
...
ccccccccccccccccfor E-t ccccccccccccccccccccccccccccccccccccccccccccccccccc
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      n_rep=14                  !reset number of replicas to this number
...
c      call read_CAcontact       !read CAcontact restrains from 'combCA.dat'
      ...
      n_repf=n_rep                 !number of output replicas

      call umbrella_init_params(n_repf,natf)
...
ccccccccccccccc trajectory files cccccccccccccccccccccccccccccccccccccc
      ...
      tra_suffix='.tra'
      if(runID.ne.'') tra_suffix='.'//runID//'.tra'
      do i=1,n_repf
         if(i.lt.10)then
            fn=char(48+i)
            ...
            fn=char(48+(i-10))
            open(unit=30+i,file='rep1'//fn//tra_suffix,status='unknown')
         endif
      enddo
...
c     ########################################################
c     check if the configuration has changed
      subroutine has_changed()
      implicit integer(i-z)
      parameter(ndim=1000)
      common/debugg/xd(ndim),yd(ndim),zd(ndim)
      common/debugd/exd(ndim),eyd(ndim),ezd(ndim)
      ...
      do i=1,Lch
         if(mv(i).lt.0)then
            adif=ex(i)-exd(i)
            if(adif*adif.gt.amindif2) frozen_flag=1
            adif=ey(i)-eyd(i)
            if(adif*adif.gt.amindif2) frozen_flag=1
            adif=ez(i)-ezd(i)
            if(adif*adif.gt.amindif2) frozen_flag=1
            if(frozen_flag.eq.1)then
               write(6,'a') 'frozen fragment changed'
               return               
            endif
         else
            if(xd(i).ne.x(i).or.yd(i).ne.y(i).or.zd(i).ne.z(i))then
               write(6,'a') 'flexible fragment changed'
               write(6,'a,4(i5)')'before ',i,xd(i),yd(i),zd(i)
               write(6,'a,4(i5)')'after  ',i,x(i),y(i),z(i)
               return
            endif
         endif         
      enddo
...
c     #####################################################
c     update previos configuration with new configuration
      subroutine init_prev_conf()
      implicit integer(i-z)
      parameter(ndim=1000)
      common/debugg/xd(ndim),yd(ndim),zd(ndim)
      common/debugd/exd(ndim),eyd(ndim),ezd(ndim)
...
c     ############################################################
c     read input arguments from command line
      subroutine read_command_line(runID,natf)
c     these are the variables to be initialized from command line
      character runID,natf*256
c     variables intrinsic to the subroutine
      character inputs*255(26)
      character wellcome*255(0:26)
      integer c2i
c     the wellcome message
      wellcome(0)='./casB_gene2_umbrella.x [options]'
      wellcome(1)='-a _RA_natf native file name'
      wellcome(2)='-b __runID simulation ID (single digit [0-9]) '
     &     //'for multiple simulations of same target (def:none)'

c     init default variables
      runID=''
c     pass command values from the command line
      call proccess_command_line(wellcome,inputs)
      read(inputs(c2i('a')), *) natf
      if(inputs(c2i('b'))(1:1).ne.'') read(inputs(c2i('b')), *) runID
      end
...
c      if(Lch.gt.165)then        !50
c         N_rep=N_rep+10
c      endif
c      if(Lch.gt.240)then        !60
c         N_rep=N_rep+10
c      endif
c      if(Lch.gt.300)then        !70
c         N_rep=N_rep+10
c      endif
c      if(Lch.gt.400)then      !80
c         N_rep=N_rep+10
c      endif
c      if(N_rep.gt.80)N_rep=80
...
****************************************************
      ...
      do k=1,i_thr0
         ...
         do i=1,Nal
            read(24,*)ii,a1,a2,a3            
            cx0(ii)=umbrella_getNat(1,ii) !override with native coordinates
            cy0(ii)=umbrella_getNat(2,ii)
            cz0(ii)=umbrella_getNat(3,ii)
            q(ii)=1
         enddo
      enddo
 1237 format(A22,I4,A4,3F8.3)
...
***   ax(i)=cx(i)/0.87, for the decision of (x,y,z):
      do i=1,Lch
         ax(i)=umbrella_getNat(1,i)/0.87 !override with native, C_alpha scaled by 0.87
         ay(i)=umbrella_getNat(2,i)/0.87
         az(i)=umbrella_getNat(3,i)/0.87
c         ax(i)=cx(i)/0.87       !C_alpha scaled by 0.87
c         ay(i)=cy(i)/0.87
c         az(i)=cz(i)/0.87
      enddo
c     (ax,ay,az) into (x,y,z)----------------------->
...
c     record replcas of (x,y,z), icarep:
*************************************************************************
      !write(6,*)'template_initial2'
      !do i=1,Lch
      !   write(6,'i,3(f8.3,x)')i,0.87*x(i),0.87*y(i),0.87*z(i)
      !enddo
      !call exit(1)
      do itemp=1,N_rep
         do i=1,Lch
...
cTTTTTTTTTTccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      do 1111 icycle=1,ncycle
         do 2222 itemp=1,N_rep  !iterate for all the replicas
            ...
            call initial_move   !update center, axis, energy
            call umbrella_init_rep(itemp) !store current replica number in umbrella "namespace"
ccc   
c            do i=1,Lch
c               if(mv(i).lt.0)then
c                  write(6,'a,i3,x,3(f8.3)')'frozen ',i,ex(i)*0.87,
c     &                 ey(i)*0.87,ez(i)*0.87
c               else
c                  write(6,'i3,x,3(f8.3)')i,x(i)*0.87,y(i)*0.87,z(i)*0.87
c               endif
c            enddo
c            write(6,*)'temp',phot,nfl
c            call exit(1)

            
            do 3333 iphot=1,phot !N_swap, iterate at fixed temperature
               do 4444 i_nfl=1,nfl
c                  call init_prev_conf() !store conf. (for debugging purposes only)
                  fff=aranzy()
                  if(fff.le.bh2)then
                     call move2
                  elseif(fff.le.bh3s)then
...
c                  call has_changed() !did conf. changed ? (for debugging purposes only)
 4441             continue
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
 4444           continue
 3333         continue
c              write(6,'a,i2,2(a,f6.2)')'rep=',itemp,' target ',
c     &             armsdtarget(itemp),' rmsd=',armsdprev(itemp)
ccc
...
ccccccccccccccccccccc snapshots of E(1), E(N_rep) ccccccccccccc
         if(icycle.eq.icycle/1*1)then
            i_tr=i_tr+1
            do k=1,n_repf
               write(30+k,'i8,1x,f10.1,2i8,1x,a,f6.2')Lch,E_rep(k),i_tr,
     &              icycle,'rmsd=',armsdprev(k)
               do i=1,Lch
                  if(mv(i).gt.0)then
                     abx=xrep(i,k)*0.87
                     aby=yrep(i,k)*0.87
...
c     filter by umbrella rmsd
         !write(6,'a')'move2'
         call umbrella_reject_accept(id,x,y,z,ex,ey,ez,mv)
         if(id.ne.1) goto 660
c     calculate E_new--------------->
         do pp=1,Lch
