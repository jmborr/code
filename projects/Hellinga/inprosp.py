...
inpHand('Usage: inprosp.py [options]',
        ...
        '  -d _A_seqdatf pass seq.dat file instead of calculating it (def:none)'
...
opts='xf'
if 'bz2' in blastout: opts='jxf'
pastry('/bin/tar '+opts+' '+blastout) #below we assume certain naming convention for the files
...
if not msas['3590']: sys.stderr.write('ERROR inprosp.py empty msas[3590]\n')
output_msa(msas['3590'],'3590/'+header+'.aln')
print 'msas[e10]'
msas['e10']=gen_msa(seq,header,Eco=10.0,maxId=0.90,minId=0.10,red=0.95,blastOuts=blastOuts)
if not msas['e10']: sys.stderr.write('ERROR inprosp.py empty msas[e10]\n')
output_msa(msas['e10'],'e10/'+header+'.aln')

gen_mtx(seq,'profile/'+header+'.mtx',header=header,blastOuts=blastOuts) #profile

if seqdatf:
    pastry('/bin/cp '+seqdatf+' seq/'+header+'.SEQ')
else:
