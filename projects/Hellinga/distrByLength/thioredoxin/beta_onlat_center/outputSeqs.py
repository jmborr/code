    ...
    for nSS in range(len(distrnSS)):
        if distrnSS[nSS]>0:
            ...
            while nToOut:
                ...
                while remnant and ntries: #cycle to generate one sequence
                    ...
                    if type=='H' or (type=='E' and len(elemChunks)==0):
                        ...
                        while 1: #cycle to accept a chunk
                            n=randint(3,19) #lenght of a chunk (element or loop)
                            if random()<hist[n]:
                    ...
                    #derive strand from length difference distr. from consecutive strands
                        ...
                        hist2=histLength['SS']
                        while 1: #cycle to accept a chunk
                            n=randint(3,19) #no strand smaller than three residues
                            if hist2[n]>0:
                                d=prevLSS-n
                                if random()<hist[d]:
                                    elemChunks.append(n) ; elemL+=n ; prevLSS=n ;break
