...
maxLength=40
maxLss={'H':18,'E':11} #maximum allowed length of a secondary element
Nseqs=600
Ntarget=150
outd='.'
alph2dir={'H':'a','E':'b'}
        ...
        if distrnSS[nSS]>0:
            ...
            while nToOut:
                ...
                while 1: #loop to generate one sequence
                    ...
                    if type=='H' or (type=='E' and len(chunks)==0):
                        ...
                        while 1: #cycle to accept a chunk
                            n=randint(0,maxLss[type]) #lenght of a chunk (element or loop)
                            if random()<hist[n]:
                                chunks.append(n)
                                norm+=n
                                sscontent+=n
                    ...
                    #derive strand from length difference distr. from consecutive strands
                        ...
                        hist2=histLength['SS']
                        while 1: #cycle to accept a chunk
                            n=randint(2,maxLss[type]) #no strand smaller then three residues
                            if hist2[n]>0:
                                d=prevLSS-n
                                if random()<hist[d]:
                                    chunks.append(n)
                ...
                if excess: #we failed trying to generate a sequence. Start again                 
                    ntries+=1
                    continue
                if float(sscontent)/Ntarget<0.28: #require more than %28 content
                    continue
                #print Nseqs-iseq,'type',type,' sscontent=',sscontent,'  ',float(sscontent)/Ntarget
