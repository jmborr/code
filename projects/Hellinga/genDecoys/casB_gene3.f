...
*****************************************************************************
*     To compile, use:
*     pgf77 -Mextend -O -s -fast -Wl,-static -o casB_gene2 casB_gene2.f *
...
* THE OTHER
* THIS CODE IS MODIFIED SO THAT WE PASS ALL CA'S IN CHAIN.DAT, AND SIMULATION
* WILL BUILD AN INITIAL CONFIGURATION IN WHICH HELIXES AND STRANDS, AS GIVEN
* BY SEQ.DAT FILE ARE SUBSTITUTED BY GENERIC ONES, AND LOOPS ARE KEPT. HELIXES
* AND STRANDS WILL BE SIMULATED OFF-LATTICE. WE IMPOSE AN UMBRELLA SAMPLING IN
* RMSD, SO THAT RMSD OF MODELS CANNOT EXCEED SOME RMSD CUT-OFF AT A PARTICULAR
* TIME, AND THE CUT-OFF BEGINS AT THREE ANGSTROMS AND FINISH AT 8ANGSTROMS
*****************************************************************************
...
ccccc variables needed for the rmsd umbrella sampling
      common/umbrella/armsdco,drmsdco,aminrmsdco,amaxrmsdco,anatx(ndim)
     $     ,anaty(ndim),anatz(ndim)
...
c     ensure we call secondary subroutine
      ssp=1
c     ensure we do switch=5 to include rmsd penalization
      switch=5
...
ccccccccccccccc trajectory files cccccccccccccccccccccccccccccccccccccc
      ...
      open(unit=31,file='rep1'//tra_suffix,status='unknown') !dump all into one file

c      do i=1,n_repf
c         if(i.lt.10)then
c            fn=char(48+i)
c            open(unit=30+i,file='rep'//fn//tra_suffix,status='unknown')
c         else
c            fn=char(48+(i-10))
c            open(unit=30+i,file='rep1'//fn//'.tra',status='unknown')
c         endif
c      enddo
...
c     set paramters from rmsd cut off
      aminrmsdco=3.0/0.87
      amaxrmsdco=8.0/0.87      
      armsdco=aminrmsdco
      drmsdco=(amaxrmsdco-aminrmsdco)/ncycle
...
c     fill GAP: cx0(i) -> cx(i)
********************************************************
c     we pass all Lch residues in chain.dat, thus no need to generate a random walk
 70   do i=1,Lch
         cx(i)=cx0(i)           !initialize cx(i)
         cy(i)=cy0(i)
         cz(i)=cz0(i)
      enddo
c      n_walk=0                  !for number of reject by excluded volumn
c 70   n_walk=n_walk+1
c      if(n_walk.gt.1000)then
c         exc_eff=exc_eff*0.99
c      endif
c      if(n_walk.gt.10000)then
c         write(20,*)'unsolvable structure',pass,i,exc_eff
c         write(*,*)'unsolvable structure',pass,i,exc_eff
c         call exit(1)
c      endif
c      do i=1,Lch
c         if(q(i).eq.1)then
c            cx(i)=cx0(i) !initialize cx(i)
c            cy(i)=cy0(i)
c            cz(i)=cz0(i)
c         else
c            cx(i)=1000000.      !for checking excluded volumn
c            cy(i)=1000000.
c            cz(i)=1000000.
c         endif
c      enddo
...
c         i1=n_i(i)
c         i2=n_f(i)
c         call connect(i1,i2,pass) !fill missed cooridinates, no move others
c         if(pass.ge.3)goto 70             !re-walk
c      enddo
c      if(n_g.ge.1)then
c         i1=n_i(1)
c         i2=n_f(1)
c         call connect(i1,i2,pass)
c         if(pass.ge.3)goto 70   !re-walk
c      endif
*^^^^^^^^^^^^^^Fill gap done, cx(i) is continuous ^^^^^^^^^^^^^^^^^^^^^^^
...
ccccccccccccccccccccc snapshots of E(1), E(N_rep) ccccccccccccc
         if(icycle.eq.icycle/1*1)then
            i_tr=i_tr+1
            do k=1,n_repf
               call TMscore(i,atm,armsd)
               write(31,'"L=",i3," E=",f8.1," n=",i6," rep=",i2,
     &              " cycle=",i5," rmsd=",f5.2," TM=",f5.3')
     &              Lch,E_rep(k),i_tr,k,icycle,armsd,atm

               write(31,'i8,1x,f10.1,2i8')Lch,E_rep(k),i_tr,icycle
               do i=1,Lch
                  if(mv(i).gt.0)then
                     abx=xrep(i,k)*0.87
                     aby=yrep(i,k)*0.87
...
c     update rmsd cutoff
         armsdco=armsdco+drmsdco
 1111 continue
c--------------------------Main cycle ended here!!---------------
...
***   panelty for the deviation from template---------------->
      if(switch.eq.5)then
         ...
         armsd=0.87*dsqrt(rms/nn)    !RMSD is real, rms is double precision
         if(armsd.gt.armsdco)then
            ESHORT11=ESHORT11+ainfinite
         endif
...
c         if(ISTAT.eq.10)then    !total energy
c            do i=1,nfr
c               nn=0
c               do j=nfr_i(i),nfr_f(i)
c                  nn=nn+1
c                  r_1(1,nn)=ex0(j)
c                  r_1(2,nn)=ey0(j)
c                  r_1(3,nn)=ez0(j)
...
c                  r_2(3,nn)=ez(j)
c               enddo
c               call u3b(w,r_1,r_2,nn,0,rms,u,t,ier)
c               armsd=dsqrt(rms/nn) !RMSD is real, rms is double precision
c               ESHORT11=ESHORT11+armsd**4
c            enddo
c         else                   !fragment energy
c            if(i_E_defo.eq.1)then !for defo_M but not trot_M
c               if(iiii.eq.1)then
c                  i=i_chunk(1)
c               elseif(iiii.eq.Lch)then
c                  i=i_chunk(Lch)
c               else
c                  i=i_chunk(iiii+2) !iiii+2 is first residue of the fragment
c               endif
c               if(i.gt.0)then
c                  nn=0
c                  do j=nfr_i(i),nfr_f(i)
...
c     $     +er6*ESHORT10
c     $     +er7*ESHORT11
        ESHORT=es2*ESHORT2+es3*ESHORT5+es3a*ESHORT5a+es3b*ESHORT5b+
     $     es3c*ESHORT5c
...
c     normal TM-score:
      subroutine TMscore(repi,atm,armsd)
      implicit integer(i-z)
      PARAMETER(nmax=3000)
      parameter(ndim=1500)
      parameter(nrep=100)
      common/lengths/Lch,Lch1,Lch2
      common/stru/axt(nmax),ayt(nmax),azt(nmax)
      common/para/d,d0
      common/align/iA(nmax),iB(nmax)
      common/nscore/i_ali(nmax),n_cut
      dimension L_ini(100),iq(nmax)
      double precision ascore,ascore_max
      dimension ax(nmax),ay(nmax),az(nmax)
      common/sw2/xrep(ndim,nrep),yrep(ndim,nrep),zrep(ndim,nrep)
      common/sw4/exrep(ndim,nrep),eyrep(ndim,nrep),ezrep(ndim,nrep) !Ca
      common/umbrella/armsdco,drmsdco,aminrmsdco,amaxrmsdco,anatx(ndim)
     $     ,anaty(ndim),anatz(ndim)
...
ccccc store replica coordinates to local array and with Angstroms units,
ccccc since units of d0 parameter are Angstroms

      DO i=1,Lch
         if(mv(i).gt.0)then
            ax(i)=exrep(i,repi)*0.87
            ay(i)=eyrep(i,repi)*0.87
            az(i)=ezrep(i,repi)*0.87
         else
            ax(i)=xrep(i,repi)*0.87
            ay(i)=yrep(i,repi)*0.87
            az(i)=zrep(i,repi)*0.87
         endif

c         write(*,'3F8.3') ax(i),ay(i),az(i)
      enddo
c      write(*,*) '=================='
c      do i=1,Lch
c         write(*,'3F8.3') dx(i),dy(i),dz(i)
c      enddo

cccc calculate rmsd to native of structure
      do i=1,Lch
         r_1(1,i)=ax(i)
         r_1(2,i)=ay(i)
         r_1(3,i)=az(i)
         r_2(1,i)=anatx(i)*0.87
         r_2(2,i)=anaty(i)*0.87
         r_2(3,i)=anatz(i)*0.87
c         write(*,'6f8.3')r_1(1,i),r_1(2,i),r_1(3,i),r_2(1,i),
c     &                   r_2(2,i),r_2(3,i)
      enddo
      call u3b(w,r_1,r_2,Lch,1,rms,u,t,ier)
      armsd=sqrt(rms/Lch)
c      write(*,*) "armsd=",armsd," rms=",rms
...
c     u rotate r_1 to r_2
           call u3b(w,r_1,r_2,L_init,1,rms,u,t,ier)
c           write(*,*) 'L_init=',L_init,' rms=',rms

c     global superposition
           do j=1,Lch
              axt(j)=t(1)+u(1,1)*ax(j)+u(1,2)*ay(j)+u(1,3)*az(j)
              ayt(j)=t(2)+u(2,1)*ax(j)+u(2,2)*ay(j)+u(2,3)*az(j)
              azt(j)=t(3)+u(3,1)*ax(j)+u(3,2)*ay(j)+u(3,3)*az(j)
c              write(*,'3F8.3') axt(j),ayt(j),azt(j)
           enddo
           d=d0_search-1
c           write(*,*) 'd=',d
...
c     init, get scores, n_cut+i_ali(i) for iteration
           ...
           ascore_sum=0
           do i=1,Lch
              dis=sqrt( (axt(i)-dx(i))**2 + (ayt(i)-dy(i))**2 +
     &                  (azt(i)-dz(i))**2 )
c              write(*,*) 'dis=',dis
              if(dis.lt.d)then
                 n_cut=n_cut+1
              ...
              ascore_sum=ascore_sum+1/(1+(dis/d0)**2)
           enddo
           ascore=ascore_sum/float(Lch)           
           if(ascore_max.lt.ascore)then !update best alignment
              ascore_max=ascore
           endif
c     iteration for extending
           ...
           do 301 it=1,n_it
              ...
              enddo
c     u rotate r_1 to r_2
              call u3b(w,r_1,r_2,n_cut,1,rms,u,t,ier)
c              write(*,*) 'n_cut=',n_cut,' rms=',rms
              do j=1,Lch
                 axt(j)=t(1)+u(1,1)*ax(j)+u(1,2)*ay(j)+u(1,3)*az(j)
                 ayt(j)=t(2)+u(2,1)*ax(j)+u(2,2)*ay(j)+u(2,3)*az(j)
                 azt(j)=t(3)+u(3,1)*ax(j)+u(3,2)*ay(j)+u(3,3)*az(j)
...
c     init, get scores, n_cut+i_ali(i) for iteration
     ...
     &                     (az(i)-dz(i))**2 )
              ...
              enddo
              ascore=ascore_sum/float(Lch)
c              write(*,*)'ascore=',ascore,' rms=',rms,' it=',it,' iL=',iL,
c     &                  '  L_init=', L_init
              if(ascore_max.lt.ascore)then !update best alignment
                 ascore_max=ascore
              endif
...
c     for initial length, L_ali/M
 333  continue                  
      ...
      atm=ascore_max
c      write(*,*) 'repi=',repi,' atm=',atm,' ascore_max=',ascore_max
      return
      END
ccccccccccc
