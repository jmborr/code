...
c     pgf77 -Mextend -O -s -fast -Wl,-static -o casGenDecoys casGenDecoys.f
...
c     number of vectors on lattice
      ...
      common/para/d,d0
      common/par/apar(ndim,ndim),nopardat
      common/seed1/no
...
c     SG-pair potential
      open(unit=15, file='par.dat', iostat=nopardat,  status='old')
...
c     TM-score cut-off
      atmco=0.35
c     increase in atmcod every cycle up to 0.7
      atmcod=(0.70-0.35)/ncycle
