...
*****************************************************************************
*     To compile, use:
*     pgf77 -Mextend -O -s -fast -Wl,-static -o casB_gene4 casB_gene4.f *
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      parameter(ndim=1000)
      parameter(nvec=312)
      parameter(longrange=15) !long range hydrogen bond interaction
      common/seqe/seq(ndim),sec(ndim)
      common/lengths/Lch,Lch1,Lch2
      common/hb/hbx(nvec,nvec),hby(nvec,nvec),hbz(nvec,nvec)
      common/bisec/cax(nvec,nvec),cay(nvec,nvec),caz(nvec,nvec)
...
c     increase strength of hydrogen bond for long range HB's between pre-assigned strands
                                 if(abs(i-k).gt.longrange)then
                                    EHB5b=EHB5b+arfavor*energyHBb(
     $                                   i,k,fact,axki,ayki,azki,bxi,
     $                                   byi,bzi,bxk,byk,bzk)
                                 else
