...
c
c     	   !! MODIFIED TO PRINT 100 clusters !!
c       	shashi       May 22, 2006
...
c      parameter(nst=100)      !number of used structure, maximum allowed
      ...
      character filen(ntr)*72,c2*7,seq(ndim)*3,protein*10
...
******** following parameter has been optimized ********************
      ...
      ratio1=0.7
      ratio2=0.15
      nc_max=100 !number of clusters to output
********************************************************************
...
cccccccccccccc output the 5 clusters in PDB format ccccccccccccccc
      ...
      do i=1,nc
         ...
         elseif (i.lt.100)then
          ...
          c2=char(48+i/100)//char(48+(i/10)-10)//char(48+i-10*(i/10))
     $    //'.pdb'
         endif
...
cccccccccccccccccc output cluster analysis cccccccccccccccccccc
     ...
     $        rmsd_close_min(i),E_combo(i),E(k),istr(k),filen(itra(k))
      enddo
 123  format(i3,i6,f6.2,f7.0,f6.2,2f9.1,i6,1x,a13)
...
cccccccccccccccccc output cluster analysis cccccccccccccccccccc
     ...
     $        n_str_cl_ex(i),R_ex(i),Rc_ex(i)
      enddo
 133  format(i3,i6,f7.2,f7.2,i8,f7.2,f7.2)
