*****************************************************************************
*     To compile, use:
*     pgf77 -Mextend -O -s -fast -Wl,-static -o casB_gene4.1 casB_gene4.1.f *
*     pgf77 -g -Mbounds -Mchkfpstk -Mextend  -Wl,-static -o casB_gene4.1 casB_gene4.1.f
*     To debug, type in fec02: pgdbg casB_gene4.1
*     This program is to generate protein structures by Monte Carlo         *
*     simulations under an on-and-off-lattice CAS model.                    *
*     Please address comments/bug-reports to: zhang6@buffalo.edu            *
*
* THIS CODE IS MODIFIED TO DO SIMULATIONS FOR POLY-ALA ONLY. THE ENERGY TERMS
* ARE MODIFIED.
* HYDROGEN BOND ENERGY FOR BETA IS STRONGER IF LONG RANGE (MORE THAN 15 AA APART)
* HYDROGEN BONDS only between alpha-alpha, alpha-coil, and beta-beta. No bond for
* loop-beta, and loop-loop
* 3-BODY term to strengthen the beta sheet conformation
* THIS CODE IS MODIFIED IN THE QUARSI3 POTENTIAL SUCH THAT TWO AMINO ACIDS
* HAVE NO INTERACTIONS IF BOTH ARE IN COILS, 1/4 IF ONE IS IN COIL AND NOT 
* THE OTHER
*****************************************************************************
...
c 3456789012345678901234567890123456789012345678901234567890123456789012345678
      ...
      parameter(nrep=100)       !maximum number of repl2icas
      parameter(nvec=312)       !number of vectors on lattice
      common/logica/goodc
      ...
      common/nana1/nana
      common/arfv/arfavor
ccccc variables needed for runID
      character arg_runID*8,arg_seed*16
      character crunID*2
      common/args/nargs,arg_runID,runID,crunID,arg_seed
ccccc variables needed for hydrogen bond network
      common/hbnet/mhbond,khbond,mnewhbond,knewhbond,mnewhbi,knewhbi,
     $     moldhbi,koldhbi
      integer mhbond(ndim)  !mhbond(i) number of HB bonds in which "i" participates
      integer khbond(2,ndim)!khbond(j,i) is one of the HB partners of "i"
      integer mnewhbond(ndim) !mnewhbform(i) number of new HB to form  upon chunk movement
      integer knewhbond(2,ndim) !knewhbform(j,i) is one of the new HB partners of "i"
      integer moldhbi      !old number of res outside fragment that are HB to res in fragment
      integer koldhbi(ndim)!old list of res outside fragment HB to res in fragment
      integer mnewhbi      !new number of res outside fragment that are HB to res in fragment
      integer knewhbi(ndim)   !new list of res outside fragment HB to res in fragment
ccccc read arguments cccccc
      nargs=iargc()
      if(nargs.eq.0)then
         runID=-1
      else
         call getarg(1,arg_runID)
         if(arg_runID(1:2).eq.'-h' .or.
     $        arg_runID(1:6).eq.'--help') call message()
c     we assume the argument is "runID=3" or "rundID=03"
         read(arg_runID(7:8),'i') runID
      endif

ccccc massage arg_runID to proper format
      if(runID.ge.0)then
         if(runID.le.9)then
            crunID(1:1)='0'
            crunID(2:2)=char(48+runID)
         else
            read(arg_runID(7:8),'a')crunID
         endif
      endif
...
c      open(unit=15,file='par.dat',   status='old') !SG-pair potential
c      open(unit=16,file='comb.dat',   status='old') !SG-contact restraints
c      open(unit=17,file='dist.dat',    status='old') !CA-distant restraints
c      open(unit=18,file='combCA.dat',status='old') !CA-contact restraints
      open(unit=21,file='rmsinp',status='old') !chain length for Lch
c      open(unit=22,file='distL.dat',status='old') !long range CA dist restraint
c      open(unit=25,file='pair3.dat',status='unknown') !orientation specific
c      open(unit=26,file='pair1.dat',status='unknown') !no orientation
c      open(unit=27,file='exp.dat',status='unknown')
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      read(19,*) eh6                                       !cooperativity for HB
...
c      call read_par             !read 'par.dat', pair-wise potential
c      call read_concut          !read cut-off for contact prediction
c      call read_contactrestrain !read contact restrains from 'comb.dat'
c      call read_distantrestrain !read distant restrains from 'dist.dat'
c      call read_longdistantrestrain !read long distant restrain from 'distL.dat'
c      call read_exp             !read slovent expose prediction
c      call read_CAcontact       !read CAcontact restrains from 'combCA.dat'
      ...
      n_repf=16                 !number of output replicas

      if(switch.gt.1)then
         n_repf=14
         call template_initial2 !initial model from templates
      endif
...
ccccccccccccccc trajectory files cccccccccccccccccccccccccccccccccccccc
      ...
      do i=1,n_repf
         if(i.lt.10)then
            fn=char(48+i)
            open(unit=30+i,file='rep'//fn//tra_suffix,status='unknown')
         else
            fn=char(48+(i-10))
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      if(switch.gt.1)then
         ...
         write(*,*) 'template simulation finished successfully'
         call exit(0)
      endif

      call read_initial2        !read initial (x,y,z) from 'chain.dat', or create random conf.
...
ccccccccccccccccccccccc <E>, NNa/NNt ccccccccccccccccccccccccccc
 ...
 5006 format(I4,f7.2,f8.1,f15.3,2f12.1,f11.6,6f8.1)
      ...
      write(*,*) 'on lattice simulation finished successfully'
      call exit(0)
      END
ccccccc=======================================================cccccccccc
...
ccccc subroutine message presents a wellcome message and then exits
      subroutine message()
      n=system('clear')
      write(*,*)'cas_abinitio runID=??'
      write(*,*)' value of runID is an integer (enter -1 for none)'
      call exit(1)
      end
...
cccccccccccccc set the random generator cccccccccccccccccccccccc
      if(runID.ge.0)then
c     different seed for different independent run
         no=random+runID
      else
         no=random
      endif      
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      do i=1,Lch
         ...
         enddo
      enddo
      mk_pair3=1                !use 'pair3.dat' and 'pair1.dat'
c      mk_pair3=-1               !do not use 'pair3.dat' and 'pair1.dat'
...
c      rewind(25)
c      i_pair3=-11
c      Nline=100                 !maximum Lch is 2500 !!!
c      do i=1,Lch
c         read(25,*)
c         do i_line=1,Nline
c            line_end=min(25*i_line,Lch) !25,50,75,100,...., ending point
c            read(25,*)(apba(i,j),j=(i_line-1)*25+1,line_end)
c            if(line_end.eq.Lch) go to 4074
c         enddo
c 4074    continue
cc         read(25,*)
c      enddo
...
c         do i_line=1,Nline
c            line_end=min(25*i_line,Lch) !25,50,75,100,...., ending point
cc            read(25,*)(apbm(i,j),j=(i_line-1)*25+1,line_end)
c            if(line_end.eq.Lch) go to 4075
c         enddo
c 4075    continue
c         read(25,*)
c      enddo
...
c         do i_line=1,Nline
c            line_end=min(25*i_line,Lch) !25,50,75,100,...., ending point
c            read(25,*)(apbp(i,j),j=(i_line-1)*25+1,line_end)
c            if(line_end.eq.Lch) go to 4076
c         enddo
c 4076    continue
c         read(25,*)
c      enddo
...
c            if(i_pair3.gt.0.and.mk_pair3.gt.0)then
c               apa(i,j)=fract_pair3*apba(i,j)+(1-fract_pair3)*apabla(ii,jj)-dd
cc               apm(i,j)=fract_pair3*apbm(i,j)+(1-fract_pair3)*apablm(ii,jj)-dd
c               app(i,j)=fract_pair3*apbp(i,j)+(1-fract_pair3)*apablp(ii,jj)-dd
c            endif
         enddo
      enddo

      write(*,*)i_pair3,mk_pair3,chuan
c      do i=1,Lch
c         do j=1,Lch
c            write(*,*)i,j,apa(i,j),apba(i,j),apabla(seq(i),seq(j))
c         enddo
c      enddo
...
***   combine and rescale the pair interaction ---------------->
      ...
      do i=1,Lch
         do j=1,Lch
            apar(i,j)=0         !decide not to use 'pair3.1.dat'
            if(mk_pair3.gt.0)then !decide to use 'pair3.1.dat'
               if(i_pair3.gt.0)then !'pair3.dat' exist
                  apar(i,j)=apar1(i,j)*apar2_max/apar1_max*
     &                 (1-fract_pair1)+apar2(i,j)*fract_pair1
                  if(chuan.lt.0.00001)then !not par but use pair1
                     eh1c=1     !redefine eh1c
                     apar(i,j)=apar2(i,j) !'pair1.dat' only
                  endif
               else
                  apar(i,j)=apar1(i,j) !'par.dat' only
               endif
            endif
         enddo
      enddo
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      dimension r1(50000),r2(50000),dis(50000),deviation(50000)
...
ccccccccccccccset reset temperature ccccccccccccccccccc
      ...
      implicit integer(i-z)
      parameter(ndim=1000) !number of residues
      parameter(nrep=100) !number of replicas
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      do i=1,3
         ...
         enddo
c         write(6,'ai4ai4')'ica(0)=',ica(0),' ica(2)=',ica(2)
         ica(0)=ica(2)
         ica(Lch)=ica(Lch-2)
         do j=1,Lch
...
ccc <Np2(i,j)>=27.
      ...
      write(*,*)'maximum of Np2(i,j)=',max,maxa
...
*****************************************************************
      ...
      dimension x0(1000),y0(1000),z0(1000)
...
*****************************************************************
*     read initial (x,y,z) from chain.dat
*****************************************************************
 ...
 104  rewind(24)
      read(24,*)n_thr           !Number of real templates in 'init.dat'
      if(n_thr.eq.0)then
         call random_initial    !produce initial structure randomly.
         return
      endif
      do 102 i_init=1,n_thr
...
****************************************************
c     read q(i), cx0(i) from 'chain.dat'
****************************************************
      rewind(24)
      read(24,*)n_thr           !Number of real templates in 'init.dat'
      if(n_thr.eq.0) then       !no templates, thus do simulation on-lattice
         switch=1               !marks simulation as on-lattice
         return
      endif
      if(i_thr0.gt.n_thr)then
         write(20,*)'without threading structure at this i_thr0!'
...
**********************************************************
      if(ssp.eq.1)then
         call secondary         !redefine q(i) and cx0(i)
      endif
...
********************************************************
      ...
      do i=1,M_a
         L_a=M_f(i)-M_i(i)+1
         if(L_a.le.L_cut)then !remove if aligned segments less than L_cut residues
            do j=M_i(i),M_f(i)
               q(j)=0
            enddo
         endif
...
c      do i=1,Lch !print q(i)
c         write(*,'ai3ai1a')'q(',i,')=',q(i)
c      enddo
...
c      do i=1,Lch
c         write(*,'ai3ai2ai3ai3')'q(',i,')=',q(i),' mv(',i,')=',mv(i)
c      enddo
c      write(*,'a,i2')'nfr=',nfr
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      implicit integer(i-z)
      athita=acos(1.-2.*aranzy()) !thita angle in random, [0,pi]
      aphi=2.*3.1415926*aranzy() !phi angle in random, [0,2pi]
...
ccc   
            do 3333 iphot=1,phot !N_swap, iterate at fixed temperature
               do 4444 i_nfl=1,nfl
                  fff=aranzy()
                  write(6,'ai3ai3ai3ai3')'#######\nicycle=',icycle,
     $                 ' itemp=',itemp,' iphot=',iphot,' i_nfl=',i_nfl
                  if(fff.le.bh2)then
                     write(6,'a')'call move2'
                     call move2
                  elseif(fff.le.bh3s)then
                     if(nfl3.ge.1)then
                        write(6,'a')'call move3s'
                        call move3s
                     else
                        write(6,'a')'call move2'
                        call move2
...
c      write(6,'10(f5.3x)')bh2,bh3s,bh3d,bh4s,bh4d,bh5s,bh5d,bh6,
c     $     bhendn,bhendc
...
c      call get_axis
      call inithbnet !initialize HB net to zero
...
ccc   afs(): keep in store.
      ...
      call updhbnet(1,Lch) !make hydrogen bond
      call checkhbnet
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      parameter(ndim=1000)
      parameter(nvec=312)
      parameter(longrange=20) !long range hydrogen bond interaction
      common/seqe/seq(ndim),sec(ndim)
      common/lengths/Lch,Lch1,Lch2
      common/hb/hbx(nvec,nvec),hby(nvec,nvec),hbz(nvec,nvec)
      common/bisec/cax(nvec,nvec),cay(nvec,nvec),caz(nvec,nvec)
...
ccccc variables needed for hydrogen bond network
     ...
     $     moldhbi,koldhbi
      ...
      write(6,'ai2xi3xi3')'entering EHB istat=',istat,jjjj,kkkk
      EHB1=0                    !+1/r of Ca-SC
      EHB1a=0                   !+1/r for non-parallel contact of Ca-Ca
      EHB1b=0                   !excluded volume of SC-SC
      EHB1c=0                   !pair-wise potential of SC-SC

      EHB2=0                    !quarsi3 for SC-SC
      ...
      EHB6=0                    !cooperativity for hydrogen bond

      if(istat.gt.0)then        !Enew
         icnt=icnto
         sumct=sumcto
         mnewhbi=0 !initialize number of residues outside fragment that are HB to fragment
         do i=1,Lch
            mnewhbond(i)=0 !reset number of HB partners
         enddo
      else                     !Eold
         moldhbi=0 !initialize number of residues outside fragment that are HB to fragment
      endif
...
*****************************************************************
         do 1001 i=1,Lch
            ...
            if(Cr2.lt.120)then  !120-->9.53, there is pairwise interaction
               idist=iabs(i-k)
               iseq=seq(i)
               aratio=1.0 !ratio to multiply the quarsi3 potential ehb2
               if(ksec.eq.1) then !k in coil state
                  if(sec(i).eq.1) then !i in coil state
                     aratio=0.0        !coil-coil has no interaction
                  else                 !i not in coil state
                     aratio=0.25       !coil-no-coil has 0.25 interaction
                  endif
               else                    !k not in coil state
                  if(sec(i).eq.1) then !i in coil state
                     aratio=0.25
                  endif
               endif
               ...
               IF(cc.gt.0.5)THEN !c_i//c_k
                  ...
                  if(Gr2.lt.alp(iseq,kseq))then
                     ...
                     if(Gr2.gt.arlp(iseq,kseq))THEN
                        EHB2=EHB2+app(i,k)*aratio !quarsi3
                     else
...
************************************************************************
               ...
               newbond=.false.  !assume no (i,k) HB for now
c     alpha-helix
               if(idist.eq.3)then
...
***   ->
     ...
     $                    axki,ayki,azki,bxi,byi,bzi,bxk,byk,bzk)
               ...
               elseif(idist.gt.4.and.idist.lt.longrange)then !!!!!!!!!!!!!!!!!!!!!!!
***   antiparallel-sheet
                  if(sec(k).eq.4.and.sec(i).eq.4)then
...
c     increase strength of hydrogen bond for long range HB's between pre-assigned strands
                                 EHB5b=EHB5b+energyHBb(i,k,fact,
     $                                axki,ayki,azki,bxi,byi,bzi,bxk,
     $                                byk,bzk)
                                 newbond=.true.
                              endif
                           endif
               ...
               elseif(idist.ge.longrange)then
                  if(sec(k).eq.4.and.sec(i).eq.4)then !strict beta-beta prediction
                     if(Cr2.lt.Cr2b.and.cc.gt.bcut_cc)then
                        bb=bxi*bxk+byi*byk+bzi*bzk !cos(Hbi.Hbk) 
                        if(bb.lt.-bcut_bb)then !antiparallel
***   antiparallel-sheet:
                           av12=avv(aax(i-1),aay(i-1),aaz(i-1),axi,ayi,
     $                          azi,axk,ayk,azk,aax(k+1),aay(k+1),
     $                          aaz(k+1))
                           if(av12.lt.-bcut_vv)then
                              av21=avv(axi,ayi,azi,aax(i+1),aay(i+1),
     $                             aaz(i+1),aax(k-1),aay(k-1),aaz(k-1),
     $                             axk,ayk,azk)
                              if(av21.lt.-bcut_vv)then
***   ->
                                 fact=abs(bb)*cc !bb->1,cc->1
                                 EHB5b=EHB5b+arfavor*energyHBb(i,k,fact,
     $                                axki,ayki,azki,bxi,
     $                                byi,bzi,bxk,byk,bzk)
                                 newbond=.true.
                              endif
...
***   parallel-sheet:
                           av11=avv(aax(i-1),aay(i-1),aaz(i-1),axi,ayi,
     $                          azi,aax(k-1),aay(k-1),aaz(k-1),axk,ayk,
     $                          azk)
                           if(av11.gt.bcut_vv)then
                              av22=avv(axi,ayi,azi,aax(i+1),aay(i+1),
     $                             aaz(i+1),axk,ayk,azk,aax(k+1),
     $                             aay(k+1),aaz(k+1))
...
***   ->
     ...
     $                                byi,bzi,bxk,byk,bzk)
               ...
               if(newbond.eq..true.)then !include bond in the HB network
                  if(istat.gt.0)then !new conformation, new HB network
                     if(i.lt.jj.or.i.gt.kk)then !res outside the fragment that is HB to fragment
                        do l=1,mnewhbi !cycle over list of res outside fragment HB to fragment
                           if(i.eq.knewhbi(l))goto 1004 !res already in the list
                        enddo
                        mnewhbi=mnewhbi+1       !increase the list with newcomer
                        knewhbi(mnewhbi)=i
 1004                endif
                     mnewhbond(k)=mnewhbond(k)+1 !update new HB network
                     if(mnewhbond(k).gt.2) write(6,*)'ERROR k',k
                     knewhbond(mnewhbond(k),k)=i
                     mnewhbond(i)=mnewhbond(i)+1
                     if(mnewhbond(i).gt.2)then
                        write(6,*)'ERROR i',i
                     endif
                     knewhbond(mnewhbond(i),i)=k
                  else               !old conformation, old HB network
                     if(i.lt.jj.or.i.gt.kk)then !res outside the fragment that is HB to some res
                        do l=1,moldhbi
                           if(i.eq.koldhbi(l))goto 1005 !res already in the list
                        enddo
                        moldhbi=moldhbi+1       !whithin the fragment
                        koldhbi(moldhbi)=i
 1005                endif
                  endif
               endif
...
c^^^^^^^^^^^^^H-bond energy finished ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
 ...
 1002 continue                  !k -> [jj,kk]
      ...
      write(6,'a')'cooperativity term in HB network'
      if(istat.gt.0)then !new conformation
         do j=1,mnewhbi !cycle over res outside fragment that are HB to some res within fragment
            i=knewhbi(j)!one of these residues outside fragment
            do l=1,mhbond(i)
               ii=khbond(l,i)   !an old HB partner of i
               if(ii.lt.jj.or.ii.gt.kk)then !complete the HB network with i--i' bonds, where
                  mnewhbond(i)=mnewhbond(i)+1 !both i and i' are outside the fragment but
                  if(mnewhbond(i).gt.2)then
                     write(6,*)'ERROR i=',i
                  endif
                  knewhbond(mnewhbond(i),i)=ii !i is HB to some res within fragment
               endif
            enddo
            if(mnewhbond(i).eq.2) EHB6=EHB6-1 !cooperativity between the two partners of i
         enddo
         do k=jj,kk
            if(mnewhbond(k).eq.2) EHB6=EHB6-1 !cooperativity between the two partners of k
         enddo
      else !old conformation
         do j=1,moldhbi
            i=koldhbi(j)
            if(mhbond(i).eq.2) EHB6=EHB6-1
         enddo
         do k=jj,kk
            if(mhbond(k).eq.2) EHB6=EHB6-1
         enddo
      endif
...
c     $     +eh1c*EHB1c          !pair-wise potential of SC-SC
     ...
     $     +eh6*EHB6           !H-bond cooperativity

c      write(*,*)EHB,eh1c,EHB1c,jjjj,kkkk

c ^^^^^^^^^^ EHB finished ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
      write(6,'a')'leaving EHB'
      RETURN
      END !end of function ehb
...
***** Bury energy of SG --------------------->
c         gxn=HH(1,1)*(agxi-cex)+HH(1,2)*(agyi-cey)+HH(1,3)*(agzi-cez)
c         gyn=HH(2,1)*(agxi-cex)+HH(2,2)*(agyi-cey)+HH(2,3)*(agzi-cez)
c        gzn=HH(3,1)*(agxi-cex)+HH(3,2)*(agyi-cey)+HH(3,3)*(agzi-cez)
c         fff=gxn**2/EE(1)+gyn**2/EE(2)+gzn**2/EE(3) !=5 for ellipsoid surphase
c         if(fff.lt.2.5)then
c            aaa=fff-2.5
c            if(aaa.lt.-1)aaa=-1
c            ESHORT2=ESHORT2-aaa*area(i) !area(i)=-1, buried; +1, exposed
c         endif

          fff=agxi**2/cex+agyi**2/cey+agzi**2/cez-3.0
...
c     $     +er6*ESHORT10
c     $     +er7*ESHORT11
        ESHORT=es2*ESHORT2+es3*ESHORT5+es3a*ESHORT5a+es3b*ESHORT5b+es3c*ESHORT5c
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      dimension ox(ndim),oy(ndim),oz(ndim),oo(0:ndim)
...
c     choose the position of (m) ----------->
      ...
      m1=m+1
      m2=m+2
c      write(6,'ai3ai3a')'fragment=[',m,',',m2,']'
...
c^^^^^^^^^^ new conformation chosen finished ^^^^^^^^^^^^^^^^^^^^^^^^
      if(nn(m).eq.ica(m))then
c         write(6,'a')'same conformation, thus nothing to do'
         goto 113
      endif
...
c     calculate E_new--------------->
         do pp=1,Lch
            ...
            noa(pp)=noaa(pp)
         enddo
c         write(6,'a')'before calculating new conformation'
c         call checkhbnet
         Enew=EHB(m,m2,1)+ESHORT(m,m2,1) !icnt,nop are repeated.
c         write(6,'a')'after calculating new conformation'
c         call checkhbnet
         do kkk=m,m2
...
c     return back the conformation and calculate E_old --------->
         ...
         ica(m)=oo(m)
         ica(m1)=oo(m1)
c         write(6,'a')'before calculating old conformation'
c         call checkhbnet
         Eold=EHB(m,m2,-1)+ESHORT(m,m2,-1) !repeated part of icnt, nop removed
c         write(6,'a')'after calculating old conformation'
c         call checkhbnet
...
c     change the conformation to the new position--------->
            ...
            call updhbnet(m,m2) !update hydrogen bond network
c            call get_axis
         endif                  !for id
...
c            energ=energ+de	
            ...
            ica(m2)=nn(m2)
            call get_center       !update (amx,amy,amz)
            call updhbnet(m,m3) !update hydrogen bond network
c            call get_axis
         endif
      else                      !for look(i,m) loop
c     change back to old conformation ----------------------->
...
c            energ=energ+de	
            ...
            ica(m3)=nn(m3)
            call get_center       !update (amx,amy,amz)
            call updhbnet(m,m4) !update hydrogen bond network
c            call get_axis
         endif
      else                      !for look(i,m) loop
c     change back to old conformation ----------------------->
...
c            energ=energ+de	
            ...
            ica(m4)=nn(m4)
            call get_center       !update (amx,amy,amz)
            call updhbnet(m,m5) !update hydrogen bond network
c            call get_axis
         endif
      else                      !for look(i,m) loop
c     change back to old conformation ----------------------->
...
c            energ=energ+de	
            ...
            ica(mnob1)=nn(mnob1)
            call get_center     !update (amx,amy,amz)
            call updhbnet(m,mnob) !update hydrogen bond network
c            call get_axis
         endif
      else                      !for look(i,m) loop
c     change back to old conformation ----------------------->
...
c     energ=energ+de	
            ...
            enddo
            call get_center     !update (amx,amy,amz)
            call updhbnet(1,n) !update hydrogen bond network
c            call get_axis
         endif
      else                      !for look(i,m) loop
c     change back to old conformation ----------------------->
...
c     energ=energ+de	
            ...
            enddo
            call get_center     !update (amx,amy,amz)
            call updhbnet(m,n) !update hydrogen bond network
c            call get_axis
         endif
      else                      !for look(i,m) loop
c     change back to old conformation ----------------------->
...
c     energ=energ+de	
            ...
            enddo
            call get_center     !update (amx,amy,amz)
            call updhbnet(m,Lch) !update hydrogen bond network
c            call get_axis
         endif
      else                      !for look(i,m) loop
c     change back to old conformation ----------------------->
...
c     calculate eprofn while dord was calculated when call EHB(i,m,1)--->
         ...
         do pp=1,Lch
            ...
            ip=nop(pp)
            im=nom(pp)
c            write(6,'i3,xi2xi2xi2xi2x')pp,ia,im,ip,is
            eprofn=eprofn+envir(ia,im,ip,is,3)
         enddo
...
c            energ=energ+de
            ...
            didevsum=didev
            call get_center     !update (amx,amy,amz)
            call updhbnet(1,m+1) !update hydrogen bond network
c            call get_axis
         endif
      else                      !for look(i,m) loop
         do i=1,m
...
c     energ=energ+de
            ...
            didevsum=didev
            call get_center       !update (amx,amy,amz)
            call updhbnet(mm-1,Lch) !update hydrogen bond network
c            call get_axis
         endif
      else                      !for look(i,m) loop
         do i=mm,Lch
...
c            energ=energ+de	
            ...
            enddo
            call get_center     !update (amx,amy,amz)
            call updhbnet(m1,m2) !update hydrogen bond network
c            call get_axis
         endif
      else                      !for look(i,m) loop
c     change back to old conformation ----------------------->
...
c            energ=energ+de	
            ...
            enddo
            call get_center     !update (amx,amy,amz)
            call updhbnet(2,lenf1) !update hydrogen bond network
c            call get_axis
         endif
      else                      !for look(i,m) loop
c     change back to old conformation ----------------------->
...
c      energy_tot=energy_tot+en1*eprofo+en2*E_cord+en3*E_cnum
      energy_tot=energy_tot+en2*E_cord+en3*E_cnum
...
c     #############################################
      subroutine inithbnet()
c     initialize hydrogen bond network
      implicit none
c     global variables
      integer ndim
      parameter(ndim=1000)      !maximum length of chain-length
      ...
      common/hbnet/mhbond,khbond,mnewhbond,knewhbond,mnewhbi,knewhbi,
     $     moldhbi,koldhbi
      integer mhbond(ndim)  !mhbond(i) number of HB which "i" participates
      integer khbond(2,ndim)!khbond(j,i) is one of the HB partners of "i"
      integer mnewhbond(ndim)  !mnewhbform(i) number of new HB to form  upon chunk movement
      integer knewhbond(2,ndim)!knewhbform(j,i) is one of the new HB partners of "i"
      integer moldhbi      !old number of res outside fragment that are HB to res in fragment
...
c     local variables
      ...
      end !end of subroutine inithbnet
c     #############################################


c     #############################################
      subroutine updhbnet(jjjj,kkkk)
c     update hydrogen bond network
      implicit none
c     global variables
      integer ndim
      parameter(ndim=1000)      !maximum length of chain-length
...
c     input variables
      integer jjjj,kkkk
c     local variables
      integer jj,kk,i,j,k,l,ii,npart,kpart(ndim)
      logical breakbond

      write(6,'a')'entering updhbnet'      
c     coupling of secondary structure with pairwise interactions
c     included - thus expanded range
      jj=jjjj-1
      if(jj.lt.1)jj=1
...
c     first step, find residues in oldhbi list not present in newhbi
c     list. These "abandoned" residues used to have HB to the
c     fragment. Now we have to remove those bonds from the list of bonds
      do j=1,moldhbi
         i=koldhbi(j)
         do l=1,mnewhbi !check if residue also present in new list
            ii=knewhbi(l)
            if(i.eq.ii) goto 10 !residue present in new list. We'll apply second step
         enddo
         npart=0
         do l=1,mhbond(i) !search for res outside fragment in list of HB partners for i
            k=khbond(l,i)
            if(k.lt.jj.or.k.gt.kk)then !filter out partners within fragment
               npart=npart+1
               kpart(npart)=k
            endif
         enddo
         mhbond(i)=npart !overwrite list of HB partners with filtered list
         do l=1,npart
            khbond(l,i)=kpart(l)
         enddo
 10   enddo
c     second step, replace list of bonds for residues in the newhbi list
      npart=0
      do j=1,mnewhbi
         i=knewhbi(j)
         mhbond(i)=mnewhbond(i)
         do l=1,mhbond(i)
            khbond(l,i)=knewhbond(l,i)
         enddo
      enddo
c     third step, replace list of bonds for fragment residues
      do k=jj,kk !cycle through residues in fragment
         mhbond(k)=mnewhbond(k)
         do l=1,mhbond(k)
            khbond(l,k)=knewhbond(l,k)
         enddo         
      enddo
      end !end of subroutine updhbnet
c     #############################################

      
c     #############################################
      subroutine checkhbnet()
c     update hydrogen bond network
      implicit none
c     global variables
      integer ndim
     ...
     $     moldhbi,koldhbi
      ...
      integer knewhbi(ndim)   !new list of res outside fragment HB to res in fragment
c     local variables
      integer i,j,k,l,m,n,lnblnk,n3,inhb
      real nhb
      character *10000 buf
      
      do i=1,Lch
         do l=1,mhbond(i)
            k=khbond(l,i) !(i,k) bond in i-list
            do m=1,mhbond(k) !search for (k,i) bond in k-list
               n=khbond(m,k)
               if(n.eq.i) goto 10
            enddo
            write(6,'ai3ai3ai3a')'ERROR: (',k,',',i,') missing in ',
     $           k,'-list'
            write(6,'i3a')i,'-list'
            do m=1,mhbond(i)
               write(6,'ai3ai3a')'(',i,',',khbond(m,i),')'
            enddo
            write(6,'i3a')k,'-list'
            do m=1,mhbond(k)
               write(6,'ai3ai3a')'(',k,',',khbond(m,k),')'
            enddo
            stop
 10      enddo
      enddo

      write(6,'a')'old HB network'
      nhb=0.0
      do i=1,Lch
         nhb=nhb+1.0*mhbond(i)
         if(mhbond(i).gt.2)then
            write(*,'ai3xi3')'ERROR: more than 2 old partners for ',i,
     $           mhbond(i)
            stop
         endif
c         if(mhbond(i).gt.0) write(6,'i3xi1')buf(1:lnblnk(buf))//'',i,
c     $        mhbond(i)
      enddo
      write(6,'af5.1')'total number HB:',nhb/2.0
      inhb=nhb
      if(inhb/2.lt.nhb/2)then
         write(6,'ai3') 'non-integer number HB!'
         stop
      endif

      write(6,'a')'new HB network'
      nhb=0.0
      do i=1,Lch
         nhb=nhb+1.0*mnewhbond(i)
         if(mnewhbond(i).gt.2)then
            write(*,'ai3xi3')'ERROR: more than 2 new partners for ',i,
     $           mnewhbond(i)
            stop
         endif
      enddo
      end !end subroutine checkhbnet
c     #############################################

     
c     #############################################
      subroutine printsnapshot(irep)
      implicit none
c     global variables
      integer ndim
      parameter(ndim=1000)      !maximum length of chain-length
     ...
     $              '  CA  VAL   ',i,'    ',ax,ay,az
         ...
         do i=1,Lch
            if(mv(i).gt.0)then
               ...
               ax=exrep(i,irep)*0.87
               ay=eyrep(i,irep)*0.87
               az=ezrep(i,irep)*0.87
            endif
               write(6,'ai3ai3a,3(f8.3)')'ATOM    ',i,
     $              '  CA  VAL   ',i,'    ',ax,ay,az
         enddo
      ...
      write(6,'a')'TER'
      end !end of subroutine printsnapshot
c     #############################################
