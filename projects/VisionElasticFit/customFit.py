...
wsName='VIS_1085_event'  # workspace name of the bank or tube. It must be an EVENT workspace
centerPixel=567   # workspace index of the pixel with maximum intensity in a given tube
pixelRange=30   # do the fit for pixelRange pixels to the left and right of centerPixel, in addition to centerPixel
...
outFitFilePrefix='/tmp/pixel_XXX.dat' #contains the data and fitted spectrum for a pixel number XXX
############################

# Uncomment the following lines to run as standalone script
#import sys
...
def FindHighestPeak(sp,PeakFunction='Gaussian', tofRange=(2000,5000), dx=1.0):
	...
	(2500,5000) is the expected TOF range for the elastic line
...
def fitSpectrum(spectrum,fitMethod=None,isToTheRight=True):
	...
	if fitMethod == 'two BackToBackExponentials':
		...
		#clean up
		suffixes=('_cloned','_cloned_peaks','_cloned_NormalisedCovarianceMatrix','_cloned_Parameters','_cloned_Workspace','_NormalisedCovarianceMatrix','_removedElastic','_removedAdjacent')
		for suffix in suffixes: DeleteWorkspace(spectrum.getName()+suffix)
		#suffixes=('_Workspace','_Parameters')
		#for suffix in suffixes: DeleteWorkspace(spectrum.getName()+suffix)
...
for ix in range(centerPixel-pixelRange, centerPixel+pixelRange+1):
	...
	pFile.flush() #We can't wait :)
	
	outFile = outFitFilePrefix.replace('XXX','%03d'%ix)
	...
	wFile.close()
	#Clean up
	for ws in (sp,spRebinned): DeleteWorkspace(ws) #clean up
pFile.close()
