...
c     pgf77 -Mextend -O -s -fast -Wl,-static -o outputEnergies.x outputEnergies.f
...
c     safe when vr^2<30
      ...
      common/CA/dx(ndim),dy(ndim),dz(ndim),ndx(ndim),ndy(ndim),ndz(ndim)
      ...
      common/commonuse2/atemp1,atemp2,N_rep,phot,photi(100),i_trk(100)
...
c     chuan denotes the energy scale of threading related parameters
      common/weight/chuan
      ...
      common/ichos/ichos
      common/nana1/nana
      common/setnat/tonat(100),atmscores(100),armsds(100)
...
ccccc open trajectory files, initialize i_trk, tonat
      if(n_repf.gt.N_rep)n_repf=N_rep
      open(unit=31,file='rep1.tra',status='unknown')
      do i=1,n_repf
         tonat(i)=0
         i_trk(i)=0 !snapshot index per replica k
c         if(i.lt.10)then
c            fn=char(48+i)
c            open(unit=30+i,file='rep'//fn//'.tra',status='unknown')
c         else
c            fn=char(48+(i-10))
...
ccccc initialize photmin,photmax,photi
      photmax=1000
      photmin=1
      do i=1,N_rep
         photi(i)=phot
...
c      atmco=0.35
      atmco=0.85
c     increase in atmcod every cycle
c      atmcod=(0.90-atmco)/ncycle
      atmcod=(1.00-atmco)/ncycle
c     increase/decrease photi
      aphotd=0.9
      

ccccc The main cycle starts from here
...
ccc
            do 3333 iphot=1,photi(itemp) !N_swap, iterate at fixed temperature
               do 4444 i_lch=1,Lch
                  fff=aranzy()
                  if(fff.le.bh2)then
                     call move2
 ...
 3333       continue
ccc   
ccccccccccrecord energy and (x,y,z) for the replica ccccccccccccc
...
c            write(*,*) "atm=",atm," armsd=",armsd
            armsds(i)=armsd
            atmscores(i)=atm
c            write(*,*) 'photi(',i,')',photi(i),' TMscore(',i,')=',atmscores(i)
            if(atm.gt.atmco)then         
               tonat(i)=0
            else                !structure wandered away too much from native
               tonat(i)=1       !mark the replica to restart again from native
            endif
         enddo
...
ccccccccccccccccccccc snapshots of E(1), E(N_rep) ccccccccccccc
         if(icycle.eq.icycle/1*1)then
            do k=1,n_repf
               if(tonat(k).eq.0)then !only write if below tmco
                  i_tr=i_tr+1
                  i_trk(k)=i_trk(k)+1 !increase snaphot index
                  write(31,'"L=",i3," E=",f8.1," n=",i6," rep=",i2,
     &                  " cycle=",i5," rmsd=",f5.2," TM=",f5.3,
     &                  " phot=",i4')Lch,E_rep(k),i_tr,k,icycle,
     &                  armsds(k),atmscores(k),photi(k)
                  do i=1,Lch
                     abx=xrep(i,k)*0.87
                     aby=yrep(i,k)*0.87
                     abz=zrep(i,k)*0.87
                     write(31,'f10.3,1x,f10.3,1x,f10.3')abx,aby,abz
                  enddo
               endif
            enddo
         endif

cccc update photi (after printing old photi's to rep1.tra)
         do i=1,N_rep
            if(atmscores(i).gt.atmco)then 
               photi(i)=photi(i)/aphotd !lengthen time
            else
               photi(i)=photi(i)*aphotd !shorten time
            endif
            if(photi(i).gt.photmax)then
               photi(i)=photmax
            else if(photi(i).lt.photmin)then
               photi(i)=photmin
            endif
         enddo
...
c     swap two replicas only if not marked to restart as native
         if(icycle.eq.icycle/2*2)then
            do i=1,N_rep-1,2    !swap odd replicas 
               if(tonat(i).eq.0 .and. tonat(i+1).eq.0)then
                  call swap(i,i+1)
               endif
            enddo
         else
...
ccccc restart to native those marked replicas
      do i=1,N_rep
         if(tonat(i).eq.1)then
...
ccccc store projection of native coordinates
         do i=1,Lch
            ndx(i)=x(i)
            ndy(i)=y(i)
            ndz(i)=z(i)
...
c         write(*,*)'native file after projection...'
c         do i=1,Lch
c            write(*,'i4,1x,3f8.3')i,dx(i),dy(i),dz(i)
c         enddo
c         stop
...
ccccc put the itemp'th replica conformation as current conformation
      if(tonat(itemp).eq.0)then
         do i=1,Lch
            x(i)=xrep(i,itemp)
            y(i)=yrep(i,itemp)
            z(i)=zrep(i,itemp)
         enddo
      else !reset replica to the native state structure
         do i=1,Lch
            x(i)=ndx(i)
            y(i)=ndy(i)
            z(i)=ndz(i)
...
c     energy penalization due to deviation from expected distribution of contact orientation
      eprofo=0.0
      DO k=1,Lch
         is=seq(k)
         ia=noa(k)
...
c     energy penalization due to deviation from expected contact order
      E_cord=abs(ICNT/(0.00001+float(SUMCT))-acorder)
c     energy penalization due to deviation from expected number of contacts
