...
*     pgf77 -g -Mbounds -Mchkfpstk -Mextend  -Wl,-static -o cas cas.f       *
...
c     output first replica in PDB format to STDOUT
               do i=1,Lch
                  if(mv(i).gt.0)then
                     write(6,'ai3ai3a,3(f8.3)')'ATOM    ',i,
     $                    '  CA  VAL   ',i,'    ',xrep(i,1)*0.87,
     $                    yrep(i,1)*0.87,zrep(i,1)*0.87
                  else
                     write(6,'ai3ai3a,3(f8.3)')'ATOM    ',i,
     $                    '  CA  VAL   ',i,'    ',exrep(i,1)*0.87,
     $                    eyrep(i,1)*0.87,ezrep(i,1)*0.87
                     endif
               enddo
               write(6,'a')'TER'
