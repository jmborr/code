...
ccccccccccccccc sequence specified input files cccccccccccccccc///////////
      ...
      open(unit=24,file='chain.dat',status='old') !template coordinate files
...
ccccccccccrecord energy and (x,y,z) cccccccccccccccccccc
            ...
            do i=1,Lch
               ...
               yrep(i,itemp)=y(i)
               zrep(i,itemp)=z(i)
               egxrep(i,itemp)=x(i)+gx(ica(i-1),ica(i),seq(i)) !SideGroups
               egyrep(i,itemp)=y(i)+gy(ica(i-1),ica(i),seq(i))
               egzrep(i,itemp)=z(i)+gz(ica(i-1),ica(i),seq(i))
            enddo
 2222    continue
         ...
         if(icycle.eq.icycle/1*1)then
            ...
            do k=1,n_repf
               ...
               do i=1,Lch
                  ...
                  aby=yrep(i,k)*0.87
                  abz=zrep(i,k)*0.87
                  write(30+k,'axf10.3xf10.3xf10.3')'CA',abx,aby,abz
                  if(seq(i).ne.0)then !not GLY
                     abx=egxrep(i,k)*0.87
                     aby=egyrep(i,k)*0.87
                     abz=egzrep(i,k)*0.87
                     write(30+k,'axf10.3xf10.3xf10.3')'SG',abx,aby,abz
                  endif
               enddo
            enddo
         endif
...
ccccccccccrecord energy and (x,y,z) cccccccccccccccccccccccc
            ...
            do i=1,Lch
               ...
               if(mv(i).gt.0)then
                  ...
                  yrep(i,itemp)=y(i)
                  zrep(i,itemp)=z(i)
                  egxrep(i,itemp)=x(i)+gx(ica(i-1),ica(i),seq(i)) !SG
                  egyrep(i,itemp)=y(i)+gy(ica(i-1),ica(i),seq(i))
                  egzrep(i,itemp)=z(i)+gz(ica(i-1),ica(i),seq(i))
               else
...
c     reverse direction as v(i---k), if (k<i);
      ...
      if(k.gt.i)then            !bki,bxk,axki are in same directory
         ...
         if(br.lt.acut_hh)then
            b=1/(1+abs(br-3.2)) !br->3.4 (1.56A**2)
