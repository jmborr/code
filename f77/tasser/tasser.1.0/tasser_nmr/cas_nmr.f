...
c  pgf77 -Mextend -O -s -fast -Wl,-static -o cas_nmr cas_nmr.f
...
c aco: summ of sequence separation for all noe restraints; arco: summ of 
c sequence separation for all noe restraints present at current time.
c isrco is a flag that signals ignoring predicted restraints
      common/noe5/aco,arcomax,arco,arcos(nrep),isrco
      common/noe6/ESHORTNOE,nrecrco,mrecrco,avrco(nrep)
c-noe-----------------------------------------------------------
...
c     initialize some noe-restraints related parameters
      call set_NMR_parameters
...
c            write(*,'a4i2xa2f7.1xa5f4.2')'rep=',itemp,'E=',E_rep(itemp),
c     $           'arco=',arco
...
ccccccccccccccccccccc snapshots of E(1), E(N_rep) ccccccccccccc
     ...
     &                 Lch,E_rep(k),i_tr,icycle,arcos(k),armsCA
               else
...
ccccccccccccc check if NRM restraints stablished ccccccccccccccccccc
         if(nrecrco.gt.0)then
            call check_NMR_restraints(icycle)
         endif
...
ccccccccccccc check if NRM restraints stablished ccccccccccccccccccc
      subroutine check_NMR_restraints(icycle)
      implicit integer(i-z)
      parameter(nrep=100)       !number of replicas
      common/noe5/aco,arcomax,arco,arcos(nrep),isrco
      common/noe6/ESHORTNOE,nrecrco,mrecrco,avrco(nrep)
...
c      write(6,*)'This is subroutine check_NMR_restraints'
c store arcos(i) for nrecrco times after 0.1*nycle cycles
      if(nrecrco.gt.0.and.icycle.gt.0.1*ncycle)then
         do i=1,n_repf
            avrco(i)=avrco(i)+arcos(i)
         enddo
         nrecrco=nrecrco-1 !will store only nrecrco times
      endif
c      write(*,*)'nrecrco=',nrecrco
      if(nrecrco.eq.0)then         
         do i=1,n_repf !find the average rco for each replica
            avrco(i)=avrco(i)/mrecrco
            write(*,*)'avrco(i)=',avrco(i)
            if(avrco(i).ge.0.8)then
c     nrecrco is now number of replicas with rco bigger than 0.8
               nrecrco=nrecrco+1
            endif
         enddo
c check if at least 40% replicas have most NMR restraints present.
c If so, switch on the predicted restraints (isrco=1) because
c the templates seem to agree with the NMR restraints and the
c predicted restraints are derived from the templates
c In not, them trash the coordinates and generate randomly. Do not
c switch on the predicted restraints.
         if(nrecrco.ge.0.4*n_repf)then
            isrco=1
            write(*,*) 'isrco=1'
         else
            write(*,*)'calling random initial'
            call random_initial
         endif
c     signal we calculated already avrco
         nrecrco=0
      endif
      return
      end
...
c-noe--scnoe----------------------------------------------------
      ...
      do i=1,nnoe
         ...
         aco=aco+sep
      enddo
      arcomax=0.8               !arco cutoff
c     tune arcomax when we have few restraints
      if((float(aco-maxsep-minsep)/aco).lt.arcomax)then
         arcomax=float(aco-maxsep-minsep)/aco
      endif
      write(20,'a8f4.2')'arcomax=',arcomax
c      write(*,*)'nnoe=',nnoe,'maxsep=',maxsep,'aco=',aco,'arcomax=',arcomax
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
 ...
 99         ip(i)=int(aranzy()*nvec)+1
            ...
            if(m.gt.1000000)then
               write(*,*) 'UNSOLVABLE STERIC PROBLEM > restart all over'
          ...
          if(N_noe.ge.1)then
             ...
             do k=1,N_noe
                ...
                if(j.lt.i.OR.j.gt.jjjj)then
                   ...
                   if(aij2.le.concut2(iseq,jseq))then
                      ESHORTNOE=ESHORTNOE-anneal*(1+(2.0*abs(i-j))/Lch)
                      if(istat.lt.0)then
                         coold=coold+(sqrt(aij2)-concut(iseq,jseq))*er4noe !penality
                      else
                         conew=conew+(sqrt(aij2)-concut(iseq,jseq))*er4noe !panelity
                      endif
                   endif
                endif
             enddo
             ...
             do k=1,N_noe
                ...
                if(j.lt.i.OR.j.gt.jjjj)then
                   ...
                   if(aij2.gt.49.0)then
                      axnoe=anneal*(1+(1.0*abs(i-j))/Lch)
...
c      write(*,*)er6,eshort10,er5,eshort9,istat
     ...
     $     +es6*ESHORT8
     $     +er7*ESHORT11
      if(isrco.gt.0)then !add predicted restraints
         ESHORT=ESHORT
     $        +er1*ESHORT3
     $        +er3*ESHORT4
     $        +er4*ESHORT4a
