...
c  pgf77 -Mextend -O -s -fast -Wl,-static -o cas_nmr_rdc cas_nmr_rdc.f
...
c 3456789012345678901234567890123456789012345678901234567890123456789012345678
      ...
      common/CA/dx(ndim),dy(ndim),dz(ndim),isca,armsCA
...
c-noe-----------------------------------------------------------
      common/noe/er4noe,anneal
      common/noe1/nnoe,Mnoe(ndim),Knoe(ndim,100),er3noe
      common/noe2/nnoemc,Mnoemc(ndim),Knoemc(ndim,100),er3noemc
      common/noe3/nnoesm,Mnoesm(ndim),Knoesm(ndim,100),er3noesm
      common/noe4/Mnoems(ndim),Knoems(ndim,100)
      common/noe5/aco,arco
c-noe-----------------------------------------------------------

c-rdc-----------------------------------------------------------
      common/rdc/nrdc,exrdc,eirdc,isrdc(ndim,ndim),aminpr(maxrdc),
     &           amaxpr(maxrdc)
c-rdc-----------------------------------------------------------
...
c-noe-----------------------------------------------------------
      open(unit=50,file='noe.dat',status='old') !sidechain noe contact
      open(unit=49,file='mcnoe.dat',status='old') !mainchain noe contact
      open(unit=48,file='smnoe.dat',status='old') !sidechain-mainchain noe
c-noe-----------------------------------------------------------

c-rdc------------------------------------------------------------
      open(unit=60,file='rdc.dat',status='old')
c-rdc------------------------------------------------------------

      open(unit=19, file='in.dd',     status='old')
      open(unit=20, file='out.d',     status='unknown')
      open(unit=28, file='CA', status='old', iostat=isca)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      read(19,*) nana
c-noe------------------------------------------------------------
      read(19,*) er3noe,er3noemc,er3noesm !for noe restrains (4)
      er4noe = 2.0
      write(20,*) 'er3noe(3) & er4noe',er3noe,er3noemc,er3noesm,er4noe 
c-noe------------------------------------------------------------

c-rdc------------------------------------------------------------
      read(19,*) exrdc,eirdc !energy of the residual dipolar coupling contact
c-rdc------------------------------------------------------------
...
c     call random_initial       !produce initial structure randomly.
      call read_initial2        !read initial (x,y,z) from 'chain.dat'
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      do 1111 icycle=1,ncycle
         anneal=1+(icycle*0.1)/ncycle !anneal ranges from 1 to 2
...
ccccccccccccccccccccc snapshots of E(1), E(N_rep) ccccccccccccc
         if(icycle.eq.icycle/1*1)then
            ...
            do k=1,n_repf
               call calc_arco(k)
               if(isca.eq.0) then
                  call calc_native_rms(k)
                  write(30+k,'i8,1x,f10.1,2i8,1x,f4.2,f7.2')
     &                 Lch,E_rep(k),i_tr,icycle,arco,armsCA
               else
                  write(30+k,'i8,1x,f10.1,2i8,1x,f4.2')
     &              Lch,E_rep(k),i_tr,icycle,arco
               endif
      ...
      do i=1,Lch
         ...
         do j=1,Lch
            ...
            if(i_pair3.gt.0.and.mk_pair3.gt.0)then
               apa(i,j)=fract_pair3*apba(i,j)+(1-fract_pair3)*apabla(ii,jj)-dd
               apm(i,j)=fract_pair3*apbm(i,j)+(1-fract_pair3)*apablm(ii,jj)-dd
               app(i,j)=fract_pair3*apbp(i,j)+(1-fract_pair3)*apablp(ii,jj)-dd
            endif
         enddo
      enddo
...
c-noe--scnoe----------------------------------------------------
      write(20,*) 'colim=',colim
      aco=0
      read(50,*)nnoe
      do i=1,nnoe
         read(50,*) r1(i),r2(i)
         aco=aco+iabs(r1(i)-r2(i))
      enddo

      do i=1,Lch
         Mnoe(i)=0              !number of contacts with 'i'
         do j=1,Nnoe
            if(r1(j).eq.i)then
               Mnoe(i)=Mnoe(i)+1
               Knoe(i,Mnoe(i))=r2(j)
      ...
      colim=colim+1.5*Nnoe*er4noe    !background number for derviation

      write(20,*)'Number of noe restraints:',Nnoe
      nnc=0
      do i=1,Lch
         nnc=nnc+Mnoe(i)
         write(20,12)i,Mnoe(i),(Knoe(i,j),j=1,Mnoe(i))
      enddo
      write(20,*)'Number of contact=',nnc,' Lch=',Lch
c-noe--scnoe----------------------------------------------------
...
c-noe--mcnoe----------------------------------------------------
      ...
      do i=1,Lch
         Mnoemc(i)=0              !number of contacts with 'i'
         do j=1,Nnoemc
            if(r1(j).eq.i)then
               Mnoemc(i)=Mnoemc(i)+1
               Knoemc(i,Mnoemc(i))=r2(j)
            endif
            if(r2(j).eq.i)then
               Mnoemc(i)=Mnoemc(i)+1
               Knoemc(i,Mnoemc(i))=r1(j)
            endif
         enddo
      enddo

      colim=colim+1.5*Nnoemc*er4noe    !background number for derviation

      write(20,*)'Number of noemc restraints:',Nnoemc
      nnc=0      
      do i=1,Lch
         nnc=nnc+Mnoemc(i)
         write(20,12)i,Mnoemc(i),(Knoemc(i,j),j=1,Mnoemc(i))
      enddo
      write(20,*)'Number of contact=',nnc,' Lch=',Lch
c-noe--mcnoe----------------------------------------------------
...
c-noe--smnoe----------------------------------------------------
      ...
      do i=1,Lch
         Mnoesm(i)=0              !number of contacts with 'i'
         Mnoems(i)=0              !number of contacts with 'i'
         do j=1,Nnoesm
            if(r1(j).eq.i)then
               Mnoesm(i)=Mnoesm(i)+1
               Knoesm(i,Mnoesm(i))=r2(j)
            endif
            if(r2(j).eq.i)then
               Mnoems(i)=Mnoems(i)+1
               Knoems(i,Mnoems(i))=r1(j)
            endif
         enddo
      enddo

      colim=colim+1.5*Nnoesm*er4noe    !background number for derviation

      write(20,*)'Number of noesm restraints:',Nnoesm
      nnc=0
      do i=1,Lch
         nnc=nnc+Mnoesm(i)
         write(20,12)i,Mnoesm(i),(Knoesm(i,j),j=1,Mnoesm(i))
      enddo
      write(20,*)'Number of contact=',nnc,' Lch=',Lch
      write(20,*)'Number of noems restraints:',Nnoesm
      nnc=0
      do i=1,Lch
         nnc=nnc+Mnoems(i)
         write(20,12)i,Mnoems(i),(Knoems(i,j),j=1,Mnoems(i))
      enddo
      write(20,*)'Number of contact=',nnc,' Lch=',Lch
      write(20,*) 'colim=',colim
c-noe--smnoe----------------------------------------------------
...
cccccccccccccccc read residual dipolar coupling info cccccccccccc
c res_i res_j minimum_projection maximum_projection          
c for each pair of residues (res_i,res_j), we allow the absolute
c value of the dot product betwen the internuclear vector HN
c of res_i and that of res_j to fall between minimum_projection
c and maximum_projection
c NOTE: not all possible pairs (res_i,res_j) have rdc info
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine read_rdc
      implicit integer(i-z)
      parameter(ndim=1000)	!maximum length of chain-length
     ...
     &           amaxpr(maxrdc)
      do i=1,Lch 
         do j=1,Lch
            isrdc(i,j)=0; !initialize isrdc;
         enddo
      enddo
      k=0
      read(60,*)nrdc !number of residual dipolar couplings
      do i=1;nrdc
         read(60,*) j,k,aminpr(i),amaxpr(i); !store info
         if(abs(j-k)>4)
         isrdc(j,k)=isrdc(k,j)=i; !associate index i to pair (j,k)
      endif
      enddo
...
ccccccccccccccccc  read_native  ccccccccccccccccccccccccccccccccc
c only CA atoms and first line is first CA atom
c we may add later some exceptions to handle more complicated pdb files
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine read_native
      implicit integer(i-z)
      parameter(ndim=1000)
      common/CA/dx(ndim),dy(ndim),dz(ndim),isca,armsCA
      common/lengths/Lch,Lch1,Lch2
      character prech*30
      do i=1,Lch
         read(28,fmt='A30,3F8.3F')prech,dx(i),dy(i),dz(i)
c         print*,'prech=',prech,'x=',dx(i),'y=',dy(i),'z=',dz(i)
      enddo
      return
      end
c^^^^^^^^^^^^^^^^^ read_native finished ^^^^^^^^^^^^^^^^^^^
...
c      a_rest=float(Ncom)/(1.3*Lch)

c-noe-----------------------------------------------------------
      a_rest=float(Ncom+(nnoe+nnoemc+nnoesm))/(1.3*Lch)
c-noe-----------------------------------------------------------
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c  calc_arco returns percentage of total contact order of side-chains
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine calc_arco(repf)
      ...
      if(aco.gt.0) then !aco==0 if no restraints
         ! block to find the coordinates of side-groups
         do i=1,Lch
            x(i)=xrep(i,repf)
            y(i)=yrep(i,repf)
            z(i)=zrep(i,repf)
         ...
         ica(Lch)=ica(Lch-2)
         do i=1,Lch
            ax(i)=x(i)+gx(ica(i-1),ica(i),seq(i))
            ay(i)=y(i)+gy(ica(i-1),ica(i),seq(i))
            az(i)=z(i)+gz(ica(i-1),ica(i),seq(i))
         enddo
         ! block to find the percentage of total contact order
         arco=0
         do i=1,Lch
            N_noe=Mnoe(i) !number of side-groups contacting side-group of "i"
            do k=1,N_noe
               j=Knoe(i,k)      !(i,j) contacting pair
               aij2=(ax(i)-ax(j))**2+(ay(i)-ay(j))**2+(az(i)-az(j))**2
               if( aij2.le.concut2(seq(i),seq(j)) ) then
                  arco=arco+iabs(i-j)
               endif
            enddo
         enddo
...
c     5, E13,E15,E15.
c     6  noe restraints
c     7, residual dipolar couplings
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
c      ESHORT=0.0
      ...
      ESHORT12=0 !residual dipolar coupling contacts
...
c-noe--scnoe-----------------------------------------------------------
       do i=iiii,jjjj
          N_noe=Mnoe(i)         !number of contacts on i
          if(N_noe.ge.1)then
             iseq=seq(i)
             if(mv(i).gt.0)then
                agxi=x(i)+GX(ica(i-1),ica(i),iseq) !SG
             ...
             do k=1,N_noe
                j=Knoe(i,k)     !k'th contact with i
                if(j.lt.i.OR.j.gt.jjjj)then
                   jseq=seq(j)
                   if(mv(j).gt.0)then
                      agxj=x(j)+gx(ica(j-1),ica(j),jseq)
                   ...
                   if(aij2.gt.concut2(iseq,jseq))then
                      axnoe=anneal*(1+abs(i-j)/Lch)
                      ESHORT4=ESHORT4+2*axnoe*er3noe !no contact
                      if(istat.lt.0)then
                         coold=coold+(sqrt(aij2)-concut(iseq,jseq))
     &                                 *er4noe !penality
                      else
                         conew=conew+(sqrt(aij2)-concut(iseq,jseq))
     &                                 *er4noe !panelity
                      endif
                   endif
                endif
       ...
       enddo
c-noe--scnoe--finished-------------------------------------------------

c-noe--mcnoe-----------------------------------------------------------
       do i=iiii,jjjj
          N_noe=Mnoemc(i)         !number of contacts on i
          if(N_noe.ge.1)then
             if(mv(i).gt.0)then
                axi=x(i)
                ayi=y(i)
             ...
             do k=1,N_noe
                j=Knoemc(i,k)     !k'th contact with i
                if(j.lt.i.OR.j.gt.jjjj)then
                   if(mv(j).gt.0)then
                      axj=x(j)
                      ayj=y(j)
                   ...
                   aij2=(axi-axj)**2+(ayi-ayj)**2+(azi-azj)**2
                   
                   if(aij2.gt.49.0)then
                      axnoe=anneal*(1+abs(i-j)/Lch)
                      ESHORT4=ESHORT4+2*axnoe*er3noemc !no contact
                      if(istat.lt.0)then
                         coold=coold+(sqrt(aij2)-7.0)*er4noe !penality
                      else
                         conew=conew+(sqrt(aij2)-7.0)*er4noe !panelity
                      endif
                   endif
                endif
             enddo
       ...
       enddo
c-noe--mcnoe--finished-------------------------------------------------

c-noe--smnoe-----------------------------------------------------------
       do i=iiii,jjjj
          N_noe=Mnoesm(i)         !number of contacts on i
          if(N_noe.ge.1)then
             iseq=seq(i)
             if(mv(i).gt.0)then
                agxi=x(i)+GX(ica(i-1),ica(i),iseq) !SG
             ...
             do k=1,N_noe
                j=Knoesm(i,k)     !k'th contact with i
                   ...
                   aij2=(agxi-axj)**2+(agyi-ayj)**2+(agzi-azj)**2
                   
                   if(aij2.gt.49.0)then
                      axnoe=anneal*(1+abs(i-j)/Lch)
                      ESHORT4=ESHORT4+2*axnoe*er3noesm !no contact
                      if(istat.lt.0)then
                         coold=coold+(sqrt(aij2)-7.0)*er4noe !penality
                      else
                         conew=conew+(sqrt(aij2)-7.0)*er4noe !panelity
       ...
       enddo
c-noe--smnoe--finished-------------------------------------------------

c-noe--ms-----------------------------------------------------------
       do i=iiii,jjjj
          N_noe=Mnoems(i)         !number of contacts on i
          if(N_noe.ge.1)then
             if(mv(i).gt.0)then
                axi=x(i)
                ayi=y(i)
             ...
             do k=1,N_noe
                j=Knoems(i,k)     !k'th contact with i
                if(j.lt.iiii.OR.j.gt.jjjj)then
                   jseq=seq(j)
                   if(mv(j).gt.0)then
                      agxj=x(j)+gx(ica(j-1),ica(j),jseq)
                      agyj=y(j)+gy(ica(j-1),ica(j),jseq)
                   ...
                   aij2=(axi-agxj)**2+(ayi-agyj)**2+(azi-agzj)**2
...
c  Residual dipolar coupling penalizations
      do i=iiii,jjjj
         bxi=HBX(ica(i-1),ica(i)) !NH vector estimation for i
         byi=HBY(ica(i-1),ica(i))
         bzi=HBZ(ica(i-1),ica(i))
         do k=1,Lch
            l=isrdc(i,k)
            if(l.gt.0) then
               bzk=HBX(ica(k-1),ica(k)) !NH vector estimation for k
               byk=HBY(ica(k-1),ica(k))
               bzk=HBZ(ica(k-1),ica(k))
               proj=abs(bxi*bxk+byi*byk+bzi*bzk) !projection (absolute value)
               if(proj<aminpr(l)) then
                  ESHORT12=ESHORT12 + exrdc !penalty
               else if(proj>amaxpr(l)) then
                  ESHORT12=ESHORT12 + eirdc !penalty
               endif
            endif
         enddo
      enddo
...
c      write(*,*)er6,eshort10,er5,eshort9,istat
     ...
     $     +er6*ESHORT10
     $     +er7*ESHORT11
     $     +er3*ESHORT12 !assume same weight than for contact restraints
c ^^^^^^^^^^ E_short finished ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
      RETURN
      END
...
c     d_xyz0=0.5               !range of translation movement
      ax0=(aranzy(no)*2-1)*d_xyz00(ifr) !translation of coordinates, [-det,det]
      ay0=(aranzy(no)*2-1)*d_xyz00(ifr)
      az0=(aranzy(no)*2-1)*d_xyz00(ifr)
...
***   draw distant fragment closer ----->
 ...
 33      t4=int(aranzy(no)*anvec)+1 !random vector
 ...
 34         t3=int(aranzy(no)*anvec)+1 !random vector
...
ccc   rotation angle           !in arc-angle, 1=57o, rotation angle.
      ang=(aranzy(no)*2-1)*angle00(ifr)/defo_angle ![-angle00/d,angle00/d]
      bng=(aranzy(no)*2-1)*angle00(ifr)/defo_angle ![-angle00/d,angle00/d]
ccc   rotation points:
      n_rot=1+int(n2*aranzy(no)) ![1,n2]
      ax=ex(n_rot)
      ay=ey(n_rot)
      az=ez(n_rot)
***   rotation matrix:
...
***   draw distant fragment closer ----->
 ...
 31      t1=int(aranzy(no)*anvec)+1 !random vector
         ...
         if(dis2_new.lt.dis2_old)then
 32         t2=int(aranzy(no)*anvec)+1 !random vector
...
c     rotation angle           !in arc-angle, 1=57o, rotation angle.
      ang=(aranzy(no)*2-1)*angle00(ifr) ![-angle00,angle00]
c     rotation points:
      n_rot=m2+int((n2-m2+1)*aranzy(no)) ![m2,n2]
      ax=ex(n_rot)
      ay=ey(n_rot)
      az=ez(n_rot)
***   rotation matrix:
...
ccc   rotation angle           !in arc-angle, 1=57o, rotation angle.
      ...
      bng=(aranzy(no)*2-1)*angle00(ifr)/defo_angle ![-angle00/d,angle00/d]
ccc   rotation points:
      n_rot=m2+int((Lch-m2+1)*aranzy(no)) ![m2,Lch]
      ax=ex(n_rot)
      ay=ey(n_rot)
      az=ez(n_rot)
***   rotation matrix:
...
cccccccccccccccc Metropolis: id=1, accepted; id=3, rejected ccccccccccccccccc
      ...
      id=1
      if(dE.gt.0)then
         if(aranzy(no).gt.weight12(dE))then
            id=3
         endif
      endif
      return
...
ccccccccccccccccc   calc_native_rms   cccccccccccccccccccccccccccccccc
c C_alpha rmsd to native
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine calc_native_rms(repf)
      implicit integer (i-z)
      parameter(ndim=1000)	!maximum length of chain-length
      parameter(nrep=100)       !maximum number of replicas
      common/CA/dx(ndim),dy(ndim),dz(ndim),isca,armsCA
      ...
      do i=1,Lch
         r_m(1,i)=xrep(i,repf)*0.87   !model conformation
         r_m(2,i)=yrep(i,repf)*0.87
         r_m(3,i)=zrep(i,repf)*0.87
         r_d(1,i)=dx(i)          !native conformation
         r_d(2,i)=dy(i)
         r_d(3,i)=dz(i)
      enddo
      call u3b(w,r_m,r_d,Lch,0,rms,u,t,ier)
      armsCA=dsqrt(rms/Lch)	! armsCA is real, rms is double precision
      return
      end
c^^^^^^^^^^^^^^^^   calc_native_rms  ends here  ^^^^^^^^^^^^^^^^^^^^^^
