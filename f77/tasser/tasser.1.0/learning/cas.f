...
c     nwmax is the possible number of vectors joining two CA's
                  nwmax=nwmax+1
c     vx(i),vy(i),vz(i) are the actual coordinates of one bond vector
                  vx(nwmax)=x
                  vy(nwmax)=y
                  vz(nwmax)=z
...
****************************************************************
c     translation distance d_xyz00(i), rotation angle00(i)
      do i=1,nfr !cycle through all fragments
         siz=nfr_f(i)-nfr_i(i)+1 !fragment size
         angle00(i)=angle0*10/float(siz) !angle0=0.2radians (11.4degrees) (10 is typical fragm siz)
         d_xyz00(i)=d_xyz0*10/float(siz) !d_xyz0=1.0
         if(siz.gt.40)d_xyz00(i)=d_xyz00(i)/2.0
      enddo
c^^^ d_xyz00(i), angle00(i) done ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
c     (bxk,byk,bzk) direction where H-bond can be made
            ...
            byk=HBY(nv1,nv2)
            bzk=HBZ(nv1,nv2)
c     (cxk,cyk,czk) unit vector resulting from the normalization of the
c     substraction of (avxj,avyj,avzj) to (avxi,avyi,avzi)
            cxk=CAX(nv1,nv2)    
            cyk=CAY(nv1,nv2)
            czk=CAZ(nv1,nv2)
...
***   alpha-helix:
                  if(sec(k).ne.4.and.sec(i).ne.4)then !both not assigned as strands
...
c                write(*,'i3ai5')k,' contact is residue',j
...
c     Move the N-terminal of the selected off-lattice ifr-fragment
c     This movement include an off-lattice rotation of fragment
c     and a two-bond movement in each end of fragment.
c     The minimum length of fragment is 2, because of m3
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
ccc   d_xyz0=0.5               !range of translation movement
      ax0=(aranzy()*2-1)*d_xyz00(ifr) !translation of coordinates, [-d_xyz00(ifr),d_xyz00(ifr)]
      ay0=(aranzy()*2-1)*d_xyz00(ifr)
      az0=(aranzy()*2-1)*d_xyz00(ifr)
ccc   rotation axis (awx,awy,awz)->N-terminal; (bwx,bwy,bwz)->C-terminal:
      ...
      asin_theta=sqrt(1.0-acos_theta*acos_theta)
      aphi=2.*3.1415926*aranzy() ![0,2*pai]
      awx=asin_theta*cos(aphi)   !N-terminal
      awy=asin_theta*sin(aphi)
      awz=acos_theta
      ...
      asin_theta=sqrt(1.0-acos_theta*acos_theta)
      aphi=2.*3.1415926*aranzy() ![0,2*pai]
      bwx=asin_theta*cos(aphi)   !C-terminal
      bwy=asin_theta*sin(aphi)
      bwz=acos_theta
ccc   rotation angle           !in arc-angle, 1=57o, rotation angle.
      ang=(aranzy()*2-1)*angle00(ifr)/defo_angle ![-angle00/d,angle00/d]  (defo_angle=2.5)
...
***   rotation matrix:
      ...
      asin=sin(ang)             !*** should not be bng instead ? ***
...
***   draw distant fragment closer ----->
     ...
     $     +(z(n1)-nint(ez(n2)))**2
      ...
      if(r2_bond.gt.25)then
         dis2_old=(x(n1)-ex(n2))**2+(y(n1)-ey(n2))**2+(z(n1)-ez(n2))**2
 33      t4=int(aranzy()*anvec)+1 !randomly pick a bond vector (anvec=nvec-0.0001)
         if(.not.goodc(t4,ica(n)))goto 33
         xn1=x(n)-vx(t4)
         yn1=y(n)-vy(t4)
         zn1=z(n)-vz(t4)
