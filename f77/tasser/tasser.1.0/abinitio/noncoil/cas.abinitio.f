...
*     pgf77 -Mextend -O -s -fast -Wl,-static -o cas.abinitio.x cas.abinitio.f
...
*****************************************************************************
c      pgf77 -Mextend -O -s -fast -Wl,-static -o cas_abinitio cas_abinitio.f
c        1         2         3         4         5         6         7 !
...
c      open(unit=22,file='distL.dat',status='old') !long range CA dist restraint
      ...
      open(unit=24,file='chain.dat',status='unknown')
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c      write(*,*) 'switch=',switch,' ncycle=',ncycle,' N_rep=',N_rep
...
c
      ...
      do i=1,Lch
         ...
         do j=1,Lch
            ...
            if(is.eq.4.or.js.eq.4) then !at least one is beta
               idist=iabs(i-j)
               if(is*js.ne.8.and.idist.gt.4)then !none is helix
                  EHBIJ(i,j)=EHBIJ(i,j)+0.5 !beta-beta or beta-coil, enhanced
                  if(idist.gt.longrange) EHBIJ(i,j)=2*EHBIJ(i,j) !enhanced long-range
               endif
            endif
         enddo
      enddo
...
***   1: ras => gap +- 2, smallest moveable-length is 3
      ...
      do i=1,n_g
         ...
         !end of modification stamp 20070412
         if(i1.lt.1)i1=1
         if(i2.gt.Lch)i2=Lch
         do ii=i1,i2
            nfl=nfl+1           !number of moveable residues
