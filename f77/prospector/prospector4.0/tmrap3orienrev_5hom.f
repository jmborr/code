...
c	4/7/01  Jeffrey Skolnick
	...
	COMMON/nam/name3
	OPEN(UNIT=9,FILE='stat_tmrap3orienrev_5')
...
9432    format(i6,3x,a5,3x,a5,1f7.3,,3x,a60)
	open(unit=16,file=home(1:nhome)//'/pdbhomoljan05/'//namep//'.homol')
	read(16,*)nhom
	nhomold=nhom
	do mk=1,nhom
	read(16,16)nameh(mk)
...
c	3590 % id
	...
	write(9,*)namep,'   ****'
	OPEN(unit=45,file=home(1:nhome)//'/cafiles4/'//NAMEp//'.real')
	rewind(45)
	read(45,*)mres
 	L2=MRES
        do i=1,L2
 ...
 103  format(A22,i4,A4,3F8.3)
	...
	OPEN(UNIT=31,FILE='/net/dell/06/adrian_dat/pdb300/3590/'//namep//'.aln')
	rewind(31)
	...
	NSEQ1=MRES
	jk=1
	OPEN(unit=30,file=home(1:nhome)//'/orien6/'//NAMEp//'.SEQ')
		rewind(30)
...
C Prepare a SCORE table
	...
	nrank=0
        ...
        OPEN(unit=34,file=home(1:nhome)//genome//'pdborienrev/'//namep//'rap3orienrev1.pdb')
	read(3,*)nth
	if(nth.gt.5)nth=5
	rmsmin=9000
	TMSmax=0.
...
21	format(a5,1x,i3,1x,i3,1x,2(1f7.3,1x))
211	format(a8,2x,i3,1x,2(1f7.3,1x))
