	OPEN(UNIT=9,FILE='stat_splinerap3orienrev_53')
	CALL r14STAT()
	STOP
	END
...
c	PARAMETER(REJECT=25.)	
	...
	character*20 a33,a20(nmax)
	character*1 a3,qdir
	CHARACTER*6 a6
	CHARACTER*60 head
	CHARACTER*5 NAMEH(500),namel
	CHARACTER*255 NAMEP,NAMEV(0:ncases)
	CHARACTER*1 AA(0:19),ad
	CHARACTER*21 TEXT
	CHARACTER*5 name5,nameth(100)
	character*4 genome
	CHARACTER*6 prodin
	LOGICAL*1 DIR,kselect,hommap(NCASES),TM1(0:NMAX,5)
        logical*1 tgood
	logical*1 taln(0:nmax),tmb(0:nmax,5)
...
c	DIMENSION AMUT(-1:19,0:NMAX)	
	...
	dimension i1(nmax),i2(nmax),tmat(20)
        common/para/d,d0	
...
13      format(a6)
	...
	OPEN(unit=63, file='listthread_splinerap3orienrev')
	rav=0
	read(33,*)ndata2
	iden=0
	rmsmin=0.
...
9432    format(i6,3x,a,3x,a5,1f7.3,,3x,a60)
	...
	conpred3(i,j)=0
	 ...
	 OPEN(unit=34,file='/nfs/users/skolnick/'//genome//'pdborienrev/'//namep(1:nsize)//'rap3orienrev1.pdb')
	read(3,*)nth
	...
	read(34,15)nameth(jth),ic
15	format(a5,1x,i4)

	do id=1,ic
        read(34,1033)a33,i,(xa(l,i,jth),l=1,3)
...
1999	CONTINUE
	...
	L1=Id1
	IF(L1.eq.0) go to 2000
	if(L2.eq.0)go to 2000
        call TMscore(L1,x1,y1,z1,n1,L2,x2,y2,z2,n2,TMS,Rcomm,Lcomm,SS)
c	write(9,8)L1,L2,TMS,Rcomm,Lcomm,ith,jth
	tmat(jth)=TMS
	if(TMS.gt.tb)then
	lth=jth
	l1b=l1
	...
	jth=lth
	write(9,8)namep,L1b,L2b,Tb,Rcommb,Lcommb,ith,lth
8	format(a5,1x,2(i3,1x),1x,2(1f7.3,1x),4(i3,1x))
	do jth=1,nth
	item(jth)=jth
	end do
        do jk=1,nth
        do kk=jk+1,nth
        if(tmat(jk).lt.tmat(kk))then
        itt=item(jk)
        item(jk)=item(kk)
        item(kk)=itt
        etem=tmat(jk)
	...
	if(L2.eq.0)go to 2000
        call TMscore(L1,x1,y1,z1,n1,L2,x2,y2,z2,n2,TMS,Rcomm,Lcomm,SS)
	do i=1,mres
	if(tm1(i,jth) .and. .not.tmb(i,ith))then
	ii1=i1(i)
	tmb(i,ith)=.true.
	xb(1,i,ith)=x1(ii1)
	xb(2,i,ith)=y1(ii1)
...
14	continue
2000	continue
	OPEN(unit=35,file='/nfs/users/skolnick/'//genome//'splinerap3orienrev/'//namep(1:nsize)//'spline3.pdb')
	do ith=1,nth
	ic=0
	do i=1,mres
	if(tmb(i,ith))ic=ic+1
	end do
	write(35,15) nameth(ith),ic
	do i=1,mres
	if(tmb(i,ith))then
        write(35,1033)a20(i),i,(xb(l,i,ith),l=1,3)
	end if
	end do
	write(35,35)'TER'
35	format(a3) 
...
1001	continue
        write(19,*)'========='
        write(19,*)'average contact statistics'
