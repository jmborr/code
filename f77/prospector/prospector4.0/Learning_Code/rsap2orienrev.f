...
1100	continue
	...
	if(nres(ik).gt.nmax)nres(ik)=nmax
        ...
        open(unit=31,file=templatedat(1:nadr)//'pdb_jan05/e10/'//name(ik)//'.aln')
	nunit=31
        call read_msa(nunit)
        rewind(31)
        close(31)
        ...
        do ic=1,nseq_ori
	        ...
	        nden=nden+1
		ifact2=20+ires
                xm(ifact2,i,ik)=xm(ifact2,i,ik)+1
                end if
        end do  
		do ires=0,19
		ires2=20+ires
                xm(ires2,i,ik)=xm(ires2,i,ik)/nden
                end do
        end do
	close(31)	
        open(unit=31,file=templatedat(1:nadr)//'pdb_jan05/3590/'//name(ik)//'.aln')
	nunit=31
        call read_msa(nunit)
        rewind(31)
        close(31)
...
13      format(a6)
	close(11)
	OPEN(UNIT=9,FILE='stat_rap2orienrev'//prodin)
	OPEN(UNIT=69,FILE='completedrap2orienrev'//prodin)	
...
11	format(a)
	...
	conpred(i,j)=0.
       ...
       OPEN(unit=44,file=home(1:nhome)//genome//'zpot4a/'//namep(1:nsize)//'.potcontactexprsaorienrev')
        do ils=1,3
       do i=1,mres
       read(44,*)
...
c	==============================================
	...
	an(i,j,ils)=5*an(i,j,ils)/nseq_ori
        an01(i,j,ils)=(an02(i,j,ils)+0.25*an01(i,j,ils))/1.25
...
c871 	format(a1,3x,a1,2x,i4,2x,i4)
	...
	OPEN(unit=44,file=home(1:nhome)//genome//'zpot4a/'//namep(1:nsize)//'.potcontactexprsap2orienrev')
	do ils=1,3
       do i=1,mres
        iseq=jcode(i)
        write(44,*)i,aa(iseq)
