...
1100	continue
	...
	nunit=31
		...
		do ires=20,39
                xm(ires,i,ik)=xm(ires,i,ik)/nden
                end do
        end do
	close(31)	
...
c	==============================================
	...
	an(i,j,ils)=0.5*(an(i,j,ils)-an01(i,j,ils))
	end do
	end do
	end do
...
c	==============================================
	...
	an2(i,j,ils)=0.3*(an2(i,j,ils)-an01(i,j,ils))
	end do
	end do
	end do
...
c	BWT=1.0
	...
	ibin=jbin(j,jk)
        ...
        ibinseq=jseq(i)
	II=NSEQ1-I+1
 	score(ii,j)=score(ii,j)+eseq(ibinseq,ibin)*awt
        if(nn.gt. 0)then
                do jj=1,nc(j,jk)
                ll=icon(jj,j,jk)
	        ils=ior(jj,j,jk)		
...
c	call  DP(NSEQ1,NSEQ2,RESULT,ils,length)
	...
	ibin=jbin(j,jk)
        DO i=1,NSEQ1	
        ibinseq=jseq(i)
 	score(i,j)=score(i,j)+eseq(ibinseq,ibin)*awt
        if(nn.gt. 0)then
                do jj=1,nc(j,jk)
                ll=icon(jj,j,jk)
	        ils=ior(jj,j,jk)		
