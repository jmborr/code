	PARAMETER(nmax=1000)   !maximum template-sequence-length
        parameter (maxseq=300) !maximum number of sequences in the multiple sequence alignment
        parameter (maxres=500) !maximum query-sequence-length
...
1	   format(a5)
	   OPEN(unit=30,
     &  file='/local/images/templates-2007081300/orien6/'
     &  //NAME(ik)//'.SEQ')
	   rewind(30)
 ...
 2	   format(a5,1x,i5)
	   ...
	   OPEN(unit=71,
     &  file='/local/images/templates-2007081400/orien6fitgly/'
     &         //NAME(ik)//'.FITCONT_ORGLY')
	   rewind(71)
	   do i=1,nres(ik)
...
64	   format(i2,1x,2(1f7.3,1x))
	   ...
	   do jk=1,ndata !sort by decreasing energy
	      do kk=jk+1,ndata
	   ...
	   imv=0
	   do jk=1,30 !cycle through the first 30 templates
	      tpsec(jk,k)=.false.
	      jjm=item(jk,k)
	      zz=(energ(jk,k)-en(k))/sd(k)
	      if(zz .gt. 1.3 )then
		 imv=imv+1
		 if(imv .gt.5)go to 1111 !stop if we already processed five templates
		 tpsec(jk,k)=.true.
...
c       fill matrix touch2
	      do j=1,nres(jjm) !cycle over template residues
		 if(invmap(j,jjm,k).gt.-1)then
		    i=invmap(j,jjm,k) !residue in query "i" aligned to residue "j" in template
		    ires=jcode(i)     !residue type
		    jres=icode(j,jjm) !residue type
		    if(imut(ires,jres).gt.0)then !blosum favorable mutation
		       do jj=1,nc(j,jjm) !cycle trhough all contacts of "j" in the template structure
			  ll=icon(jj,j,jjm) !"ll" is neighbor residue of "j" in the template structure
			  li=invmap(ll,jjm,k) !residue in query "li" aligned to residue "ll" in template
			  if(li .gt. -1)then
			     ires2=jcode(li)
			     jres2=icode(ll,jjm)
			     if(imut(ires2,jres2).gt.0)then
				touch2(i,li)=.true. !putative (i,li) contact by threading transfer
				touch2(li,i)=.true.		
			     end if
			  end if
		       end do
...
6543	      continue	
	   END DO !(jk->jk+1) another template
	END DO !(k->k+1) another alignment scheme
	...
	do j=1,mres	
	   ...
	   do i=1,mres
		 ...
		 if(conpred3(i,j).gt.3)then
		    do ils=1,3
		       an2(i,j,ils)=-alog(conpred3(i,j)/aden) !irrespective of orientation
		    end do
		 end if
	      end if
	   end do
	...
	write(9,*)'============================='
 	...
 	OPEN(unit=44,file=home(1:nhome)//genome//'zpot4a/'//namep(1:nsize)//
     &  '.potcontactexpr5saorienrev')
	do ils=1,3
	   do i=1,mres
	      iseq=jcode(i)
	      write(44,*)i,aa(iseq)
