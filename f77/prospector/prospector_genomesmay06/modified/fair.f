	OPEN(UNIT=9,FILE='stat_fair')
	CALL r14STAT()
	open(unit=1,file='finish_fair')
	rewind(1)
	write(1,*)'1'
	close(1)
	STOP
...
1	format(a)
	...
	OPEN(unit=1, file='list_verygood')
	nn=nfz
	read(1,*)nfz
	do ik=1,nfz
	nn=nn+1
	read(1,1)namep5(nn)(1:nsize)
	end do
	nfz=nn
	OPEN(unit=33, file='STRUCTURE_rap3orienrev')
	OPEN(unit=63, file='list_fair')
	rav=0
	read(33,*)ndata2
	iden=0
	rmsmin=0.
...
201	continue
        rewind(30)
	OPEN(unit=30,file='/local/images/templates-2007081400/orien6/'//NAME5//'.SEQ')
	rewind(30)
...
1033    FORMAT(a20,2X,I4,1x,3X,3F8.3,1x,i4,1x,a3)
	...
	read(34,*)
        ...
        OPEN(unit=71,file='/local/images/templates-2007081400/orien6fitgly/'//name5//'.FITCONT_ORGLY')
        rewind(71)
        do i=1,NSEQ3
