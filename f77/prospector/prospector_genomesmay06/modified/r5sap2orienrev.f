...
1100	continue
	CLOSE(20)
	DO 1101 IK=1,NDATA
	   open(unit=59,file='/local/images/templates-2007082700/struprofilejul07/'//name(ik)//
     &     '.profile2')
...
11	format(a)
	...
	rewind(31)
	call read_msa(nunit) !fill up kcode with the 3590 multiple sequence alignment
	rewind(31)	
	close(31)
...
c	==============================================
	...
	do j=1,mres
	   do i=1,mres
		 ...
		 an(i,j,ils)=0.9*(an(i,j,ils)-an02(i,j,ils))!some combination of generic/specific potential
	      end do
	   end do
	end do
!	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
c             prepare position specific scoring matrix between query and template
		 ...
		 DO i=1,NSEQ1	!cycle over query residues	
		    ...
		    score(ii,j)=ss+eseq(ibinseq,ibin)*bwt+awt !mixture of e10 and mtx and sec.st
		 end do
	      end do
C Prepare a SCORE table
	      lst=1
