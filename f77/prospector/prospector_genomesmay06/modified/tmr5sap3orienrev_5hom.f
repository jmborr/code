...
c	4/7/01  Jeffrey Skolnick
	...
	CHARACTER*21 TEXT
	CHARACTER*5 name5,namep2(ncases),nameh(ncases)
	character*4 genome
	CHARACTER*6 prodin
...
13      format(a6)
	...
	do 1001 IIk=1,NDATA2
        read(33,9432)iselected,namep
9432    format(i6,3x,a5,3x,a5,1f7.3,,3x,a60)
	open(unit=16,file=home(1:nhome)//'/pdbhomolmay06/'//namep//'.homol')
	read(16,*)nhom
	nhomold=nhom
	do mk=1,nhom
	read(16,16)nameh(mk)
...
c	3590 % id
	do jk=1,nfz
	if(namep.eq.namep2(jk))go to 1001
	end do
	write(9,*)namep,'   ****'
	OPEN(unit=45,file='/library/cafiles6/'//NAMEp//'.real')
	rewind(45)
	read(45,*)mres
 	L2=MRES
        do i=1,L2
 ...
 103  format(A22,i4,A4,3F8.3)
	...
	OPEN(UNIT=31,FILE='/gpfs1/archive/adrian/dat/pdb200/3590/'//namep//'.aln')
	rewind(31)
	...
	NSEQ1=MRES
	jk=1
	OPEN(unit=30,file='/library/orien6/'//NAMEp//'.SEQ')
		rewind(30)
...
C Prepare a SCORE table
	...
	nrank=0
        OPEN(unit=3,file=namep//'.threadrap3orienrev')
        OPEN(unit=34,file=home(1:nhome)//genome//'pdborienrev/'//namep//'rap3orienrev5s.pdb')
	read(3,*)nth
	if(nth.gt.5)nth=5
	rmsmin=9000
	TMSmax=0.
...
1033    FORMAT(a20,2X,I4,1x,3X,3F8.3,1x,i4,1x,a3)
	...
	write(9,39)namep,name5, pid, mres,LCOMM, Rcomm,TMS,SS
39	format(2(a5,1x),1f7.3,1x,2(i4,1x),3(1f7.3,1x))
	end if

	
	if(SS .gt.SMAX)SMAX=ss
...
2000	continue
2001    continue
	if(nnmin.eq.0) go to 9999
c	iF(SMAX.gt.0.01)then
	nmaxs=nmaxs+1
	SMAXAV=SMAXAV+SMAX
c	end if
...
22	continue
	...
	imin=int(rmsmin)+1
	if(rmsmin .le.6.5)then
	if(imin .lt.8)then
	do ir=imin,7
	rg(ir)=rg(ir)+rmsmin
	iav(ir)=iav(ir)+1
	cav(ir)=cav(ir)+float(nnmin)/mres
	end do
	endif
	end if
...
9999	continue
	if(nhom.ne.nhomold)then
	write(53,166)namep,nhom,nhomold
166	format(a5,1x,2(i4,1x))
	rewind(16)
	write(16,*)nhom
	do mk=1,nhom
	write(16,16)nameh(mk)
	end do
	end if
1001	continue
	write(9,*)'number of cases=',iden,' <rms>=',rav/iden
	write(9,*)'average coverage= ',fav/iden		
	write(9,*)'number of cases=',iden,' <%Id>=',pidav/iden
	write(9,*)'number of proteins<6.5 A =',ngood
	write(9,*)'average RMSD of proteins<6.5 A =',ravg/ngood
	write(9,*)'average coverage of proteins<6.5 A =',favg/ngood		
	write(9,*)
	write(9,*)'total number of cases=',ndata2
	write(9,*)'number of cases where TM-score >0.17 =', NTMGOOD
	write(9,*)'average rank of proteins tmscore<0.17 A =',float(nav)/ntmgood
	write(9,*)
	write(9,*)'	============='
	write(9,*)' Number of MAX SUB targets=',nmaxs
	write(9,*)' CUMULATIVE MAX SUB SCORE=',SMAXAV
	write(9,*)' average MAX-sub score=',smaxav/nmaxs
	write(9,*)'	============='
	write(9,*)
	write(9,*)'CUMULATIVE TM SCORE= ',TMAV
	write(9,*)'average TM SCORE=', TMAV/NTMGOOD
	write(9,*)'TM SCORE HISTOGRAM:'
	do ihist=0,20
...
50	format(1f7.4,1x,i4)
	...
	if(iden .gt.0)then
	if(imin .lt.7)then
	write(9,21)'rms< ',imin,iden,rg(imin)/iden,cav(imin)/iden
	else
	write(9,211)'rms<6.5 ',iden,rg(imin)/iden,cav(imin)/iden
	end if
	end if
	end do
21	format(a5,1x,i5,1x,i5,1x,2(1f7.3,1x))
211	format(a8,2x,i5,1x,2(1f7.3,1x))
