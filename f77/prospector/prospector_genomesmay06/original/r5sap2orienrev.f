...
c	&&&&&&& templatedir:    &&&&&&&&&&
	...
	jmap(5)=2
     	jmap(0)=0
	OPEN(UNIT=20,FILE='LIST.may2006good35')
...
2       format(a5,1x,i5)
	...
	rewind(30)
	close(30)
	OPEN(unit=71,file='/library/orien6fitgly/'//NAME(ik)//
     &	'.FITCONT_ORGLY')
	rewind(71)
	do i=1,nres(ik)
	...
	close(71)
	OPEN(UNIT=14,FILE='//gpfs1/archive/adrian/dat/pdb_may06.apr06/seq/'//name(ik)//'.SEQ')
...
1100	continue
	CLOSE(20)
	DO 1101 IK=1,NDATA
        open(unit=59,file='/library/struprofilemay06/'//name(ik)//'.profile')
...
1101	continue
	...
	DIMENSION EN(4),EN2S(4),SD(4)
	DIMENSION E(4)
	COMMON/MAPPINGS/JCODE(-1:MAXRES),IMAP(0:NMAX)
...
c	==============================================
	DIMENSION AN2(-1:MAXRES,-1:MAXRES,3),CONPRED(-1:MAXRES,-1:MAXRES)
        DIMENSION JMAP(0:6)
	DIMENSION CONPRED2(-1:MAXRES,-1:MAXRES)
	DIMENSION AN0(-1:MAXRES,-1:MAXRES)
	DIMENSION AN01(-1:MAXRES,-1:MAXRES,3),AN02(-1:MAXRES,-1:MAXRES,3)
        COMMON/SEQ/NRESSEQ,NSEQ_ORI,KCODE(NMAX,MAXSEQ)
	COMMON/PROFILE/XM(0:59,0:NMAX,0:NCASES)
...
c	======================================================
	...
	OPEN(UNIT=27,file='/library/orien6fitgly/PAIRGLY',status='old')
	read(27,*)
	read(27,*)
	DO I=0,19
...
13      format(a6)
	close(11)
	OPEN(UNIT=9,FILE='stat_r5sap2orienrev'//prodin)
	OPEN(UNIT=69,FILE='completedr5sap2orienrev'//prodin)	
...
11	format(a)
	...
	conpred(i,j)=0.
       ...
       do i=1,mres
       read(44,*)
       read(44,167) (an02(i,j,ils),j=1,mres)
       read(44,*)
       end do
       end do
       close(44)       
c	==============================================
	...
	an(i,j,ils)=0.9*(an(i,j,ils)-an02(i,j,ils))
	end do
	end do
	end do
!	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	an2(i,j,ils)=0.45*(an2(i,j,ils)-an02(i,j,ils))
	end do
	end do
	end do
	do i=1,mres
...
c       attempt to extractcontacts
        ...
        tpsec(jk,k)=.true.
	write(9,*)jjm,' ',name(jjm),energ(jk,k),zz
        do j=1,nres(jjm)
...
6543    continue
	...
	OPEN(unit=21, file=home(1:nhome)//genome//'zpot4a/'//namep(1:nsize)//'.predictedrap2orienrev')
	write(21,*)nt/2
	do j=1,mres	
	do i=1,mres
	if(iabs(i-j).gt.4)then
...
C  Dynamic Programming for Sequence Alignment, Fortran vers
C
C
