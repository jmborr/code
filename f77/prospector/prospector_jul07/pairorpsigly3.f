...
1112	format(a6)
	...
	OPEN(unit=26,file='../strlist/PAIRORGLY3')
	do it=1,3
	read(26,*)
	read(26,*)
	do i=0,19
...
17	FORMAT(A3,20F5.1)
	...
	OPEN(unit=50,file='LIST.jul2007good35')
	read(50,*)NDATA
	NTOT=0
...
221	format(a5)
	...
	do ik=1,ndata
	OPEN(unit=30,file=' /local/images/templates-2007081300/orien6/'//NAME(ik)//'.SEQ')
	READ(30,2)NAME2,Nres(ik)
	if(nres(ik).gt.nmax)nres(ik)=nmax-1
...
2	format(a5,1x,i5)
	DO IK=1,NDATA
        open(unit=31,file='../templatesmtxjul07/'//name(ik)//'.mtx')
	do i=1,nres(ik)
	do ires=0,19
	xm(ires,i,ik)=0.
	end do
	...
	read(31,*)
        do i=1,nres(ik)
        read(31,59)id,(xm(ires,i,ik),ires=0,19)
59      format(i4,1x,20(1f7.3,1x))
	read(31,*)
	end do
	END DO
	do ik=1,ndata
...
c	call contb(NRES)
	...
	read(71,*)ii,nc(ii,ik),(lor(j,ii,ik),j=1,nc(ii,ik))
        do j=1,nc(ii,ik)
...
c	read in of homologous sequence homologous sequences
	...
	hommap(jk)=.true.
        ...
        open(unit=8,file=home(1:nhome)//'/pdbhomoljul07/'//names(iik)(1:nsize)//'.homol')
        read(8,*)nhom
        do id=1,nhom
...
c	OPEN(unit=71,file='../mar07dir/orien6fitgly_mar07')
	DO IK=1,NDATA
	do i=1,nres(ik)
	read(71,*)ii,nc(ii,ik),(icon(j,ii,ik),j=1,nc(ii,ik))
	nt=0
