*************************************************************************
*     pgf77 -g -Mbounds -Mchkfpstk -Mextend  -Wl,-static -o TMalign_listsII TMalign_listsII.f
*     pgf77 -Mextend -O -s -fast -Wl,-static -o TMalign_listsII TMalign_listsII.f
*
...
*************************************************************************
      ...
      program compares
      PARAMETER(nmax=2000)
      PARAMETER(nmax2=4000)
      PARAMETER(lmax=16501) !maximum number of structures and targets

c     variables introduced for alignments among more than two proteins
      real xas(3,nmax,0:1,lmax) !extension to XA
      integer nseqXs(lmax,0:1) !extension to nseq1,nseq2
      character seqXs(0:nmax,lmax,0:1)!extension to seq1,seq2
      character*3 ssXs(nmax,lmax,0:1) !extension to ss1,ss2
      character*200 structDir !directory of structures
      character*200 targDir !directory of targets
      ...
      character*200 outfile,outlocalfile !redirect standard output to this file
      integer nstru,ntarg !number of structures and targets
      integer mmXs(nmax,lmax,0:1) !residue number
      integer readPDBfile
      ...
      character*264 fnam,pdb(0:1),outname
...
ccc   
     ...
     $        'to many targets:'
         write(*,*)' >TMalign -ss strList -tt targList -sdir structDir',
     $        ' -tdir targDir'
         write(*,*)
         ...
         write(*,*)'  >TMalign ... -simpl'
         write(*,*)
c         write(*,*)'6. If you want to set a minimum cutoff for d0, ',
c     &        'e.g. d0>3.0'
...
******* options ----------->
      ...
      narg=iargc()
      i=0
      nstru=0 !number of structures
      ntarg=0 !number of targets
 115  continue
      i=i+1
      call getarg(i,fnam)
      ...
      if(fnam.eq.'-s')then
         i=i+1
         call getarg(i,fnam)
         write(stru(1),*) fnam(1:lnblnk(fnam)) !load model file name to memory
         nstru=1
      elseif(fnam.eq.'-t')then
         i=i+1
         call getarg(i,fnam)
         write(targ(1),*)fnam(1:lnblnk(fnam)) !load native file name to memory
         ntarg=1
...
c     read data from structures
      ...
      do istru=1,nstru
         ...
         nseqXs(istru,0)=readPDBfile(fnam,xas,seqXs,ssXs,mmXs,0,istru)
      enddo
...
c     iterate over structures (and targets)
 ...
 555  iM=0
      ...
      pdb(0)=stru(istru)          
      do ires=1,nseq1
         seq1(ires)=seqXs(ires,istru,iM)
         ss1(ires)=ssXs(ires,istru,iM)
         mm1(ires)=mmXs(ires,istru,iM)
         do j=1,3
            xa(j,ires,0)=xas(j,ires,iM,istru)
         enddo
      enddo      
      if(ntarg.eq.0) then !no targets, compare among structures
 ...
 556  nseq2=nseqXs(itarg,iN)   !initialize properties of target
      ...
      do ires=1,nseq2
         seq2(ires)=seqXs(ires,itarg,iN)
         ss2(ires)=ssXs(ires,itarg,iN)
         mm2(ires)=mmXs(ires,itarg,iN)
         do j=1,3
            xa(j,ires,1)=xas(j,ires,iN,itarg)
         enddo
      enddo
...
***   remove dis>d8 in normal TM-score calculation for final report----->
     ...
     &        (ztm1(i)-ztm2(i))**2)
      ...
      anseq=nseq1 !calculate TM-score based on first protein
      if(anseq.gt.15)then
         d0=1.24*(anseq-15)**(1.0/3.0)-1.8 !scale for defining TM-score
      else
         d0=d0_min
     ...
     &     xtm2,ytm2,ztm2,n2,TM8,Rcomm,Lcomm) !normal TMscore
      TMfirstseq=TM8*n8_al/anseq       !TM-score after cutoff

*!!!  Output TM-score is based on the second protein by default-------------->
...
********* for output summary ******************************
      if(m_simpl.eq.1)then !simplifed output
         write(6,'axaxf6.4xf6.4') pdb(0)(1:lnblnk(pdb(0))),
     $        pdb(1)(1:lnblnk(pdb(1))),TMfirstseq,TM8
         goto 8999
      endif
     ...
     &     '*********************'
      write(6,*)'*                               TM-align             ',
     &     '                    *'
      write(6,*)'* A protein structural alignment algorithm based on T',
     &     'M-score             *'
      write(6,*)'* Reference: Y. Zhang and J. Skolnick, Nucl. Acids Re',
     &     's. 2005 33, 2302-9  *'
      write(6,*)'* Comments on the program, please email to: yzhang@ku',
     &     '.edu                *'
      write(6,*)'*****************************************************',
     &     '*********************'
      write(6,*)
      write(6,101)pdb(1),nseq1
 101  format('Chain 1:',A10,'  Size=',I4)
      write(6,102)pdb(2),nseq2,int(anseq)
 102  format('Chain 2:',A10,'  Size=',I4,
     &     ' (TM-score is normalized by ',I4,')')
      write(6,*)
      write(6,103)n8_al,rmsd8_al,TM8,seq_id
...
********* extract rotation matrix ------------>
      ...
      do i=1,n8_al
       ...
       if(L.gt.3)then
         ...
         armsd=dsqrt(rms/L)
         write(6,*)'-------- rotation matrix to rotate Chain-1 to ',
     &        'Chain-2 ------'
         write(6,*)'i          t(i)         u(i,1)         u(i,2) ',
     &        '        u(i,3)'
         do i=1,3
            write(6,204)i,t(i),u(i,1),u(i,2),u(i,3)
         enddo
c         do i=1,nseq1
...
*     Output: invmap(j)
            ...
            call get_score      !calculate TM-score, score(i,j)
c     write(6,21)gap_open,rmsd_al,n_al,TM
c     record the best alignment in whole search ---------->
            if(TM.gt.TMmax)then
               TMmax=TM
               do j=1,nseq2
...
c     load contents of file "list" into memory (files)
 ...
 666  call exit(1)
      ...
      PARAMETER(nmax=2000) !maximum number of residues
      PARAMETER(lmax=16501) !maximum number of model or native structures
      character*200 fnam          !pdb file name
      real xas(3,nmax,0:1,lmax)       !array to store coordinates
      character seqXs(0:nmax,lmax,0:1) !array to store sequence in numeric code
      character*3 ssXs(nmax,lmax,0:1)  !array to store sequence in three-letter code
      integer mmXs(nmax,lmax,0:1)      !array to store residue numbers
      integer iconf
 ...
 101  read(10,104,end=102) s
     ...
     &           eq.'  CA')then
               if(s(17:17).eq.' '.or.s(17:17).eq.'A')then
                  ...
                  read(s,9000)du,aanam,du,mmXs(ires,iconf,iM),du,
     $                 xas(1,ires,iM,iconf),xas(2,ires,iM,iconf),
     $                 xas(3,ires,iM,iconf) 
                  do j=-1,20    !residue type in numeric code
                     if(aanam.eq.aa(j))seqXs(ires,iconf,iM)=slc(j)
                  enddo
                  ssXs(ires,iconf,iM)=aanam !residue type in three-letter code
               endif
            endif
         endif
      goto 101
