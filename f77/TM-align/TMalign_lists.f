ccccc 
c     pgf77 -Mextend -O -s -fast -Wl,-static -o TMalign TMalign.f
c
c     TM-align version 0.1. A small bug of two-point superposition was 
c     fixed at June 1, 2005, compared with previous version.
c
c     This program is to identify the best alignment of two protein 
c     structures to give the best TM-score. By default, TM-score is 
c     normalized by the second protein. The program can be freely 
c     copied or modified or redistributed.
c
c     Reference:
...
c     (For comments, please email to: zhang6@buffalo.edu)
      ...
      PARAMETER(lmax=1001) !maximum number of model or native structures

      common/buff/cs,buffer,minibuf
      integer cs !current size of "buffer"
      character*10000000 buffer !10Mb maximum buffer size
      character*512 minibuf

      COMMON/BACKBONE/XA(3,nmax,0:1)
      common/dpc/score(nmax,nmax),gap_open,invmap(nmax)
      common/alignrst/invmap0(nmax)
      common/length/nseq1(lmax),nseq2(lmax)
      common/d0/d0,anseq,d0_fix
      common/d0min/d0_min
      common/d00/d00,d002

      character*200 models(lmax),nats(lmax),structDir,listA,listB,outfile

      character*512 fnam,pdb(100),outname
      character*3 aa(-1:20),aanam,ss1(nmax,lmax),ss2(nmax,lmax)
      character*100 s,du
      character seq1(0:nmax,lmax),seq2(0:nmax,lmax)
      character aseq1(nmax2),aseq2(nmax2),aseq3(nmax2)

      real xyzmodel(3,nmax,lmax), xyznat(3,nmax,lmax)
      ...
      common/d8/d8
      common/m_/m_out,m_fix,m_ave,m_d0_min,m_d0
      common/colors/n8_al_5,m1_5(nmax),m2_5(nmax),nrgbs(3,nmax)
...
ccc   
     ...
     $     'H','F','Y','W','C'/
      ...
      if(fnam.eq.' '.or.fnam.eq.'?'.or.fnam.eq.'-h')then
         write(*,*)'TM-alignn ''structure'' to ''target'''
         write(*,*)'  (By default, TM-score is normalized by the ',
     $        'length of ''target.pdb'')'
         write(*,*)
         write(*,*)'TM-align_lists [options]:'
         write(*,*)
         write(*,*)' -m model file'
         write(*,*)' -n native pdb file'
         write(*,*)' -o superposition pdb file'
         write(*,*)' -e directory containing pdb files'
         write(*,*)' -k list of "models" (basename file)'//
     $        ',instead of -m option'
         write(*,*)' -l list of "natives" (basename file)'//
     $        ',instead of -n option'
         write(*,*)' -x output file (default: standard output)'
         write(*,*)' -L specified normalization length'
         write(*,*)' -a if you want score normalized by the ',
     $        'average length of two structures:'
         write(*,*)' -dmin if you want to set a minimum cutoff for d0'//
     $        '(default 0.5) ',
         goto 9999
      endif
...
c     number of arguments passed (separated by one (or more) spaces)
      narg=iargc()

ccccc parse command line arguments
      i=0
      nmodels=0
      nnats=0
 115  continue
      ...
      if(fnam.eq.'-m')then
         i=i+1
         read(models(1),*) fnam(1:lnblnk(fnam)) !load model file name to memory
         nmodels=1
      elseif(fnam.eq.'-n')then
         i=i+1
         read(nats(1),*)fnam(1:lnblnk(fnam)) !load native file name to memory
         nnats=1
      elseif(fnam.eq.'-e')then
         i=i+1
         call getarg(i,structDir)
      elseif(fnam.eq.'-k')then
         i=i+1
         call getarg(i,listA)
         call loadFileNames(listA,models,nmodels)
      elseif(fnam.eq.'-l')then
         i=i+1
         call getarg(i,listB)
         call loadFileNames(listB,nats,nnats)
      elseif(fnam.eq.'-x')then
         i=i+1
         call getarg(i,outfile)
         open(unit=6,file=outfile,access='append',status='unknown') !redirect to standard output
      elseif(fnam.eq.'-o')then
         m_out=1
         i=i+1
         call getarg(i,outname)
c     change both L_all and d0
      elseif(fnam.eq.'-L')then  
         m_fix=1
         i=i+1
         call getarg(i,fnam)
c     TM-score normalized by passed length L_fix
         read(fnam,*)L_fix
      elseif(fnam.eq.'-dmin')then
         m_d0_min=1
         i=i+1
         call getarg(i,fnam)
c     minimum d0 cutoff that we pass, useful for very short proteins
...
c     fixed d0 cutoff
         read(fnam,*)d0_fix
      elseif(fnam.eq.'-a')then
c     we'll use the average sequence length
         m_ave=1
         i=i+1
c     we're out of options, thus the next two arguments are the names
c     first of the template, then of the target
      endif
c     continure reading arguments until we are done reading
      if(i.lt.narg)goto 115

c     read data from models  
      isD=lnblnk(structDir) !number of characters
      do imodel=1,nmodels
         j=lnblnk(models(imodel))
         write(fnam,'a')structDir(1:isD)//'/'//models(imodel)(1:j) !absolute file name
         nseq1(imodel)=readPDBfile(fnam,xyzmodel,imodel)
      enddo

c     read data from natives, if we are indeed passing any native files
      do inat=1,nnats
         j=lnblnk(nats(inat))
         write(fnam,'a')structDir(1:isD)//'/'//nats(inat)(1:j) !absolute file name
         nseq2(inat)=readPDBfile(fnam,xyznat,inat)
      enddo

c     Paramters
      d0_min=0.5
      if(m_d0_min.eq.1)then !if we passed a minimum d0 cutoff
         d0_min=d0_min_input    
      endif

      if(nnats.eq.0)then !TMalign among models
         do imodel=1,nmodels-1
c         do imodel=1,nmodels
            imS=lnblnk(models(imodel)) !size of models(imodel) file name
            do jmodel=imodel+1,nmodels
c            do jmodel=1,nmodels
               call doTMalg(xyzmodel,imodel,xyzmodel,jmodel)
               jmS=lnblnk(models(jmodel))
               write(minibuf,'axaxf5.3')models(imodel)(1:imS),
     $              models(jmodel)(1:jmS),score_max
               call addToBuffer(minibuf)
            enddo
         enddo
      else !TMscore between models and nats
         do imodel=1,nmodels
            imS=lnblnk(models(imodel)) !size of models(imodel) file name
            do inat=1,nnats
               call doTMalg(xyzmodel,imodel,xyznat,inat)
               inS=lnblnk(nats(inat))
               write(minibuf,'axaxf5.3')models(imodel)(1:imS),
     $              nats(inat)(1:inS),score_max
               call addToBuffer(minibuf)
            enddo
         enddo
      endif
      write(6,'a') buffer(1:cs-1) !flush buffer except last end-of-line character
      write(buffer,'a')''       !"empty" buffer
      
      call exit(0) !exit here. We will deal with the rest of the program later

      subroutine doTMalg(xyzM,iM,xyzN,iN)
      PARAMETER(nmax=1000) !maximum number of residues
      PARAMETER(lmax=1001) !maximum number of model or native structures
      real xyzM(3,nmax,lmax) !coordinates for models
      real xyzN(3,nmax,lmax) !coordinates for models/natives
      integer iM,iN !indexes of model and native
      
      common/length/nseq1(lmax),nseq2(lmax) !length of models and natives
      common/d0/d0,anseq,d0_fix
      common/d0min/d0_min
      common/d00/d00,d002
      common/d8/d8
      ...
      anseq_min=min(nseq1(iM),nseq2(iN))
      anseq=anseq_min !length for defining TMscore in search based on smaller sequence       
      ...
      if(m_d0.eq.1)d0=d0_fix !if we passed a particular d0, use it
...
ccccc for output summary
     ...
     $     's. 2005 33, 2302-9  *'
      write(*,*)'* Comments on the program, please email to: zhang6@bu',
     $     'ffalo.edu           *'
      write(*,*)'*****************************************************',
     $     '*********************'
      write(*,*)
 ...
 103  format('Aligned length=',I4,', RMSD=',f6.2,', TM-score=',f7.5,
     &     ', ID=',f5.3)
      write(*,*)

ccccc for output superposition
      if(m_out.eq.1)then
 1237    format('ATOM  ',i5,'  CA  ',A3,A2,I4,4X,3F8.3)
 1238    format('TER')
 1239    format('CONECT',I5,I5)
 900     format(A)
 901     format('select atomno=',I4)
 902     format('color [',I3,',',I3,',',I3,']')
c     pdb1.aln + pdb2.aln
         OPEN(unit=7,file=outname,status='unknown') 
ccccc script:
         write(7,900)'load inline\nselect all\nset connect false'
         write(7,900)'backbone\ncolor white'
         write(7,900)'select :A\nbackbone 80\nselect :B\nbackbone 30'
c     color the aligned residues according to scale from blue to red
         call blue2redRasmol(n8_al,m1,m2,xtm1,ytm1,ztm1,xtm2,ytm2,ztm2)
         do i=1,n8_al_5
            write(7,901)m1_5(i)
            write(7,902)nrgbs(1,i),nrgbs(2,i),nrgbs(3,i)
            write(7,901)5000+m2_5(i)
            write(7,902)nrgbs(1,i),nrgbs(2,i),nrgbs(3,i)
         enddo
         write(7,900)'select all\nexit'

ccccc   chain1:
         call superimpose(n8_al,m1,xtm1,ytm1,ztm1) !adapt xa(j,i,0) to xa(j,i,1) by global u,t
         do i=1,nseq1
            write(7,1237)i,ss1(i),'A',i,xa(1,i,0),xa(2,i,0),xa(3,i,0)
         enddo
c     TER
         write(7,1238)          
         do i=2,nseq1
...
ccccc   chain2:
         do i=1,nseq2
            write(7,1237)5000+i,ss2(i),'B',5000+i,
     &                   xa(1,i,1),xa(2,i,1),xa(3,i,1)
         enddo
         write(7,1238)
         do i=2,nseq
            write(7,1239)4999+i,5000+i
         enddo
      endif
ccccc output finished
...
ccccc output aligned sequences
 ...
 9999 END
ccccc program compares finished
...
c     secondary structure and rmsd-derived score
      ...
      return
      end !end of subroutine super_align
...
c     position "i"
      ...
      enddo
      do i=1,nseq2
c     initialize secondary structure to coil
         jsec(i)=1
         j1=i-2
         j2=i-1
         j3=i
...
ccccc get_score1 finished
      ...
      subroutine blue2redRasmol(n8_al,m1,m2,x1,y1,z1,x2,y2,z2)
      PARAMETER(nmax=1000)
      common/colors/n8_al_5,m1_5(nmax),m2_5(nmax),nrgbs(3,nmax)
      dimension m1(nmax),m2(nmax)
      dimension x1(nmax),y1(nmax),z1(nmax)
      dimension x2(nmax),y2(nmax),z2(nmax)
c     calculate first number of aligned residues below 5.0A
      n8_al_5=0
      do i=1,n8_al
         dis2=sqrt((x1(i)-x2(i))**2+
     $        (y1(i)-y2(i))**2+(z1(i)-z2(i))**2)
         if(dis2.le.5)then
            n8_al_5=n8_al_5+1
            m1_5(n8_al_5)=m1(i) !amino acid number
            m2_5(n8_al_5)=m2(i) !amino acid number
         endif
      enddo
c     assign now color to each aligned amino acid
      do i=1,n8_al_5
         nn=int((i*30.0)/n8_al_5)
         x=float(nn)/30
c     nrgbs(1,i) acts as "pointer" to the i-th 3D-vector
         call blue2red(x,nrgbs(1,i))
      enddo
      return
      end
cccc blue2redRasmol finished


      subroutine blue2red(x,nrgb)
      dimension nrgb(3)
      integer interval,residue,r,g,b
      ...
      x=x*2.0/3
      interval=int(x*6.0)
      residue=int(255* (x*6.0-float(interval) ))
      if(interval.eq.0)then
         r=255
         g=residue
         b=0
      else if(interval.eq.1)then
         r=255-residue
         g=255
         b=0
         ...
         g=255-residue
         b=255
      else if(interval.eq.4)then
         r=residue
         g=0
         b=255
      else if(interval.eq.5)then
         r=255
         g=0
         b=255-residue
      else if(interval.eq.6)then
         r=255
         g=0
         b=0
      endif
...
cccc blue2red finished

c     modify xa(i,j,0) according to stored values of u and t
      subroutine superimpose(n8_al,m1,x1,y1,z1)
      PARAMETER(nmax=1000)
      COMMON/BACKBONE/XA(3,nmax,0:1)
      common/length/nseq1,nseq2
      dimension m1(nmax)
      ...
      data w /nmax*1.0/
c     record initial positions in temporary array
      ...
      do i=1,n8_al
         ...
         r_2(3,i)=z1(i)
      enddo
c     find out rotation and translation
      call u3b(w,r_1,r_2,n8_al,1,rms,u,t,ier)
c      write(*,*)u(1,1),u(2,1),u(3,1)
c      write(*,*)u(1,2),u(2,2),u(3,2)
c      write(*,*)u(1,3),u(2,3),u(3,3)
c      write(*,*)t(1),t(2),t(3)
c     now change all residues of seq1 according to u and t
      do i=1,nseq1
         xx=t(1)+u(1,1)*xa(1,i,0)+u(1,2)*xa(2,i,0)+u(1,3)*xa(3,i,0)
         yy=t(2)+u(2,1)*xa(1,i,0)+u(2,2)*xa(2,i,0)+u(2,3)*xa(3,i,0)
         zz=t(3)+u(3,1)*xa(1,i,0)+u(3,2)*xa(2,i,0)+u(3,3)*xa(3,i,0)         
...
c        TM-score superposition.
c     
c     We pass global u and t in order to store final rotation and traslation
c     normal TM-score:
...
c     
 ...
 101  close(10)
      ...
      function readPDBfile(fnam,xyz,seqX,ssX,iconf)
      PARAMETER(nmax=1000) !maximum number of residues
      PARAMETER(lmax=1001) !maximum number of model or native structures
      character*200 fnam          !pdb file name
      real xyz(3,nmax,lmax)       !array to store coordinates
      character seqX(0:nmax,lmax) !array to store sequence in numeric code
      character*3 ssX(nmax,lmax)  !array to store sequence in three-letter code

      character*100 s,du
      character*3 aanam
      integer readPDBfile,iconf,ires,jres
      dimension mm1(nmax)
      character*3 aa(-1:20)
      data aa/ 'BCK','GLY','ALA','SER','CYS','VAL','THR','ILE',
     $     'PRO','MET','ASP','ASN','LEU',
...
9000 format(A17,A3,A2,i4,A4,3F8.3)
      ires=0
      open(unit=10,file=fnam,status='old')
 101  read(10,104,end=1010) s
         if(s(1:3).eq.'TER') goto 102
         if(s(1:4).eq.'ATOM')then
            if(s(13:16).eq.'CA  '.or.s(13:16).eq.' CA '.or.s(13:16).
     &           eq.'  CA')then
            ires=ires+1
            if(ires.ge.nmax)goto 102 !too long sequence. Consider only first nmax residues
            read(s,9000)du,aanam,du,mm1(i),du,
     $           xyz(1,ires,iconf),xyz(2,ires,iconf),xyz(3,ires,iconf) 
            do j=-1,20 !residue type in numeric code
               if(aanam.eq.aa(j))seqX(ires,iconf)=slc(j)
            enddo
            ss1(ires,iconf)=aanam !residue type in three-letter code
         endif
      endif   
      goto 101
 102  continue
