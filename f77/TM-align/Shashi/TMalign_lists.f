*************************************************************************
*     pgf77 -g -Mbounds -Mchkfpstk -Mextend  -Wl,-static -o TMalign_lists TMalign_lists.f
*     pgf77 -Mextend -O -s -fast -Wl,-static -o TMalign_lists TMalign_lists.f
*
...
c     variables introduced for alignments among more than two proteins
      ...
      character*512 fnam,pdb(0:1),outname
...
c         write(7,900)'color [255,20,147]'
         write(7,900)'color [255,20,237]'
         write(7,900)'wireframe .45'
         write(7,900)'select none'
         write(7,900)'select atomno>2000'
         write(7,900)'color [100,149,237]'
...
**************************************************************
     ...
     &            ,jali(nmax,top) ! for alignment
      ...
      if(aL.lt.170) then
        idel=aL/2.5
        minoverlap=max(10,idel)
        if(fr.lt.0.16)then
c         istep=(minoverlap/3)-2
          istep=3
      ...
      temp=0    ! no. of alignment to be considered
      temp1=0   ! no. of alignment for dpout
...
cccc the best alignment resulting from progression of best GL alignment.
       ...
       call get_score
        if(TM.gt.TMlcmax) then
         TMlcmax=TM
          do j=1,nseq2
           invmapB(j)=invmap(j)
          enddo
        endif
       call iter(20,TMlcmax,0)   ! iteration step acts on invmap(i) and returns invmap_i(i)

       do j=1,nseq2
        invmapB(j)=invmap_i(j) ! keep invmap(j) for SS alignment and record it
        invmap(j)=invmap_i(j)
       enddo
...
cccc and second alignment is from best of all alignment
       ...
       do istep=1,iRUN ! 1-best alignment, 2-best GL alignment,3- best DP alignment
           ...
           call get_score
             ...
             elseif(istep.eq.2)then
                if(TM.gt.TMlcmax) then
                 TMlcmax=TM
                 do jj=1,nseq2
                  invmapB(jj)=invmap(jj)
                 enddo
                endif
               call iter(20,TMlcmax,1)
                do jj=1,nseq2
                  invmapB(jj)=invmap_i(jj)
                  invmap(jj)=invmap_i(jj)
                enddo
             elseif(istep.eq.3) then
...
cccc Increasing score matrix using SS alignment 
       ...
       imax=0 ! if imax =1 -> invmapA, imax=2->invmapB, imax=3->map
       bmax=0
       if(b2TMmax.gt.bmax)then
        imax=1
        bmax=b2TMmax
       endif
       if(TMlcmax.gt.bmax)then
        imax=2
        bmax=TMlcmax
       endif
...
c     load contents of file "list" into memory (files)
 ...
 102  close(10)
      readPDBfile=ires
      return
 666  write(6,'a')'ERROR: reading',fnam
      call exit(1)
      end
