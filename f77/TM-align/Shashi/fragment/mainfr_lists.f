...
************************************************************************  
      ...
      character*512 pdb(2),outfile,outname,buffer,fnam,pdbn(npr)
...
cc so, read all the pdbs and iterate to compare list of pdbs.
   
c       if (ilist.eq.1) then
c         ii=10
c         do while (list(ii:1).eq.' ')
c             ii=ii-1
c         enddo
c
c         open (unit=1,file=list(1:ii),status='unknown')
c         nopdb=1
c         do while(.true.)
c          read(1,*,end=11)pdbid
c          call readpdb(pdbid,nopdb,nlen,aanam1,aanam2,xyz)
c          pdbn(nopdb)=pdbid
c          length(nopdb)=nlen
c          nopdb=nopdb+1
c         enddo
c         close (1)
c   11    continue
c         call readpdb (pdb(1),nopdb,nlen,aanam1,aanam2,xyz)
c         length(nopdb)=nlen
c         pdbn(nopdb)=pdb(1)
c       else
c           do i=1,2
c            call readpdb(pdb(i),i,nlen,aanam1,aanam2,xyz)
c            length(i)=nlen
c           enddo
c        nopdb=2
c       endif
c
c
ccc initialize the coordinates xa() for superposition
c
c      do 1166 ipdb=1,nopdb-1     ! main cycle
c    
c       if (ilist.eq.1) then
c          i=0
c          do j=1,length(ipdb)
c           do k=1,3
c            xa(k,j,i)=xyz(k,j,ipdb)   
c           enddo
c            seqn(j,i)=seqa(j,ipdb)
c            resn(j,i)=resa(j,ipdb)
c            ires(j,i)=ires1(j,ipdb)
c          enddo
c          
c          if (ipdb.eq.1) then ! read second protein only once
c            i=1
c            jj=nopdb
c            do j=1,length(jj)
c             do k=1,3
c                xa(k,j,i)=xyz(k,j,jj)   
c             enddo
c                seqn(j,i)=seqa(j,jj)
c                resn(j,i)=resa(j,jj)
c                ires(j,i)=ires1(j,jj)
c            enddo
c            nseq2=length(jj)
c            pdb(2)=pdbn(nopdb)
c          endif
...
c         do j=1,length(ik)
c            do k=1,3
c               xa(k,j,i)=xyz(k,j,ik)   
c            enddo
c               seqn(j,i)=seqa(j,ik)
c               resn(j,i)=resa(j,ik)
c               ires(j,i)=ires1(j,ik)
c         enddo
c        enddo
c        nseq1=length(1)
c        nseq2=length(2)
...
cccc reading pdb file
       subroutine readpdb (filen,np,nlen,aanam1,aanam2,xyz) ! filen: file name, np: 0 or 1
        ...
        character*512 buffer,fnam,filen
        ...
        common /pdbinfo/ ires1(maxres,npr),resa,seqa
         ...
         nii=10
         do while (filen(nii:1).eq.' ')
             nii=nii-1
         enddo

          open (unit=10,file=filen(1:nii))
          i=0
          do while(.true.) 
