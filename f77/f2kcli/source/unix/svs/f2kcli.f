...
C
C Platform    : Unix
C Compiler    : SVS (Silicon Valley Software) Fortran 77
C               Unlike the majority of Unix Fortran 77's, the
C               implementation of IARGC/GETARG included with this
C               compiler treats the program name as argument
C               number 1. The first true program argument is
C               thus argument number two, and so on.
C To compile  : fortran -c f2kcli.f
