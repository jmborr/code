...
!
! Platform    : HP/UX
! Compiler    : HP Fortran 90 (early versions - see below)
!               It's our understanding that, like some versions
!               of HP f77, early versions of HP f90 treat the
!               program name as argument number 1. The first
!               true program argument is thus argument number
!               two, and so on. (This is at odds with the more
!               widely adopted Unix convention where argument
!               zero returns the program name and the first true
!               argument is number one.) Unfortunately, we've not
!               had the opportunity to test such a version of
!               HP f90. However, this implementation of F2KCLI
!               is designed to cope with such behaviour, where
!               it exists. By v 2.5.3 (and possibly earlier) the
!               more common zero-based argument counting convention
!               appears to have been adopted by HP f90. If you
!               are using such a compiler version, then use the
!               generic Unix f90 F2KCLI code in source/unix/f90
!               instead.
...
!
      IF (PRESENT(VALUE)) CALL GETARG(NUMBER+1,VALUE)
!
