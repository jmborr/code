...
C
C Platform    : PRIMOS
C Compiler    : Salford FTN77
C To compile  : FTN77 -INTL -LOGL F2KCLI.F77
...
C
      ...
      INTEGER        STATUS
C
      CHARACTER*255  CMNAM,ARGSTR
      CHARACTER*2000 CMDLINE
      LOGICAL        GETCMD
C
      SAVE           CMDLINE
C
...
C
      IF (GETCMD) THEN
          ...
          CMDLINE = ' '
C
C Construct command line from each token
C
  100     ARGSTR = CMNAM()
          IF (LEN_TRIM(ARGSTR).NE.0) THEN
              CMDLINE(IPOS:) = ARGSTR
              IPOS = LEN_TRIM(CMDLINE) + 2
              IF (IPOS.LE.LEN(CMDLINE)) GOTO 100
          ENDIF
          GETCMD = .FALSE.
      ENDIF
      COMMAND = CMDLINE
...
C
      ...
      CHARACTER*2000 ARGSTR
C
C If the argument number is < 1, return an error code of 1.
C
      IF (NUMBER.LT.1) THEN
          VALUE  = ' '
          LENGTH = 0
