...
C
C Platform    : DOS (Phar Lap 32-bit DOS extender)
C Compiler    : SVS Fortran 2.8
C To compile  : svs -c -q -d f2kcli.for
...
C
      DATA          GETCMD/.TRUE./
C
C SVS Fortran GETARG does not recognise quoted strings,
C so have to reassemble command line by repeated calls
C to GETARG instead of relying on IARGC
C
      IF (GETCMD) THEN
          NARG = IARGC()
          IF (NARG.GT.1) THEN
...
C Note : Unlike most DOS implementations, the SVS compiler will
C only return the entered command name rather than its full path
C
      IF (NUMBER.EQ.0) THEN
          CALL GETARG(1,VALUE)
