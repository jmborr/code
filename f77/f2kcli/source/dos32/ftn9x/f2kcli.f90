...
!
! Platform    : DOS/DBOS and Windows 3.1/9x/Me+WDBOS
! Compiler    : Salford FTN90/DBOS
!               Salford FTN95/DBOS
! To compile  : ftn90 f2kcli.f90
!               ftn95 f2kcli.f90
...
!
      ...
      INTEGER         , INTENT(OUT), OPTIONAL :: STATUS
!
      INTEGER            :: IPOS,ISTART,IEND,NARGS,IPOS1,LENARG
      CHARACTER(LEN=127) :: ARGSTR
!
...
!
      ...
      END IF
!
! Use a run time library routine to get executable name
!
      IF (NUMBER == 0) THEN
          IF (PRESENT(VALUE)) THEN
              CALL GET_PROGRAM_NAME@(VALUE)
              IF (PRESENT(LENGTH)) LENGTH = LEN_TRIM(VALUE)
          ELSE
              IF (PRESENT(LENGTH)) LENGTH = 0
          END IF
          IF (PRESENT(STATUS)) STATUS = 0
