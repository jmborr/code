...
C Compiler    : Microsoft Fortran PowerStation 1.0
C To compile  : fl32 /c f2kcli.for
...
C
      IF (GETCMD) THEN
          ...
          IF (NARG2.GT.0) THEN
              IPOS = 1
              DO 100 IARG2 = 1,NARG2
                CALL GETARG(IARG2,ARGSTR(IPOS:),ISTAT2)
...
C   arguments.
C
      COMMAND_ARGUMENT_COUNT = NARGS() - 1
      RETURN
      END
C
...
C
      IF (NUMBER.LT.0) THEN
          ...
          STATUS = 1
          RETURN
      ELSE IF (NUMBER.GT.NARGS()-1) THEN
          VALUE  = ' '
          LENGTH = 0
          STATUS = 2
          RETURN
...
C
      IARG2  = NUMBER
      CALL GETARG(IARG2,VALUE,ISTAT2)
