...
C
C Platform    : DOS (32-bit DOS extender)
C Compiler    : Microway NDP Fortran
C To compile  : mf486 -c -f1 f2kcli.for
...
C
      DATA          GETCMD/.TRUE./
C
C Microway provides Unix-style command line access, so we must
...
C
C Get the argument
C Note : Unlike most DOS implementations, the Microway compiler will
C        only return the entered command name rather than its full
C        path, when argument zero is specified
C
