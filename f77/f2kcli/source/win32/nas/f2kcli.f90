...
!
! Platform    : Win32
! Compiler    : NA FortranPlus
!               Due to the somewhat offbeat nature of the NA compiler,
!               this implementation of F2KCLI relies on the vendor-supplied
!               iargc/getarg routines, rather than the generic API-based
!               interface which is used with all other Win32 compilers.
!               This has two consequences:
!               (a) Interrogating argument zero will return inconsistent
!                   results under Windows 9x/Me and Windows NT/2000, due
!                   to NA's implementation of getarg(0,arg). 9x/Me will
!                   return the applications' full pathname. NT/2K will
!                   return the entered command name,
!               (b) The command line returned by GET_COMMAND is
!                   reconstructed from the values returned by
!                   getarg and is therefore not necessarily the
!                   exact command line used to invoke the program.
!               All other Win32 implementations of F2KCLI do not suffer
...
!
      USE NAS_SYSTEM ! Required by NA compiler
!
