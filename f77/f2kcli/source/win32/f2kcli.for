...
C
C Platform    : Win32
C Compiler    : Win32 Fortran 77 compilers including:
C               - Salford FTN77/Win32
C               - Watcom Fortran 77
C               - GNU g77 (mingw32)
C               - Absoft Fortran 77
C To compile  : ftn77 f2kcli            (Salford)
C               wfc386 f2kcli           (Watcom)
C               g77 -c f2kcli.for       (GNU)
C               f77 -c -N109 f2kcli.for (Absoft)
...
C
      ...
      INTEGER        STATUS
C
      INTEGER        IPOS,ISTART,IEND,NARGS,IPOS1
      CHARACTER*2000 ARGSTR
C
*$pragma aux f2kgetcl  "*_"
*$pragma aux f2kgetexe "*_"
C
...
C
C If argument zero requested, get the executable name via the
C GetModuleFilename API function. The name at the start of the
C command line is not consistent across Windows 9x/Me and NT/2K.
C The former give the more useful full pathname whereas the latter
C only give the entered name. GetModuleFilename gives the full
C pathname regardless of Windows version.
C
...
C
      ...
      IF (NULL.GT.0) ARGSTR(NULL:) = ' '
C
C Find required command line argument - skip executable name
C
      NARGS = -1
      IPOS  = 1
  100 CALL F2KSUBSTR(ARGSTR(IPOS:),ISTART,IEND)
