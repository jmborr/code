...
C
C Platform    : VMS (VAX or Alpha)
C Compiler    : Digital/Compaq Fortran 77
C To compile  : FORT F2KCLI
C Implementer : Lawson B. Wakefield, I.S.S. Ltd.
C Date        : February 2001
C Updated     : April 2001 (arg 0 returned)
C
...
C
      DATA           NARGS/-1/
C
C Have we already retrieved the command line ?
C
      IF (NARGS.EQ.-1) THEN
C
C Get the command line into local string
C
          ARGSTR = ' '
          ISTAT = LIB$GET_FOREIGN(ARGSTR)
C
C If there was an error, assume no command line
C
          IF (ISTAT.NE.1) THEN
              NARGS = 0
          ELSE
...
C
      ...
      CHARACTER*2000 ARGSTR
C
C If the argument number is less than zero return an error code of 1.
C
      IF (NUMBER.LT.0) THEN
          VALUE  = ' '
          LENGTH = 0
          ...
          RETURN
C
C Get image name (Thanks to Luis Suarez of the US EPA for pointing
C out the availability of LIB$GETJPI to perform this task)
C
      ELSE IF (NUMBER.EQ.0) THEN
          ISTAT = LIB$GETJPI(JPI$_IMAGNAME,,,,ARGSTR,LENGTH)
          IF (ISTAT.EQ.1) THEN
              VALUE  = ARGSTR
              STATUS = 0
          ELSE
