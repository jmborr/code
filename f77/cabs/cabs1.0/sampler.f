...
c
	...
	COMMON /RES/ EREST, MRES(ndim),KRES(ndim,50)
	COMMON /RCN/ ERESTA, MRESA(ndim),KRESA(ndim,80),arca(ndim,ndim)
	COMMON /short2/ erestsum, arestsum, csr(ndim,2),arcs(ndim,ndim)
	DIMENSION csre(0:19,0:19,2)	
	COMMON /SG/ GX(800,800,0:19),GY(800,800,0:19),GZ(800,800,0:19)		
	DIMENSION irest(4000),jrest(4000),iresta(4000),jresta(4000)
	...
	dimension icao(ndim),icab(ndim), aga(ndim,-12:16)
...
c	reading of "average" r15 interactions, k-type of aminoacid
	read(12,*) (bsrr(i,j,k),k=1,8)
	read(12,*) (bsrr(i,j,k),k=9,16)
	enddo
	enddo
...
c	structure elements H, or E  (read in two following segments)
	...
	do j=0,19
	read(12,*)
	read(12,*) (bsrh(i,j,k),k=1,8)
	read(12,*) (bsrh(i,j,k),k=9,16)
	enddo
	enddo	
	...
	do j=0,19
	read(12,*)
	read(12,*) (bsre(i,j,k),k=1,8)
	read(12,*) (bsre(i,j,k),k=9,16)
	enddo
	enddo	
...
c	d1+d2 the width of the squqre well pairwise potential
	...
	if(i.gt.5)d22=d22+0.25
	if(j.gt.5)d22=d22+0.25
...
c
	...
	is=sec(i)
	do j=2,lenf1
	if(iabs(i-j).gt.2 .and. iabs(i-j).ne.4) then
	EHBIJ(i,j)=EHBOND
	js=sec(j)
...
c
	...
	if(sec(i).eq.2) then
	asr(i,k)=(asr(i,k)+ASRO*asrh(seq(i+1),seq(i+2),k))/2.0
	endif
	endif
	if(sec(i+1).eq.4.AND.sec(i+2).eq.4.AND.sec(i+3).eq.4)then
	if(sec(i).eq.4) then
	asr(i,k)=(asr(i,k)+ASRO*asre(seq(i+1),seq(i+2),k))/2.0
	endif
	endif
	enddo
	enddo
...
c	AVERAGING THE SHORT RANGE INTERACTIONS
...
c 	do i=1,lenf-3
c	do k=-11,11
c	asr(i,k)=0.5*aga(i,k)+0.25*aga(i,k-1)+0.25*aga(i,k+1)
c	enddo
c	enddo	
...
c
	OPEN(UNIT=11,FILE='R13',   STATUS='OLD')
	do i=0,19
	do j=0,19
	read(11,*)
	read(11,*) (csre(i,j,k),k=1,2)
	enddo
	enddo
	
	do i=2,lenf-3
	do k=1,2
	csr(i,k)=2.0*ASRO*csre(seq(i),seq(i+2),k)
	enddo
	enddo
	csr(1,1)=0.0
	csr(1,2)=0.0
...
c	
	...
	if(sec(i+1).eq.2.AND.sec(i+2).eq.2.AND.sec(i+3).eq.2)then
	bsr(i,k)=(bsr(i,k)+ASRO*bsrh(seq(i+1),seq(i+3),k))/2.0
	endif
	if(sec(i+1).eq.4.AND.sec(i+2).eq.4.AND.sec(i+3).eq.4)then
	bsr(i,k)=(bsr(i,k)+ASRO*bsre(seq(i+1),seq(i+3),k))/2.0
	endif
	enddo
	enddo
...
c
	...
	if(NREST.gt.0 ) THEN
	do k=1, NREST
	read(5,*) IREST(K),JREST(k),junki,arrr
	i=irest(k)+1
	j=jrest(k)+1
	ii=MRES(i)+1
	jj=MRES(j)+1
...
c
	...
	do k=1, NRESTAA
	read(5,*) IRESTA(K),JRESTA(k), junki, arrr
	i=iresta(k)+1
	j=jresta(k)+1
	NRESTA=NRESTA+1
	IRESTA(NRESTA)=i
...
c	dtem distance between replicas	
	...
	ATREP(i)=atemp
	atemp=atemp+dtem*((float(i-1+REPLICAS))/float(REPLICAS))**3
...
c
	...
	betan=1.0/ATREP(itemp+1)
	delta=(betan-betam)*(EREPLICA(itemp)-EREPLICA(itemp+1))
		...
		if(exp(-delta).gt.arand(ise)) then
			...
			yt(i)=yreps(i,itemp)
			zt(i)=zreps(i,itemp)
			xreps(i,itemp)=xreps(i,itemp+1)
			yreps(i,itemp)=yreps(i,itemp+1)
			zreps(i,itemp)=zreps(i,itemp+1)
			xreps(i,itemp+1)=xt(i)
			yreps(i,itemp+1)=yt(i)
			zreps(i,itemp+1)=zt(i)
			enddo
			SWAPS(itemp)=SWAPS(itemp)+1
			SWAPS(itemp+1)=SWAPS(itemp+1)+1			
			attt=EREPLICA(itemp)
			EREPLICA(itemp)=EREPLICA(itemp+1)
			EREPLICA(itemp+1)=attt
			inreps=inreps+1	
		endif
	ENDDO
	ENDIF
...
c		
	energ=energ+ardscal*(ICNT/float(SUMCT)-acorder)**2   
     *	+cstr*(float(SUMCT)-contt)**2
...
c	Move rejected - restore the initial state of the chain		
 ...
 713	format(12I5)
	...
	if(icycle.gt.1) then
	axr=float(inreps)/((icycle-1)*(replicas-1))
	else
	axr=0.0
	endif
     ...
     * 	kkk, SUMCT 
 714	FORMAT(i5, 4F7.4,f10.2,4f5.1,f6.3, i4,i4)	
...
C
 ...
 8765	format(3i4,F8.0,i4)
 	enddo
 	write(6,*) 'satisfied ',nsat,'   per ',NREST
 	ENDIF	 	
...
c
	...
	if(ir.lt.400) then
		...
		if(ir.lt.50) EHB=EHB+((25.0/(ir+1))-0.5)*EREP	
...
c	side group-Ca distance
		...
		if(aks.lt.54.0) then
		if(aks.lt.36.0) aks=36.0
		EHB=EHB+((36.0/aks)-0.6666)*EREP
		endif
		endif
...
c
c	THIS IS A PSEUDOELECTROSTATICS
c
	if(icg(k)*icg(i).ne.0) then
	EHB=EHB+alp(in,im,ib,kb)*app(i,k,ib,kb)/ar
	endif
...
c
	if(icg(k)*icg(i).ne.0) then
	EHB=EHB+ala(in,im,ib,kb)*apa(i,k,ib,kb)/ar
	endif
					endif
			else
			EHB=EHB+EREP
...
c
       	if(icg(k)*icg(i).ne.0) then
       	EHB=EHB+alm(in,im,ib,kb)*apm(i,k,ib,kb)/ar
       	endif
					endif
			else
			EHB=EHB+EREP
...
c	geometry
	...
	ENDIF
		...
		fact1=(hx*ax+hy*ay+hz*az)**2/100.0
		fact2=(fx*ax+fy*ay+fz*az)**2/100.0
		if(fact1.lt.0.125) fact1=0.125
		if(fact2.lt.0.125) fact2=0.125
		EHB=EHB-BAT*(2.0 -fact1-fact2) 
	...
	if(arrr.lt.98.0) then
	arrr=max(arrr,49.0)
	delta=delta+(49.0/arrr)**2-0.25
	endif
	k1=k-1
	i1=i-id
	dx=(px+x(k1)-xi-x(i1))/2.0
...
c
	if(icg(k)*icg(i).ne.0) then
	EHB=EHB+alp(im,in,kb,ib)*app(k,i,kb,ib)/ar
	endif
					endif
...
c
	...
	if(icg(k)*icg(i).ne.0) then
	EHB=EHB+ala(im,in,kb,ib)*apa(k,i,kb,ib)/ar
	endif
					endif
			else
			EHB=EHB+EREP
...
c
	if(icg(k)*icg(i).ne.0) then
	EHB=EHB+alm(im,in,kb,ib)*apm(k,i,kb,ib)/ar
	endif
					endif
			else
			EHB=EHB+EREP
...
c		COOPERATIVITY
 ...
 1002	CONTINUE
	...
	if(istat.lt.0) then	
        b= (ICNT/float(SUMCT)- acorder)**2 
        a= (ICNTO/float(SUMCTO)- acorder)**2 
        d= (float(SUMCT)-contt)**2
        c= (float(SUMCTO)-contt)**2 
...
c              	
        ...
        arc=arca(i,j)
        ar=abs(ar-arc)
			if(ar.lt.1.0) eshort=eshort-eresta 
			if(istat.lt.0) then
        		arold=arold+ar
        		else
        		arnew=arnew+ar
...
c	and the r13 potentials
	kkk=iconf(ii,jj)
	if(kkk.lt.85) then
	ESHORT=ESHORT+csr(i-1,1)
	else
	if(kkk.gt.115) then	
	ESHORT=ESHORT+csr(i-1,2)
	else
	aw=(kkk-85)/20.
	ESHORT=ESHORT+aw*csr(i-1,1) +(1.0-aw)*csr(i-1,2)
	endif
	endif			
      enddo
...
c
	bx=CAX(icam3,icam2)*CAX(icam2,icam1)
	by=CAY(icam3,icam2)*CAY(icam2,icam1)
	bz=CAZ(icam3,icam2)*CAZ(icam2,icam1)
	bbx=CAX(icam1,icam)*CAX(icam,icaj)
	bby=CAY(icam1,icam)*CAY(icam,icaj)
	bbz=CAZ(icam1,icam)*CAZ(icam,icaj)

	IF((bx*bx+by*by+bz*bz   +    bbx*bbx+bby*bby+bbz*bbz).lt.0.0) then
	...
	if(aor4.gt.2.0) aor4=2.0
	ESHORT=ESHORT-ff*ESC2*(2.0-aor4)
	ENDIF
...
c	
	if(iii.lt.130) THEN
				ipp=prod(icam3,icaj)
				if(ipp.gt.0) ESHORT=ESHORT-ff*ESC2
c	a helix (***)		
	if(sec(i+1).ne.4.AND.sec(i+2).ne.4.AND.sec(i+3).ne.4) then
	if(IBIN4.gt.4.AND.IBIN4.lt.8) then
     	if(prod(icam2,icam).lt.0) then
     	if(prod(icam3,icam).gt.0) ESHORT=ESHORT-ff*ESC2-ESC	
	endif
	endif
	endif
...
c	a beta (###)
	...
	aor1=(CAX(icam2,icam1)*CAX(icam1,icam)+CAY(icam2,icam1)	
     *	*CAY(icam1,icam)+CAZ(icam2,icam1)*CAZ(icam1,icam))
     	if(aor1.lt.0.0) then
	aor2=(CAX(icam3,icam2)*CAX(icam2,icam1)+CAY(icam3,icam2)	
     *	*CAY(icam2,icam1)+CAZ(icam3,icam2)*CAZ(icam2,icam1))
	IF(aor2.lt.0.0) then
	aor3=(CAX(icam3,icam2)*CAX(icam1,icam)+CAY(icam3,icam2)	
     *	 *CAY(icam1,icam)+CAZ(icam3,icam2)*CAZ(icam1,icam))
	if(aor3.gt.0.0) ESHORT=ESHORT-ff*ESC2-ESC
	endif			
	endif
	endif
	endif
