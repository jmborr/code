...
c     pgf77 -Mextend -Bstatic -g -Mbounds -Mchkfpstk -o spicker spicker.f
c     Compile for production:
c     pgf77 -Mextend -O3 -Bstatic -o spicker spicker.f
c
...
c 345678901234567890123456789012345678901234567890123456789012345678901234567
      ...
      logical isca !is there CA file to be read ?
...
c     common/template/ contains variables related to the selected template
...
c     lresid(j,i): residue "j" belongs to extended loop "i"
      common/template/isalig,nloop,lbegin,lextent,lextents,lresid
...
c     variables related to loop-derived properties
     ...
     &nrmsdl,nrmsds,lrmsdns
      ...
      character txt1*70,txt2*70,fnam*500,templf*500,pdbid*5
ccc   RMSD:
...
c     initialize optional arguments
      ctarget=0 !selected cluster id. 0 means no selected cluster
c     read command line arguments
      call getarg(1,fnam)
      if(fnam.eq.' '.or.fnam.eq.'?'.or.fnam.eq.'-h') call wellcome()
      
      narg=iargc() !number of command line arguments
      nreq=2 !number of required arguments
      ireq=0 !number of req. args. currently read
      i=0
 99   i=i+1
      call getarg(i,fnam)
      if(fnam.eq.'-a')then
         i=i+1
         call getarg(i,templf)
         ireq=ireq+1
      ...
      if(i.lt.narg) goto 99
      if(ireq.lt.nreq) call wellcome()
...
****  input:
      ...
      isca=.false. !init as no CA file present
      open(3,file='CA',status='old',err=111) !for comparison to native
      isca=.true. 
 111  continue
****  output:
...
**********************************************************************
      ...
      read(1,*)Lch       !sequence lenght
      read(1,*)protein   !PDB id for protein

********************************************************************
c     read exposed surface, initializing variable exp
      call readexpdat(Lch)

c     read template file, initializing isalig,lbegin,lextent,nloop
      call readTemplf(Lch,templf,pdbid)
...
cccccccccccc read native structure cccccccccccccccccccc
 ...
 11   read(3,'a',end=5) line
      ...
      goto 11
 5    continue
      Lch_n=i  !length of native. 0 if no native file
c      write(*,*)'Lch_n=',Lch_n
...
c     determine the cluster with maximum density
      ...
      enddo
      if(ctarget.eq.0) ctarget=cmax !study cmax cluster if we did not select other cluster
c      write(*,'ai2')'ctarget=',ctarget
...
c     regions with average statistics on member of cluster ctarget
c     ctarget: cluster ID of biggest cluster
c     i_cl(ctarget): structure ID of center structure of cluster with
c     cluster ID equal to ctarget
c     n_str_cl(ctarget): number of structures in cluster with ID ctarget
...
c     cluster with ID==i
c      write(*,'i4')i_cl(ctarget)
      call loop_prop(ctarget,i_cl(ctarget),n_str_cl(ctarget),i_str_cl)
...
c     cluster centroid conformation.
      call local_global_rms_correlation(ctarget,i_cl(ctarget),
     $     n_str_cl(ctarget),i_str_cl)
...
***   
 ...
 127     format('Minimum E    in use=',f8.3,f9.1,i8,2x,a20)
      endif !end of "if(Lch_n.gt.1)then" statement
...
cccccccccccccccccc output cluster analysis cccccccccccccccccccc
     ...
     $        rmsd_close_min(i),E_combo(i),E(k),istr(k),filen(itra(k))
      enddo
 123  format(i2,i6,f6.2,f7.2,f6.2,2f9.1,i6,1x,a13)
...
c      write(20,*)
c      write(20,*)'RMSD_cut_initial=',rmsd_cut_initial
c      write(20,*)'RMSD_cut_min=',rmsd_cut_min
c      write(20,*)'RMSD_cut_max=',rmsd_cut_max
c      write(20,*)'ratio_min=',ratio1
c      write(20,*)'ratio_max=',ratio2
      write(20,*)
...
c     to most dense cluster 
      call select_structures(.true.,ctarget) !select only str. of selected cluster
      do iloop=1,nloop
...
c     read chain.dat file, initializing isalig,lbegin,lextent,nloop,lextents
      subroutine readTemplf(Lch,templf,pdbid)
      parameter(ndim=1000)
      parameter(nst=13000)      !number of used structure, maximum allowed
     ...
     $     lresid(ndim,ndim)
      ...
      character buffer*100,xxx*3,pdbid*5,ipdbid*5,templf*500
c     initialize every residue as not covered by the template
      do i=1,Lch
         isalig(i)=0
      enddo

c     iterate over the templates until finding selected one
      open(unit=1,file=templf,err=666)
 5    read(1,*)ipdbid,ncov
      do i=1,ncov
         read(1,'a30,3(f8.5)xi4xa3') buffer,x0,y0,z0,j,xxx
         isalig(j)=1            !residue at position j is aligned
         r(1,j)=x0              !store template coordinates
         r(2,j)=y0
         r(3,j)=z0
      enddo
      read(1,*) buffer          !read 'TER' line
      if(ipdbid.ne.pdbid) goto 5
 ...
 10      if(b.gt.0)then !extend on the left
         ...
         if(e.le.Lch)then !extend on the right
            if(isalig(e).eq.1) then
               ...
               lextents(iloop)=lextents(iloop)+1
               lresid(lextents(iloop),iloop)=e               
            endif
            e=e+1
         endif
         if(lextents(iloop).lt.2*lextent(iloop) .and. 
...
c     first stem residues of N-terminal and C-terminal of loop ends
         end_to_end_dist(iloop)=-1.0 !marks it's not a loop but an unaligned terminus
         if(nterm_stem.ne.0 .and. cterm_stem.ne.0)then
c            print *,'debug message 3'
c            write(6,'2(3(f8.3)x)')r(1,nterm_stem),r(2,nterm_stem),
c     $           r(3,nterm_stem),r(1,cterm_stem),r(2,cterm_stem),
c     $           r(3,cterm_stem)
            do j=1,3               
               trans=r(j,nterm_stem)-r(j,cterm_stem)
               end_to_end_dist(iloop)=end_to_end_dist(iloop)+trans*trans
            enddo
            end_to_end_dist(iloop)=sqrt( end_to_end_dist(iloop) )
         endif
      enddo

c      print *,'debug message 2'
c      do iloop=1,nloop
c         print *,iloop,end_to_end_dist(iloop),lbegin(iloop),
c     $        lextent(iloop),lextents(iloop)
c      enddo

c      print *,'debug message 4'
c      do iloop=1,nloop
c         do j=1,lextents(iloop)
c            print *,lresid(j,iloop)
c         enddo
c      enddo

      return
 666  write(0,'s')'ERROR: could not open template file'//
     $     templf(1:lnblnk(templf))
      call exit(1)
      end
...
c      print *,'debug message 7'
c      do i=1,nstr_selected
c         do j=1,nstr_selected
c            print *,i,j,amat(i,j)
c         enddo
c      enddo
...
c     find out the biggest cluster and decide the RMSD_cut_min
     ...
     $           end_to_end_dist(iloop),.false.) !expected random rmsd for loop global
      ...
      ratio1=0.7
      ratio2=0.15
      nc_max=10  !maximum number of clusters to report
      
c      write(0,*)iloop,rmsd_cut_ini,rmsd_cut
...
c     find out size of biggest cluster
      n_str_cl_max=0
...
c      print *,'debug message 8'
c      do j=1,nstr_selected
c         print *,str_selected(j),n_str_near(str_selected(j))
c      enddo
c      print *,'n_str_cl_max=',n_str_cl_max

c     check the size of the cluster
      ratio=float(n_str_cl_max)/float(nstr_selected)
c      write(0,*)'ratios',iloop,ratio,ratio1,ratio2,n_str_cl_max,nstr_selected
...
c     find out nc_max clusters
      ...
      do i=1,nc_max
c     calculate nc(j), number of neighboring structures ----------------->
         do j=1,nstr_selected
            jj=str_selected(j)
            n_str_near(jj)=0     !number of structure in cluster
            do k=1,nstr_selected
...
c     find out the biggest cluster
         ...
         enddo
c         write(0,*)iloop,islocal,n_str_cl_max,jj,rmsd_cut

c     check the size of the cluster 
         if(n_str_cl_max.lt.1) goto 41 !no remaining structures.
c     remove the structures in the cluster
         ...
         enddo
c         write(0,*)islocal,n_str_cl(i),n_str_cl_ex(i)
...
c     section summary of clusters'
     ...
     $     ' B--------------------------------------------------\n'
      do i=1,nc
         k=i_cl(i)
c         write(0,*)n_str_cl(i),rmsd_cl_cut(i)
         density=float(n_str_cl(i))/rmsd_cl_cut(i)
         bl=lnblnk(b)
...
c     section on include and exclude averages
 ...
 133  format(a,i4,i6,f7.2,f7.2,i6,f7.2,f7.2,a)
      do i=1,nc
         bl=lnblnk(b)
c         write(0,*)islocal,i,n_str_cl(i),R_in(i),Rc_in(i),
c     $        n_str_cl_ex(i),R_ex(i),Rc_ex(i)
      ...
      do i=1,nglobal !cycle over global combo structures
         ...
         do j=1,nc !cycle over combo loop regions
            ...
            if(islocal.eq..true.)then
               ...
               call u3b(w,r_3,r_4,Lstem,1,rms,u,t,ier)
               armsd=residual(r_1,r_2,looplength,u,t)
c               write(0,*)islocal,iloop,i,j,armsd
            endif
            cl=lnblnk(c)
...
c     calculate the rmsd values between combos
         ...
         do j=i+1,nc !cycle over clusters
            ...
            if(islocal.eq..true.)then
               ...
               call u3b(w,r_3,r_4,Lstem,1,rms,u,t,ier)
               armsd=residual(r_1,r_2,looplength,u,t)
c               write(0,*)islocal,iloop,i,j,looplength,armsd
            endif
            cl=cl+7
...
c     identifies the structures belonging to selected cluster
      subroutine select_structures(isloop,ctarget)
      parameter(ndim=1000)      !Length
      parameter(nst=13000)      !number of used structure, maximum allowed
      parameter(ntr=20000)       !number of trajectories
      parameter(ncl=100)            !number of clusters
      ...
      integer ctarget !identity of selected cluster
      common/dens/n_str_all,n_str,d_rc_in,drcinmax,cmax,nstr_selected,
     $     str_selected
      real d_rc_in(100),drcinmax
      integer n_str_all,n_str,cmax,nstr_selected,str_selected(nst)
     ...
     $     i_str_cl_ex,rmsd_cl_cut
      ...
      if(isloop.eq. .true.)then !select structures belonging to most dense cluster
         nstr_selected=n_str_cl(ctarget)
         do i=1,nstr_selected
            str_selected(i)=i_str_cl(ctarget,i)
c            write(*,*)i,str_selected(i)
         enddo
c         write(*,'ai2ai5') 'ctarget=',ctarget,'nstr_selected=',nstr_selected
...
c      print *,'debug message 6'
c      print *,'nstr_selected=',nstr_selected
c      do i=1,nstr_selected
c         print *,i,str_selected(i)
c      enddo
...
c     ###########################################
      ...
      subroutine wellcome()
      call system('clear')
      write(6,*)'Usage: spicker [options]'
      write(6,*)' Required arguments:'
      write(6,*)'  -a template file'
      write(6,*)'  -b pdbid of template to select from file'
      write(6,*)' Optional arguments:'
      write(6,*)'  -c combo id to study unaligned regions (def: cluster'
      write(6,*)'     with maximum density)'
      call exit(1)      
      end
c     ###########################################
