...
/*==============================================================*/
/* GIVEN A VALUE radix IT IS MAPPED INTO A RAINBOW. RETURNS AN {R,G,B} VECTOR.
   0.0 =< radix <= 1.0, rgb is a 1D array*/
void rainbow(float radix, int* rgb){
  int r,g,b;
  if ( radix<0. || radix>1. ){ cout << "out of bounds in rainbox -- "; exit(1); }
  int interval  = int(radix * 6.0);
  int residue = int(255* (radix*6. - float(interval) ));
  switch(interval){
  case 0: r=255        ; g=residue    ; b=0          ; break;
  case 1: r=255-residue; g=255        ; b=0          ; break;
  case 2: r=0          ; g=255        ; b=residue    ; break;
  case 3: r=0          ; g=255-residue; b=255        ; break;
  case 4: r=residue    ; g=0          ; b=255        ; break;
  case 5: r=255        ; g=0          ; b=255-residue; break;
  case 6: r=255        ; g=0          ; b=0          ; break;
  }
  rgb[0]=r;  rgb[1]=g; rgb[2]=b;
}
/*==============================================================*/
/* GIVEN A VALUE radix IT IS MAPPED INTO A RAINBOW from red to blue. RETURNS AN {R,G,B} VECTOR.
   0.0 =< radix <= 1.0, rgb is a 1D array*/
void red2blue(float radix, int *rgb){ rainbow(radix*2/3,rgb); }
/*==============================================================*/
/* GIVEN A VALUE radix IT IS MAPPED INTO A RAINBOW from blue to red. RETURNS AN {R,G,B} VECTOR.
   0.0 =< radix <= 1.0, rgb is a 1D array*/
void blue2red(float radix, int *rgb){ rainbow((1.0-radix)*2/3,rgb); }
/*==============================================================*/
/* GIVEN A VALUE radix IT IS MAPPED INTO A GRAY SHADE from blue to red. RETURNS AN {R,G,B} VECTOR.
   0.0 =< radix <= 1.0, rgb is a 1D array*/
void white2black(float radix, int *rgb){ 
  int gray=static_cast<int>( (1-radix)*255 ); /*implicit cast to int*/
  for(int i=0;i<3;i++) rgb[i]=gray;
}
/*==============================================================*/

char *str2char(string &theStr){
  int l=theStr.length();
  char *ch=new char[l+1];
  theStr.copy(ch,l,0);
  ch[l]=0; /*add null terminator*/
  return ch;
}
/*==============================================================*/
...
string &operator>>(string &input,  PDBatom &thePDBatom){
  ...
  int l=input.length();
  /*80 characters is the correct lenght for an atom line. 54
    characteres if we do not output anything after the coordinates*/
  if(l<54 or l>80){
    cout<<"ATOM line contains "<<l<<" characters!\n";
    cout<<"Abort!\n";exit(0);
  }
...
/*=====================================================================*/
/*mode==0 return whole atom record. mode==1 return up (and including)
  coordinates*/
string PDBatom::exportAtom(int mode){
  string output("ATOM  "),tmp ; 
  int2string5( this->serial , tmp ) ; output += tmp ;
  output += " " ;
  output += this->name ; output += this->altLoc ;
  output += this->resName ; output += " " ; output += this->chainID ;
  int2string4( this->resSeq , tmp )  ; output += tmp ;
  output += this->iCode ; output += "   " ;
  tmp << this->r ; output +=  tmp ;
  if(mode==1) return output+"\n";
  double2string6_2( this->occupancy , tmp ) ; output += tmp ;
  double2string6_2( this->tempFactor , tmp ) ; output += tmp ;
  output += "      " ; output += this->segID ;
  output += this->element ; output += this->charge ;
  return output+"\n";
}
/*=====================================================================*/
...
/*================================================================*/
string PDBatom::getiCode( ){ return iCode ; }
/*================================================================*/
void PDBatom::setiCode(const string &ic){ iCode.assign(ic); }
/*================================================================*/
...
/*================================================================*/
string PDBatom::getAtomName( ){ return name; }
/*================================================================*/
...
/*=====================================================================*/
void PDBatom::setChainID( string &theID ){ chainID = theID ; }
/*=====================================================================*/
...
PDBatom & nodePDBatom::getTheAtom(){
  return this->theAtom;
}

nodePDBatom * nodePDBatom::getNext(){
  return this->next;
}
...
ostream &operator<<(ostream &output, const listPDBatom &theList ){
  ...
  while( currentPtr != NULL){
    tmpString << currentPtr->getTheAtom() ;
...
int listPDBatom::exportList(ostream &out, int mode){
  nodePDBatom *currentPtr=firstPtr;
  while(currentPtr){
    out<<currentPtr->theAtom.exportAtom(mode);
    currentPtr = currentPtr->next ;
  }
  return this->length();
}
...
/*=========================================================================*/
void listPDBatom::changeResSeqiCode( const int &theResSeq, const string &theiCode ){
  nodePDBatom *currentPtr = firstPtr ;
  PDBatom *a;
  while(currentPtr){
    a=&(currentPtr->theAtom);
    a->changeResSeq( theResSeq );
    a->setiCode( theiCode );
    currentPtr = currentPtr->next ;
  }
}
...
/*=============================================================*/
void listPDBatom::removeFullyiCodes(){
  nodePDBatom *currentPtr = firstPtr ;
  while(currentPtr){
    currentPtr->theAtom.setiCode(" ");
    currentPtr=currentPtr->next;
  }
}
/*====================================================================*/
/*If two atoms have same resSeq,iCode,name, remove the second one that is found*/
int listPDBatom::removeRepeatedAtoms(){
  if(firstPtr==NULL || firstPtr->next==NULL) return 0;
  int removed=0;
  nodePDBatom *prev,*c2,*currentPtr=firstPtr;
  PDBatom *a1,*a2; /*references to the atoms*/
  int resSeq;
  string name,iCode;
  while(currentPtr){
    prev=currentPtr;    c2=currentPtr->next;
    a1=&(currentPtr->theAtom);
    name=a1->getAtomName();     resSeq=a1->getResSeq();    iCode=a1->getiCode();
    while(prev && c2){
      a2=&(c2->theAtom);
      if(name==a2->getAtomName() && resSeq==a2->getResSeq() && iCode==a2->getiCode()){
	prev->next=c2->next;/*arrange pointers because we'll delete a2*/
	prev=c2->next;
	if(prev) c2=prev->next;
	delete a2;
	removed++;
      }
      else{
...
/*==============================================================*/
void listPDBatom::setChainID( string &theID ){
  nodePDBatom *pt=firstPtr;
  while(pt){ pt->theAtom.setChainID(theID); pt=pt->next; }
}
/*==============================================================*/
...
/*============================================================*/
int listPDBatom::dump_coordinates_to_array2(double **r){
  int l = (*this).length( ) ;
  PDBatom *atom ; 
  for( int i=0; i<l; i++ ){ 
    atom = getAtomAt( i+1 ) ;  
...
/*============================================================*/
/*n defaults to -1 (see header file)*/
int listPDBatom::dump_coordinates_to_chain( double **r, int n ){
  int l=(*this).length( ) ;
  if(n>=0 && l!=n) return 0; /*we can provide length of array r to check*/
...
/*============================================================*/
string listPDBatom::connectRasmol(){
  string tmp="";
  char line[20];
  int prev;
  nodePDBatom *pt=this->firstPtr;
  nodePDBatom *npt=pt->next;
  if(pt)  prev=pt->theAtom.getSerialNumber( );
  else return tmp;
  int n;
  while(npt){
    n=npt->theAtom.getSerialNumber( );
    sprintf(line,"CONNECT%5d%5d\n",prev,n);
    tmp=tmp+line;  prev=n;  npt=npt->next;    
  }
  return tmp;
}
/*============================================================*/
/*returs a series of rasmol commands whereby each atom is selected and colored from
  red to blue, according to its position in the chain.
  NOTE: stupid rasmol only allows 30 colors per chain*/
string listPDBatom::red2blueRasmol(){
  string tmp="";
  char line[128];
  int l=this->length();
  nodePDBatom *pt=this->firstPtr;
  ...
  double radix;
  while(pt){
    nn=int(n*30/l); /*rasmol only allows 30 colors*/
    red2blue((nn*1.)/30,rgb); /*return a color between red and blue*/
    m=pt->theAtom.getSerialNumber( );
    sprintf(line,"select atomno=%d\ncolor [%d,%d,%d]\n",m,rgb[0],rgb[1],rgb[2]);
    tmp=tmp+line ; n++ ; pt=pt->next ;
  }
  return tmp;
}
/*============================================================*/
/*returs a series of rasmol commands whereby each atom is selected and colored from
  blue to red, according to its position in the chain.
  NOTE: stupid rasmol only allows 30 colors per chain*/
string listPDBatom::blue2redRasmol(){
  string tmp="";
  char line[128];
  int l=this->length();
  nodePDBatom *pt=this->firstPtr;
  ...
  while(pt){
    nn=int(n*30/l); /*rasmol only allows 30 colors*/
    blue2red((nn*1.)/30,rgb); /*return a color between blue and red*/
    m=pt->theAtom.getSerialNumber( );
    sprintf(line,"select atomno=%d\ncolor [%d,%d,%d]\n",m,rgb[0],rgb[1],rgb[2]);
    tmp=tmp+line ; n++ ; pt=pt->next ;
  }
...
/*returs a series of rasmol commands whereby each atom is selected and colored from
  blue to red, according to its position in the chain.
  NOTE: stupid rasmol only allows 30 colors per chain*/
string listPDBatom::white2blackRasmol(){
  string tmp="";
  char line[128];
  int l=this->length();
  nodePDBatom *pt=this->firstPtr;
  ...
  while(pt){
    nn=int(n*30/l); /*rasmol only allows 30 colors*/
    white2black((nn*1.)/30,rgb); /*return a color between blue and red*/
    m=pt->theAtom.getSerialNumber( );
    sprintf(line,"select atomno=%d\ncolor [%d,%d,%d]\n",m,rgb[0],rgb[1],rgb[2]);
    tmp=tmp+line ; n++ ; pt=pt->next ;
  }
...
listPDBatom & nodelistPDBatom::getTheList(){
  return this->theList;
}

nodelistPDBatom * nodelistPDBatom::getNext(){
  return this->next;
}
...
ostream &operator<<(ostream &output, const listsPDBatom &theLists ){
  ...
  string tmpString ;
  while( currentPtr != NULL){
    output<<currentPtr->getTheList();
    currentPtr = currentPtr->getNext() ;
    if(currentPtr)
      if( currentPtr->getNext() ) output<<"TER   \n";
      else output<<"END   \n";
  }
  return output ;
}
...
ifstream &operator>>(ifstream &PDB, PDBamino_acid &theAminoAcid){
  ...
  line >> atomEntry ;
  /*initialize resSeq and iCode. An amino acid is specified by both
    values, not only resSeq*/
  theAminoAcid.name=atomEntry.getResName( );
  resSeq = atomEntry.getResSeq( ) ; theAminoAcid.resSeq=resSeq;
  iCode=atomEntry.getiCode()      ; theAminoAcid.iCode =iCode ;     
  /*cout<<"|"<<iCode<<"|,";*/
  while(atomEntry.getResSeq()==resSeq && atomEntry.getiCode()==iCode ||
	is_TER(line)                                                   ){
...
/*=======================================================================*/
/*return 0 means error*/
int PDBamino_acid::exportAA(ostream &out, int mode){
  int n=this->numberOfAtoms() ,m=0;
  if(!n) return 0;
  m+=backbone.exportList(out,mode) +  residue.exportList(out,mode);
  return n-m;
}
/*=======================================================================*/
...
PDBamino_acid &PDBamino_acid::operator=( PDBamino_acid &aa ){
  name = aa.name ; resSeq = aa.resSeq ; iCode=aa.iCode ;
...
/*=========================================================================*/
string PDBamino_acid::getiCode(){ return iCode; }
/*=========================================================================*/
...
/*=========================================================================*/
void PDBamino_acid::changeFullyResSeqiCode(int &newResSeq, int &prevResSeq, string &previCode){
  int dif_iCode=0, n;
  if(iCode!=previCode) dif_iCode=1;
  n=newResSeq + (resSeq-prevResSeq) + dif_iCode;
  /*store old values of resSeq and iCode, then update these values*/
  prevResSeq=resSeq  ;  resSeq=n          ;  newResSeq=n  ;
  previCode=iCode    ;  iCode.assign(" ") ;
  backbone.changeResSeqiCode(newResSeq," ");
  residue.changeResSeqiCode(newResSeq," ");
}
/*=========================================================================*/
...
/*================================================================*/
void PDBamino_acid::removeFullyiCodes(){
  iCode.assign(" ");
  backbone.removeFullyiCodes();
  residue.removeFullyiCodes();
}
/*================================================================*/
int PDBamino_acid::removeRepeatedAtoms(){
  return backbone.removeRepeatedAtoms()+residue.removeRepeatedAtoms();
}
/*================================================================*/
...
/*============================================================*/
nodePDBamino_acid::~nodePDBamino_acid( ){
  this->theAA.empty();
  next=NULL;
}
/*============================================================*/
...
/*============================================================*/
PDBamino_acid & nodePDBamino_acid::getTheAA(){
  return this->theAA;
}
/*============================================================*/
nodePDBamino_acid * nodePDBamino_acid::getNext(){
  return this->next;
}
...
/*===================================================================*/
PDBchain::PDBchain( string &file ){
  firstPtr = lastPtr = NULL ;
  char *ch=str2char(file);
  ifstream PDB(ch); PDB>>*this;
  PDB.close();
}
/*===================================================================*/
...
nodePDBamino_acid *PDBchain::getNewNode( PDBamino_acid &aa ){
  nodePDBamino_acid *ptr = new nodePDBamino_acid(aa);
  assert( ptr != 0 ) ;
  return ptr ; 
}
/*===================================================================*/
...
void PDBchain::insertAtBack(PDBamino_acid &aa){
  nodePDBamino_acid *newPtr = getNewNode(aa) ;
...
int PDBchain::importChain( ifstream &PDB ){  
  ...
  do{
	...
	if(f){ PDB.seekg( position ) ; break ; }
      ...
      this->insertAtBack(theAminoAcid) ; 
    }
    if( PDB.eof() ){ return 1 ; }
    position=PDB.tellg( ) ;
    getline( PDB, line ) ;
...
/*=====================================================*/
/*0 means error*/
int PDBchain::exportChain(char *outf, int mode, string endline){
  int n=this->numberOfAtoms(),m=0;
  if(!n) return 0;
  ofstream out(outf);
  nodePDBamino_acid *currentPtr=firstPtr ;
  while(currentPtr){
    m+=currentPtr->theAA.exportAA(out,mode);
    currentPtr=currentPtr->next;
  }
  out<<endline;
  return n-m;
...
/*=====================================================*/
int PDBchain::extract_segment(PDBchain &segment,
			      int startResSeq, int endResSeq,
			      string startiCode, string endiCode){
  /*cout<<startResSeq<<" "<<endResSeq<<" |"<<startiCode<<"|, |"
    <<endiCode<<"|\n";*/
  nodePDBamino_acid *currentPtr;
  currentPtr=this->pointToNodeWithResSeqiCode(startResSeq,startiCode);
  if(!currentPtr) return 0; /*there's no such ResSeq as 'start'*/
  segment.empty(); /*remove whatever in segment*/
  int n=0;
  int rs=currentPtr->theAA.getResSeq();     
  string ic=currentPtr->theAA.getiCode(); 
  /*cout<<"rs="<<rs<<" ic=|"<<ic<<"|\n";*/
  while(rs!=endResSeq || ic!=endiCode){
    n++;
    segment.insertAtBack(currentPtr->theAA);
    currentPtr=currentPtr->next;
    rs=currentPtr->theAA.getResSeq();
    ic=currentPtr->theAA.getiCode();  /*cout<<"rs="<<rs<<" ic=|"<<ic<<"|\n";*/
    if(!currentPtr) break; /*reach end of chain before 'end'*/
  }
  /*previous loop does not include case rs==endResSeq && ic==endiCode*/
  if(currentPtr)segment.insertAtBack(currentPtr->theAA);
  return n;
}
/*=====================================================*/
...
/*=====================================================*/
/*iCode ic is defaulted in pdbClasses2.h*/
nodePDBamino_acid *PDBchain::pointToNodeWithResSeqiCode(const int &theResSeq, 
							string ic ){
  /*cout<<"Starts nodePDBamino_acid *PDBchain::pointToNodeWithResSeqiCode\n";*/
  nodePDBamino_acid *currentPtr = firstPtr ;
  while(currentPtr){
    /*cout<<currentPtr->theAA.getResSeq()<<" |"
      <<currentPtr->theAA.getiCode()<<"|\n";*/
    if(currentPtr->theAA.getResSeq()==theResSeq &&
       currentPtr->theAA.getiCode()==ic           ) return currentPtr;
    currentPtr = currentPtr->next ;
  }
...
/*=============================================================*/
/*expand residues with different insertion codes, change insertion code to " ".
  Maintain jumps in resSeq due to missing residues*/
void PDBchain::renumberFully( const int &firstResSeq ){
  if(this->isEmpty())return;
  int index=firstResSeq;
  nodePDBamino_acid *currentPtr = firstPtr;
  while(currentPtr){
    currentPtr->theAA.changeFullyResSeq(index);
    index++;
    currentPtr = currentPtr->next ;
  }
}
...
/*=============================================================*/
void PDBchain::removeFullyiCodes(){
  nodePDBamino_acid *currentPtr = firstPtr ;
  while( currentPtr ){
    currentPtr->theAA.removeFullyiCodes();
    currentPtr = currentPtr->next ;
  }
}
/*=============================================================*/
int PDBchain::removeRepeatedAtoms(){
  int removed=0;
  nodePDBamino_acid *currentPtr = firstPtr ;
  while( currentPtr ){
    removed+=currentPtr->theAA.removeRepeatedAtoms();
    currentPtr = currentPtr->next ;
  }
  return removed;
}
/*=============================================================*/
int PDBchain::removeResAtIndex(const int &i ){
  int l=this->length(); /*printf("l=%d\n",l);*/
  if( i<1 || i>l ) return 0;
  nodePDBamino_acid *cpt=firstPtr; /*current pointer*/
  nodePDBamino_acid *ppt=firstPtr; /*previous pointer*/
  int j=1;
  while(j<i){ ppt=cpt;  cpt=cpt->next; j++; }
  if(i==1) firstPtr=firstPtr->next;
  if(i==l) lastPtr=ppt;
  ppt->next=cpt->next; /*jump over residue to remove*/
  delete cpt; /*deallocate residue*/
  return 1;
}
/*=============================================================*/
/*set initial resSeq to one, increase by one consecutive
  indexes. Remove iCodes. This method doesn't pay attention to missing
  residues*/
void PDBchain::simplify(){  
  /*renumber all residues starting from one, following amino acid
    increase resSeq by one*/
  this->renumberFully(1); 
  /*remove iCodes, since they have no sense anymore*/
  this->removeFullyiCodes();
  /*remove atoms with same name in the same residue, with different
    Alternate location indicator*/
  this->removeRepeatedAtoms();
  /*renumber all atoms starting from one, allways increasing by one,
    independent of whether there are missing residues*/
  this->renumberFullyAtomSerialNumber(1); 
}
/*=============================================================*/
...
/*================================================================*/
int PDBchain::createCAcontactMap( PDBchain &chain2, double **map,
				  double &cutoff ){
  listPDBatom CAlist, CAlist2 ;
  this->createCAchain( CAlist ) ;
  chain2.createCAchain( CAlist2 ) ;
  return CAlist.createContactMap( CAlist2, map, cutoff ) ;
}
/*================================================================*/
...
string PDBchain::output_one_letter_sequence_from_indexes( const int &a,
  ...
  while(currentPtr){
    tmp+=three_letter_code_to_one_letter_code(currentPtr->theAA.getName());
    n++;  currentPtr=currentPtr->next;
    if(n>b) break;
  }
  return tmp ;
}
...
/*============================================================*/
PDBchain & nodePDBchain::getTheChain(){
  return this->theChain;
}
/*============================================================*/
nodePDBchain * nodePDBchain::getNext(){
  return this->next;
}
/*================================================================*/
...
/*================================================================*/
PDBchains::PDBchains(string &file){
  char *ch=str2char(file);
  ifstream IN(ch);
  firstPtr = lastPtr = NULL ;
  this->importChains(IN);
