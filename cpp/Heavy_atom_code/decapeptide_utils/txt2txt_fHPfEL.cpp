...
void printNONEL_COL( ifstream &DAT, ostream& DMD, double *hcr_list,
		     const double &fHP, const double &fEL){
  cout << "NONEL_COL ...\n" ;
  ...
  for( int i=1; i<=n_dmd_atom_t; i++ ){
    for( int j=i; j<=n_dmd_atom_t; j++ ){
      ...
      /*cout<<"Van_der_Waals OK\n";*/
      if(get_default_hydrophobicity( stype1, stype2, nonel_col[i][j], DAT )){
	rescale_default_hydrophobicity_by_factor( nonel_col[i][j], fHP ) ;
	/*cout<<"hydrophobicity OK\n";*/
      }
      if(get_default_electrostatics( stype1, stype2, nonel_col[i][j], DAT )){
	rescale_default_electrostatics_by_factor( nonel_col[i][j], fEL ) ;
	/*cout<<"electrostatics OK\n";*/
      }
...
bool test_input(int argc, char ** argv ){
  ...
  if( number_of_arguments != 6 ){
    system("clear");
    cout << "Usage: ./txt2txt_fHPfEL.x relaxed.txt model.pdb force_field fHP fEL init.txt\n" ;
    cout << "rlxd    : DMD conformation with no hydrogen bond formed and with all permanent bonds present (outcome of a relaxation procedure).\n";
    cout << "model   : any conformation of the protein in PDB format\n";
    cout << "frc_fld : file with info on atoms (mass, radius, bonded and nonbonded interactions)\n";
    cout << "fHP     : rescale the default hydrophobic interactions by fHP\n" ;
    cout <<"fEL     : rescale the default electrostatic interactions by fEL\n";
...
int main(  int argc, char ** argv ){
  ...
  ifstream DAT( argv[ 3 ] ) ;
  double fHP = atof( argv[ 4 ] ) ;
  double fEL = atof( argv[ 5 ] ) ;
  ofstream DMD( argv[ 6 ] ) ;
  ...
  printNONEL_COL( DAT, DMD, hcr_list, fHP, fEL ) ;
