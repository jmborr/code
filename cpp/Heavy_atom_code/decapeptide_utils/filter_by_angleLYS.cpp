...
bool test_input(int argc, char ** argv ){
  ...
  if( number_of_arguments != 5 ){
    system("clear");
    cout << "Usage: ./filter_by_angleLYS.x pdbmov atf minA maxA out\n";
    ...
    cout << "       : remaining two entries to determine beginning and end\n";
    cout << "       : points of a vector. We then determine angle between\n";
    cout << "       : the plane and the vector\n";
    cout << "       : If the five entries are a1,a2,a3,a4,a5, then\n"; 
    cout << "         angle = pi/2-acos((a5-a4) * [ (a1-a3) x (a2-a3) ])\n";
    cout << "minA   : minimum angle\n";
    cout << "maxA   : maximum angle\n";
    cout << "out    : conformations with angles between in the range [minA,maxA]\n";
    cout << "       : are stored in file out\n";
    cout << "\n";
    return false ;
  }
  else return true ;
...
int main(int argc, char* argv[]){
  ...
  double minA=atof(argv[3]),maxA=atof(argv[4]);
  ...
  while( PDBMOV>>prot ){
    ...
    if(angle>minA && angle<maxA){OUT<<prot<<"END \n";}
  }
  OUT.close();
