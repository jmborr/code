...
/*=====================================================*/
void add_soft_inner_barrier( string &potential, const double &sf,
			     const double &inf ){
  double *w = NULL ;
  int n_w = 0 ;
  char cbuf[256] ;
  string sbuf ;
  w = split_line2doubles( potential, n_w ) ;
  sprintf( cbuf, "%8.6f %8.6f %8.6f ", w[0]*(1-sf), w[0], inf ) ;
  potential.assign( cbuf ) ;
  for( int i=1; i<n_w; i++ ){/*i=1 because w[0] already entered in potential*/
    sprintf( cbuf, " %f", w[i] ) ; sbuf.assign( cbuf ) ;
    potential += sbuf ;
  }
}
/*=====================================================*/
void add_soft_ext_barrier( string &potential, const double &sf, 
			   const double &inf ){
  double *w = NULL ;
  int n_w = 0 ;
  char cbuf[256] ;
  string sbuf ;
  w = split_line2doubles( potential, n_w ) ;
  sprintf( cbuf, " %8.6f %8.6f", -inf, w[n_w-1]*(1+sf) ) ;
  sbuf.assign( cbuf ) ; potential += sbuf ;
}
/*=====================================================*/
...
void printNONEL_COL( ifstream &DAT, ostream& DMD, const int &n_go_types,
		     ...
		     go_atom *chain_go, double *hcr_list, const double &sf,
		     const double &inf ){
  cout << "NONEL_COL ...\n" ;
  ...
  for( int i=1; i<=n_go_types; i++ ){
    for( int j=i; j<=n_go_types; j++ ){  /*cout<<"i="<<i<<", j="<<j<<endl;*/
      ...
      add_soft_inner_barrier( nonel_col[i][j], sf, inf ) ;
...
void printLINKED_PAIRS( ifstream &DAT, ofstream &DMD, const int &n_go_types,
			...
			const double &sf, const double &fr, const double &inf){
  cout << "LINKED_PAIRS ...\n" ;
  DMD << txtKeyWords[LINK_PAIRS] << endl;
  dmd_atom_t type1, type2 ;
  string stype1, stype2, link ;
  char cbuf[256], cbuf2[256] ;
  double factor2 = get_default_rotamer_barrier( DAT ) ; factor2*=fr;
  bool is_HB ;
  ...
  for( int i=1; i<=n_go_types; i++ ){
	...
	link.assign( cbuf ) ;/*fill link with new contents*/
	add_soft_inner_barrier( link, sf, inf ) ;
	if( is_HB==false ){ add_soft_ext_barrier( link, sf, inf ) ; }
	string2char( link, cbuf ) ;
...
void  printLIST_ATOMS_LIST_OF_BONDS( ifstream &RLX, ofstream &DMD,
				     go_atom *chain_go, PDBchain &chain ){
  cout << "LIST_ATOMS ...\n" ;
  cout << "LIST_BONDS ...\n" ;
  int a, b, pos = RLX.tellg( ), n_w, index=1 ; RLX.seekg( 0 ) ;
...
bool test_input(int argc, char ** argv ){
  int n_args = argc -1 ;
  if( n_args<4 || n_args>10){
    system("clear");
    cout << "Usage: ./txt2txt_Go2.x pre-rlxd nat.pdb frc_fld init.txt f_go e_go e_HB sf inf fr\n" ;
    cout << " pre-rlxd: pre-relaxed DMD conformation from a stretched chain\n" ;
    ...
    cout << " f_go    : (1+go_co_f)*(VdW_i+VdW_j) Go range for i,j. Default=0.35\n" ;
    cout << " e_go    : strength for Go model (must be negative!). Default=-1.0\n";
    cout << " e_HB    : strength hydrogen bond (must be negative!)Default=-0.1\n";
    cout << " sf      : percentage of how much we can penetrate the hard-core, or\n" ;
    cout << "           go beyond the link length. Default=0.1\n" ;
    cout << " inf     : height of the extra barrier included with sf. Default=1.0\n";
...
int main(  int argc, char ** argv ){
  ...
  if(n_args>4){
    ...
    printf("f_go=%4f e_go=%4f e_HB=%4f sf=%4f inf=%4f fr=%4f\n",
	   go_co_f,e_go,factor,sf,inf,fr);
  }
  else{
    cout<<"Using default parameters:\n";
    cout<<" f_go=0.35 e_go=-1 e_HB=-0.1 sf=0.1 inf=1.0 fr=0\n";
  }
  ...
  printNONEL_COL( DAT, DMD, n_go_types, go2dmd_atom_t, go2id, chain, go_co_f, e_go, chain_go, hcr_list, sf, inf ) ;
  printLINKED_PAIRS( DAT, DMD, n_go_types, go2dmd_atom_t, factor, sf, fr, inf);
  printREACT( DAT, DMD, n_go_types, go2dmd_atom_t ) ;
  printLIST_ATOMS_LIST_OF_BONDS( RLX, DMD, chain_go, chain ) ;
