...
/*=======================================================================*/
int read_pdb(char *name, double **r){
...
/*=======================================================================*/
int read_trajectory_snapshot(FILE *ptra, double **r, char *header){
  int i,n=0;
  char *buff=new char[512];
  if(!fgets(header,10000,ptra)) return 0;
  /*printf("%s",header);*/
  sscanf(header,"%d",&n); /*first item should be number of vector-coordinates*/
  /*printf("n=%d\n",n);*/
  for(i=0;i<n;i++)
    fscanf(ptra, "%lf %lf %lf",&r[i][0],&r[i][1],&r[i][2]);
  fgets(buff,10000,ptra); /*read past the last "\n" after reading the coordinates*/
  /*printf("%lf %lf %lf\n",r[52][0],r[52][1],r[52][2]);*/
  return n;
}
