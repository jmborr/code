...
/* declare for tmp subdir */
char tmp_dir[13];
char command_tmp[300]="";
char fname_tmp[60];
...
int open_echo_file(int is_open,  char * fname)
{
  ...
  scanf("%s",fname_tmp); /*scanf("%s",fname);*/

  /* write data file to dir /tmp/  */  
  sprintf(fname, "%s%s", tmp_dir,fname_tmp);
  echo_path=fopen(fname,"wb");
  sprintf(command_tmp, "%s cp -f %s ./;",command_tmp, fname);
