...
extern char command_tmp[300];
extern char fname_tmp[60];
...
static int ncont;
static int **cont;
static char **native_cont;
static double native_dist;
static double **contE;
...
int nonnative1(int * nnat)
{
  int i,j,nnn=0,nat1=0;int c;
  for(i=1;i<nn;i++)
    for(j=0;j<i;j++)
      {
       c=contact(j+n0,i+n0);
...
extern int init_rms(void)
{
  ...
  scanf("%s",fname_tmp);/*scanf("%s",name);*/

  /* get to the tmp dir */
  sprintf(name, "%s%s", tmp_dir, fname_tmp);
  sprintf(command_tmp, "%s cp -f %s ./;", command_tmp, name);
  ...
  if(infile){
    fscanf(infile,"%ld %ld %lf %lf",&nn,&n01,&box,&native_dist);
    n0=n01-1;
    box2=box/2;
  ...
  native_cont=(char **)malloc(nn*sizeof(char*));
  if(!cont)return 0;
  if(!native_cont)return 0;
  cont[0]=(int *)malloc(ncont*sizeof(int));
  native_cont[0]=(char *)malloc(ncont*sizeof(char));
  if(!cont[0])return 0;
  if(!native_cont[0])return 0;
  for(i=0;i<nn-1;i++)
    {
      cont[i+1]=cont[i]+i;
      native_cont[i+1]=native_cont[i]+i;
    } 
  for(i=0;i<ncont;i++)
    {
      cont[0][i]=0;
  ...
  for(i=1;i<nn;i++)
    for(j=0;j<i;j++)
      {
	double s=native_dist;
	for(k=0;k<3;k++)
	  {
	    double dd=x[i][k]-x[j][k];
	    s-=dd*dd;
	  }
	if(s>0) native_cont[i][j]=1;
      }
...
/* such that rms(x'-y) is minimal */
  ...
  //calculate the determinent of u[i][j]
  {
    double determinent=0;
    determinent  = u[0][0]*u[1][1]*u[2][2]
      +u[1][0]*u[2][1]*u[0][2]
      +u[2][0]*u[0][1]*u[1][2]
      -u[0][0]*u[2][1]*u[1][2]
      -u[1][0]*u[0][1]*u[2][2]
...
int save_rms(void)
{
  ...
  /* write  to dir /tmp/  */
  sprintf(cont_file_name, "%s%s", tmp_dir, cont_file_name);
...
int contact(int i, int j){
  well_type ct=bond((atom_number)i,(atom_number)j);
  if(ct<0)return 0;
  if((!is_bond(ct))&&is_internal(ct)){ 
    if(native_cont[j-n0][i-n0])return 1;
    else return -1;
  }
  return 0;
}
...
{ 
  ...
  {
    int nnat;
    int nnn=nonnative1(&nnat)/* pairs(nonnative) */;
    ...
    int nucleus;
    if(nnuc)nucleus=is_nucleus();
    else nucleus=nnat;
    nbyte=sprintf(&s[0],"%12.4lf\11%10.4lf\11%10.4lf\11%4ld\11%10.3lf\11%10.3lf\11%12.5lf %1d\n"
