	  ...
	  do
	    {
	      ...
	      if(index==-1)break;
/*write_start is the first atom number from which we ouput information(zero by default). write_finish is write_start+n1*/
   ...
   if(is_hb){
     if(fErr==noErr)
       nbyte=sprintf(&s[0],"%s\n//atome number, hb association1, hb association2, d0, d1, d2\n",keywords[HB_LIST]);
     if(nbyte<=0) fErr=-1;    
     if(fErr==noErr)fErr=(fwrite(&s[0],1,nbyte,path)!=nbyte);	 	   
     if(fErr==noErr) 	 
       for(i=0;i<n1;i++){
	 if(a[i].hb){
	   ...
	   if(a[i].hb==2)nbyte+=sprintf(&s[nbyte]," %4ld %lf %lf %lf",a[i].hb_a2+1-write_start, a[i].d0, a[i].d1, a[i].d2);
	   s[nbyte]='\n';nbyte++;
	   if(nbyte<=0) fErr=-1;  
	   if(fErr==noErr)fErr=(fwrite(&s[0],1,nbyte,path)!=nbyte);
	   if(fErr!=noErr)break;
...
{  
  ...
  double *perm_bonds;/*the place in memory where the list of permanent bonds begins, if any*/
  int nperm_bonds=0;/*number of permanent bonds*/
  double ***coldata;
  double ***bonddata;
...
/* number type x y [z] v_x v_y [v_z]  */
	...
	case LIST_BONDS:
	  {
		...
		if(next_key<0)
                ...
                if(!next_key)
		  {
		    ...
		    top+=2*(numword-1);
		    perm_bonds = top;/*here begins the list of permanent bonds, if any*/
		  }
	      }while(!next_key);
	    current_key=next_key;
	    break;
	...
	do{
	  ...
	  if(!next_key){
	    ...
	    if(numword!=4)return line_number;
	    nperm_bonds++;
	    i=top[0];/*number of first atom (i-1 is the index for the atom)*/
	    j=top[1];
	    if(top[2]!=(int)top[2] ||
	       top[3]!=(int)top[3]){
	      return line_number;
	    }
	    a[i-1].origc = (int)top[2];/*store the original type for atom with index i-1 (i.e, for atom number i)*/
	  ...
	  do{
		...
		if(!(datfile=next_word(datfile)))break;
		 ...
		 top[2]!=(int)top[2])return line_number;
	      i=top[0];	     
	      a[i-1].hb   = numword - 5;/*initially, hb==(0|2) for every atom.*/
	      a[i-1].gid  = top[1];
	      a[i-1].hb_a1= top[2]-1;
	      a[i-1].hb_a2= top[3]-1;
	      a[i-1].d0   = top[4];
