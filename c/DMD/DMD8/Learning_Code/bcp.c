...
static    double ly;
static    double lz;
int deltaGID;//minimal amino acid separation to allow a HB
...
{  
  int ct=ct1;
  int rtype;
  int revers;//if positive, particles are moving away from each other
  
  if(sc<0)//particles are approaching to each other
    {
...
{  
  ...
  sc=vx*x+vy*y+vz*z; //scalar product of relative velocity and vector. If negative, particles are approaching each other
  
  if(nrt)
    {
      int ctr;
      
      ctr=reaction(a,i,j,ct1,sc,x,y,z);//return -1 if unsuccessful
...
{  
  ...
  return -3;
 far_away:
  if(link_err){breakBond(i,k);printf("bond is broken %d %d\n",i,k);}
  return ct;
}	
...
{  
  ...
  if(*link_err)
	...
	int koc = a[k].origc;
	ii = icoll[ioc][koc];
	//printf("%ld and %ld is PermBonds %ld \n",i+1,k+1, ii);
	if(ii>-1){
	  if(dr<coll[ii].dd){ 
	    for(ii=coll[ii].next;ii>-1;ii=coll[ii].next){
	      if(dr>coll[ii].dd){
