...
typedef struct {
  ...
  well_type react; /* shows record of reaction and bond status; negative -- if bond less than -1 -- backward reaction, 0 if no reaction and positive if forward reaction*/
} CollisionData;
...
typedef struct {
  ...
  int origc;
  int hb;
  int gid;/*amino acid index, to which the particular atom belongs to*/
  int hb_a1;
  int hb_a2;
  double d0;
  double d1;
...
/*
  hb=0; not involved in Hydrogen-bonding
  otherwise involved in Hydrogen-bonding
  1--only one association
  2--two association
  the number of association can not exceed 2;
  Initially, hb is either 0 or 2 for every atom.
  hb_a1--the first association,ie, the atom index (not atom number!) of one
  of the two helper atoms to stablish the bond. If the atom is C, then the
  helper is the previous CA. If the atom is N, then the helper is the
  previous C.

  hb_a2--the second association, ie, the atom index (not atom number!) of the
  second helper. If the atom is C, then the helper is the next N. If the atom
  is N, then the helper is the next CA.
*/
typedef struct {
  double r[3];
  double v[3];
