...
void update_table (atom_number p, atom_number q, coll_type ct){
  /*printf("p=%4d q=%4d ct=%d junk=%8d\n",p,q,ct,junk);*/
  /*dimensions *bound=get_bounds();
    if(junk==1||junk==556015){
    printf("junk=%d\n",junk);  dump_info_of(138); dump_info_of(56); 
  ...
  renew_calendar();
  /*print_coll_info_plist(HOT,80,111);//exit(0);
    if(junk>24000){ print_coll_info_plist(HOT,80,111);}*/
  /*check_all_pointers(); 
  check_zero_times();
  check_inf_times();
  chell_all_atom_cell_addresses();
  junk++;*/
