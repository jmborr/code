...
int allocPerm(){
  int i;
  isPerm = (char *)malloc(nFriend*sizeof(char));
  if(!isPerm) return 0;
  for(i=0; i<nFriend; i++){
    isPerm[i]=0;
  }
...
/*=========================================================================*/
int setPerm(int friend1, int friend2){
  int i;
  bond_type index;
  bond_type newindex;
  bond_type newfriend;
...
/*=========================================================================*/
int breakBond (int friend1, int friend2){/*42 lines of code*/
  int i;
  bond_type index;
  bond_type oldindex;
  bond_type oldfriend;
  ...
  for(i=0;i<2;i++){
	...
	if(isPerm && isPerm[oldindex]){
	  printf("Trying to break the permanent bond %ld %ld %ld!\n",
		 friend1+1, friend2+1, isPermBonds(friend1, friend2));
	  exit(1);
	}
	res++;
...
int breakBond2 (int friend1, int friend2){/*42 lines of code*/
  int i;
  bond_type index;
  bond_type oldindex;
  bond_type oldfriend;
  int found;
  int res=0;
  printf("Beginning of breakBond2(%d,%d)\n",friend1,friend2);
  for(i=0;i<2;i++){
    if(i){
      index=friend1+nFriend;
      oldfriend=friend2;
  ...
  if(res>0)newBonds=1;
  printf("End of breakBond2(..)\n");
  return res;
}/*Matches int breakBond2(int friend1, int friend2)*/
/*========================================================================*/
...
int isFriend(bond_type atomNumber, bond_type friendNumber)
{
  ...
  if(atomNumber == friendNumber) //?? the same number?
    return 1;
  while(next[index]>=0)
    {
      index=next[index];
...
int isPermBonds(bond_type atomNumber, bond_type friendNumber)
{
  bond_type index=nFriend+atomNumber;
  if(atomNumber == friendNumber) //?? the same number?
    return 1;
  ...
  while(next[index]>=0)
    {
      index=next[index];
      if(friend[index]==friendNumber){
	if(isPerm[index])return 1;
      }
    }
  return 0;
}
...
bond_type nextPermBonds(int atomNumber, int * index)
{
  bond_type newindex;
  if(index[0]==-1)
    newindex=nFriend+atomNumber;
  ...
  if(newindex<0)
    {index[0]=-1;return -1;}
  else if(isPerm[newindex])
    {index[0]=newindex;return friend[newindex];}
  else
    {index[0]=newindex;return -1;}
}
