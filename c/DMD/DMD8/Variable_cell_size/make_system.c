...
/*  printf("%d",j);*/
  coll[j].e=-en;
  coll[j].eo=-en;
  coll[j].dd=diam*diam; /*if(diam<0.01){printf("diam=%d\n",diam); exit(0);}*/
  coll[j].dmsf2= diam>soc.sf ? (diam-soc.sf)*(diam-soc.sf) : 0.0 ;
  coll[j].dpsf2=(diam+soc.sf)*(diam+soc.sf);
...
int allocReact(ReactionData * react1, int nrt1,ReactionData * react0,int nrt0){
  ...
  if((nrt1>0)&&(nrt0>0)){
    ...
    for(i=1;i<=nrt0;i++){
      for(j=1;j<=nrt1;j++){
	if(react1[j].bond){
	  if( ( (react0[i].new1==react1[j].new1) && 
		(react0[i].new2==react1[j].new2)    ) ||
	      ( (react0[i].new1==react1[j].new2) &&
		(react0[i].new2==react1[j].new1)    )   ){
	    react0[i].old1=0;
	    k++;
	    break;
	  }
	}
	if( ( (react0[i].old1==react1[j].old1) &&
	      (react0[i].old2==react1[j].old2)    ) ||
	    ( (react0[i].old1==react1[j].old2) &&
	      (react0[i].old2==react1[j].old1)    )   ){
	  react0[i].bond=-1;
	  break;
	}
      }
    ...
    for (i=1;i<=nrt;i++)
      printf("%d %d %d %d %d %lf %lf\n",react[i].old1,react[i].old2,
	     react[i].new1,react[i].new2,react[i].bond,react[i].dd,
	     react[i].eo);
    return nrt;
  }
}/*Matches allocReact(..)*/
/*=========================================================================*/
int write_key_coord(FILE * path){ /*226 lines*/
  int nbyte;
  unsigned char s[512];
  int i,j,k; 
  int fErr=noErr;
  ...
  /*print delta res*/
  if(fErr==noErr)
    nbyte=sprintf(&s[0],"%s\n//hydrogen bond between i,i+N with N>2\n%ld\n",
		    keywords[DELTA_RES],deltaGID);
  if(nbyte<=0) fErr=-1;    
  if(fErr==noErr) fErr=(fwrite(&s[0],1,nbyte,path)!=nbyte);
  /*print cell size*/
  if(fErr==noErr)
    nbyte=sprintf(&s[0],"%s\n%lf\n",keywords[CELL_SIZE],soc.input_cell_size);
  if(nbyte<=0) fErr=-1;    
  if(fErr==noErr)fErr=(fwrite(&s[0],1,nbyte,path)!=nbyte);
  /*print safe limit*/
  if(fErr==noErr)
      nbyte=sprintf(&s[0],"%s\n%lf\n",keywords[SAFE_LIMIT],soc.sf);
  if(nbyte<=0) fErr=-1;    
  if(fErr==noErr)fErr=(fwrite(&s[0],1,nbyte,path)!=nbyte);
  /*print number of atoms*/
  if(fErr==noErr)
      nbyte=sprintf(&s[0],"%s\n%ld\n",keywords[NUM_ATOMS],write_n1);
  if(nbyte<=0) fErr=-1;    
  if(fErr==noErr)fErr=(fwrite(&s[0],1,nbyte,path)!=nbyte);
  ...
  if(fErr==noErr) 
    for(k=1;k<=nrt;k++){
      nbyte=sprintf(&s[0],"%d %d %d %d %d %lf %lf\n",react[k].old1,
		    react[k].old2,react[k].new1,react[k].new2,react[k].bond,
		    sqrt(react[k].dd),react[k].eo);
      if(nbyte<=0) fErr=-1;
      if(fErr==noErr)fErr=(fwrite(&s[0],1,nbyte,path)!=nbyte);
      if(fErr!=noErr)break;
    }
	...
	do{
	  ...
	  if(j>=0&&(j>i)&&(i>=write_start)&&(j<write_finish)) {
	    nbyte=sprintf(&s[0],"%4ld %4ld %4ld %4ld\n",i+1-write_start,
			  j+1-write_start,a[i-write_start].origc,
			  a[j-write_start].origc);
	    if(nbyte<=0) fErr=-1;  
	    if(fErr==noErr)fErr=(fwrite(&s[0],1,nbyte,path)!=nbyte);
	    if(fErr!=noErr)break;
	  }
  ...
  /*print list of hydrogen bonding associations*/
  if(is_hb){
    if(fErr==noErr)
      nbyte=sprintf(&s[0],"%s\n//atome number, gid, hb association1, hb association2, d0, d1, d2\n",keywords[HB_LIST]);
    if(nbyte<=0) fErr=-1;    
    if(fErr==noErr)fErr=(fwrite(&s[0],1,nbyte,path)!=nbyte);	 	   
    if(fErr==noErr) 	 
      for(i=0;i<n1;i++){
	if(a[i].hb){
	  nbyte=sprintf(&s[0],"%4ld %4ld %4ld",
			i+1-write_start, a[i].gid, a[i].hb_a1+1-write_start);
	  if(a[i].hb==2)
	    nbyte+=sprintf(&s[nbyte]," %4ld %lf %lf %lf",
			   a[i].hb_a2+1-write_start, a[i].d0,
			   a[i].d1, a[i].d2);
	  s[nbyte]='\n';nbyte++;
	  if(nbyte<=0) fErr=-1;  
	  if(fErr==noErr)fErr=(fwrite(&s[0],1,nbyte,path)!=nbyte);
...
int make_tables (int isfile){
  ...
  if(!allocalendar(n1)){StopAlert (MEMORY_ALRT);return -1;}
  err=init_tables(); 
  initcalendar();/*construct the binary tree*/
  if(err!=1)return err;
  init_rms();
  init_clusters();
  init_corr_func();
...
int init_keywords(){ 
  ...
  keywords[DELTA_RES]="A.1.DELTA RES";
  keywords[CELL_SIZE]="A.2.CELL SIZE";
  keywords[SAFE_LIMIT]="A.3.SAFE LIMIT";
...
/*=======================================================================*/
int make_wells (double *** coldata,int numwell,double *** bonddata,
		int numbondwell,double * bonds,int numbond,atom * a,
		int nAtom,atom * sam,int nat){
...
/*  printf("%d\n",ntot);*/
  ...
  if(!allocate_and_init_shell_order_scheme(coldata,sam,nat)){
    StopAlert (MEMORY_ALRT);
    printf("Can not allocate the shell-order scheme!\n");
    exit(0);
  }
  ...
  for(k=1;k<=nrt;k++)
	...
	while(next>-1){
      ...
      /* irreversible reactions of breaking bonds created when bond is
	    breakable but no bond-forming reaction corresponing to
	    input types is specified */
      react[k].bond=1;
...
/*	printf("%d %lf\n",num,coll[num].etot);*/
  ...
  for(i=0;i<ntot;i++){
    coll[i].lt=WARM;
    if(coll[i].prev>-1)
      if(sqrt(coll[i].dd)+2*soc.sf>sqrt(coll[coll[i].prev].dd))
	coll[i].lt=HOT;
  }
  /*printf("End of make_wells(..)\n");*/
}/*Matches int make_wells(...)*/
/*=========================================================================*/
make_key_system (int first_key, unsigned char * buf,int file_length){  
  ...
  ReactionData * react1;
  printf("dx,dy,dz=?\n");
  coord_shift[0]=0;coord_shift[1]=0;coord_shift[2]=0;
  ...
  while(current_key)
    {
	 ...
	 datfile=nextline;if(!(nextline=next_line(datfile)))return line_number;
	    ...
	    set_input_delta_res(dummy[0]);
	    datfile=nextline;
	    if(!(nextline=next_line(datfile)))return line_number;
	    next_key=iskeyword(datfile);
	    if(next_key<=0)return line_number;
	    else current_key=next_key;
	    dump_input_delta_res();
	    break;
	  }
        case CELL_SIZE :
	  {
	 ...
	 datfile=nextline;if(!(nextline=next_line(datfile)))return line_number;
    	    sscanf(datfile,"%lf",dummy);
	    set_input_cell_size(dummy[0]);
	    datfile=nextline;
	    if(!(nextline=next_line(datfile)))return line_number;
	    next_key=iskeyword(datfile);
	    if(next_key<=0)return line_number;
	    else current_key=next_key;
	    dump_input_cell_size();
	    break;
	  }
        case SAFE_LIMIT:
	  {
	 ...
	 datfile=nextline;if(!(nextline=next_line(datfile)))return line_number;
    	    sscanf(datfile,"%lf",dummy);
	    set_input_safe_limit(dummy[0]);
	    datfile=nextline;
	    if(!(nextline=next_line(datfile)))return line_number;
	    next_key=iskeyword(datfile);
	    if(next_key<=0)return line_number;
	    else current_key=next_key;
	    dump_input_safe_limit();
	    break;
	  }
        case NUM_ATOMS:
	  {
	...
	case TYPE_ATOMS: 
	  {
		...
		sam[j].c=i;
		sam[j].origc=sam[i].c;
		sam[j].hb=0; sam[j].gid=-1; sam[j].hb_a1=-1; sam[j].hb_a2=-1;
		sam[i].r=o;
		sam[i].v=o;
		sam[i].t=0.0;
		sam[i].i.x.i=0;
      ...
      case HB_LIST:
	{
	  ...
	  do{
		...
		if(!(datfile=next_word(datfile)))break;
	      ...
	      if(numword!=7)return line_number;
	      if(top[0]!=(int)top[0]||
		 top[1]!=(int)top[1]||
		 top[2]!=(int)top[2])return line_number;
	      i=top[0];
	      a[i-1].hb   = numword - 5;
	      a[i-1].gid  = top[1];
	      a[i-1].hb_a1= top[2]-1;
...
void dump_coll(CollisionData c){
  printf("%d %lf %lf ",c.lt,sqrt(c.dd),sqrt(c.dd)+2*soc.sf);
  if(c.prev>-1)
    printf("%lf",sqrt(coll[c.prev].dd));
  else printf("75.000 ");
  printf("%lf %lf %lf",sqrt(c.dmsf2),sqrt(c.dd),sqrt(c.dpsf2));
  printf("\n");
}
