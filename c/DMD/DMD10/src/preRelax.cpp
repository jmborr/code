...
void single_bond_sprint(char* buf, double ave, double dev){
  double min=0.1;
  double step=(ave*(1.0-dev)-min)/10.0;
...
void double_bond_sprint(char* buf, double l_ave, double l_dev, 
  ...
  double step=(r_ave*(1.0-r_dev)-l_ave*(1.0+l_dev))/13.0;
  for(int i=1; i<6; i++){
  ...
  pt+=sprintf(&buf[pt], "%lf %lf %lf %lf ",
	      ...
	      l_ave*(1.0+l_dev)+7*step, 10*egap);
  for(int i=8; i<13; i++){
    pt+=sprintf(&buf[pt], "%lf %lf ", 
		l_ave*(1.0+l_dev)+i*step, estep);
  }
...
int printATOM_TYPE(ostream& out, pseudoPep& p){
  ...
  for(int i=0;i<p.getLength();i++){
	...
	if(type==TRP){
	  type_index++;
	  type_sprint(buf, static_cast<dmd_atom_t>(type_index), TRP_D[D_M], TRP_D[D_HDR], TRP_D[D_IR]);
	  out << buf << endl;
	}
	else if(type==ARG){
	  type_index++;
	  type_sprint(buf, static_cast<dmd_atom_t>(type_index), ARG_D[D_M], ARG_D[D_HDR], ARG_D[D_IR]);
	  out << buf << endl;
	}
	else if(type==LYS){
	  type_index++;
	  type_sprint(buf, static_cast<dmd_atom_t>(type_index), LYS_D[D_M], LYS_D[D_HDR], LYS_D[D_IR]);
	  out << buf << endl;
	}
      }
    }
...
void printNONEL_COL(ostream& out, pseudoPep& p, int* cbIndex){
  ...
  /*N-O*/
  el_col_sprint(buf, _N_, _O_, min_NO);
  out << buf << endl;
  ...
  /*C-O*/
  el_col_sprint(buf, _O_, _C_, min_CO);
  out << buf << endl;
  el_col_sprint(buf, _O_HB_, _C_, min_CO);
  out << buf << endl;
  ...
  for(int i=0; i<p.getLength(); i++){
	...
	//CD-Backbone
	if(type==TRP||type==ARG||type==LYS){
	  CG2=p.getResidue(i)->getD();
	  if(type==TRP){
	    cg2_hdr=TRP_D[D_HDR];
	  }
	  else if(type==ARG){
...
void printBOND_TYPE(ostream& out, pseudoPep& p, int* cbIndex){
  ...
  for(int i=0; i<p.getLength(); i++){
	...
	if(dihedral[0]!=INF){
	  ...
	  if(type==PRO){
	    sprintf(buf, "%ld %ld %lf %lf",
		    _PRO_N_, index+1, Dihedral_MIN, Dihedral_MAX);
	  }
	  else{
	    sprintf(buf, "%ld %ld %lf %lf",
	...
	if(type==TRP){
	  ...
	  out << index+1 << " " << index+2 << " " << buf << endl;
	  /*CD-CB*/
	  n_dev = getTriDev_length(G1_CONST[TRP][G1_CB], TRP_D[D_CG], BOND_DEV_G,
				   TRP_D[D_CB]);
  ...
  for(int i=0; i<p.getLength(); i++){
	...
	if(D){
	  if(side_sprint(buf, D, index+2, CBp, indexp, hdr+d_hdr, ir+d_ir))
	    out << buf << endl;
	}
	if(typep!=ALA){
	  ...
	  if(CG){
	    if((type==PHE || type==TYR) && (typep==PHE || typep==TYR)){
	      double thdr=0;
	      if(type==PHE)     thdr+=ARO_R_PHE;
	      else              thdr+=ARO_R_TYR;
	      
	      if(typep==PHE)     thdr+=ARO_R_PHE;
	      else               thdr+=ARO_R_TYR;
	  ...
	  if(D){
	    if(type==TRP && (typep==PHE || typep==TYR)){
	      double thdr = ARO_R_TRP;
	      if(typep==PHE)     thdr+=ARO_R_PHE;
	      else               thdr+=ARO_R_TYR;
	      if(side_sprint(buf, D, index+2, CGp, indexp+1, thdr, ir+d_ir)){
		out << buf << endl;
	      }
	    }
	    else{
	      if(side_sprint(buf, D, index+2, CGp, indexp+1, hdr+d_hdr, ir+d_ir))
		out << buf << endl;
	    }
	  }
	  ...
	  if(typep==ILE||typep==THR||typep==VAL){
	    ...
	    if(D){
	      if(side_sprint(buf, D,  index+2, CG2p, indexp+2, hdr+d_hdr, ir+d_ir))
		out << buf << endl;
	    }
	  }
	  
	  if(typep==TRP||typep==ARG||typep==LYS){
	    Dp=p.getResidue(j)->getD();
	    if(typep==TRP){
	      hdr = TRP_D[D_HDR];
	      ir  = TRP_D[D_IR];
	    }
	    ...
	    if(CB){
	      if(side_sprint(buf, CB, index,   Dp, indexp+2, hdr+cb_hdr, ir+cb_ir))
		out << buf << endl;
	    }
	    if(CG){
	      if((type==TYR||type==PHE)&&typep==TRP){
		double thdr=ARO_R_TRP;
		if(type==TYR)thdr += ARO_R_TYR;
		else thdr += ARO_R_PHE;
		if(side_sprint(buf, CG, index+1, Dp, indexp+2, thdr, ir+cg_ir))
		  out << buf << endl;
	      }
	      else{
		if(side_sprint(buf, CG, index+1, Dp, indexp+2, hdr+cg_hdr, ir+cg_ir))
		  out << buf << endl;
	      }
	    }
	    if(CG2){
	      if(side_sprint(buf, CG2, index+2, Dp, indexp+2, hdr+cg2_hdr, ir+cg2_ir))
		out << buf << endl;
	    }
	    if(D){
	      if(type==TRP && typep==TRP){
		if(side_sprint(buf, D,  index+2, Dp, indexp+2, ARO_R_TRP+ARO_R_TRP, ir+d_ir))
		  out << buf << endl;
	      }
	      else{
		if(side_sprint(buf, D,  index+2, Dp, indexp+2, hdr+d_hdr, ir+d_ir))
		  out << buf << endl;
	      }
	    }
	  }
...
void printATOM_LIST(ostream& out, pseudoPep& p, randomGenerator& r, int* cbIndex){
  ...
  for(int i=0; i<p.getLength(); i++){
	...
	if(D = theAA->getD()){
	  if(theAA->getID()==TRP)
	    type = static_cast<dmd_atom_t>(_O_HB_+3+cbIndex[i]);
	  else if(theAA->getID()==ARG)
	    type = static_cast<dmd_atom_t>(_O_HB_+3+cbIndex[i]);
	  else if(theAA->getID()==LYS)
	    type = static_cast<dmd_atom_t>(_O_HB_+3+cbIndex[i]);
	  else{
	    cerr << "error in type" << endl;
	    exit(1);
	  ...
	  v = vec(r.nextGauss(), r.nextGauss(), r.nextGauss());
	  list_atom_sprint(buf, index++, type, *D->getR(), v);
	  out << buf << endl;
	}
      }
    }
...
void printBOND_LIST(ostream& out, pseudoPep& p, int* cbIndex){
  ...
  for(int i=0; i<p.getLength(); i++){
	...
	if(D){
	  if(D->getDist(*CBp)<(ir+d_ir))
	    out << D->getIndex() << " " << CBp->getIndex() << endl;
	}
	if(typep!=ALA){
	  CGp=p.getResidue(j)->getG();
	  hdr=G1_CONST[typep][G1_HDR];
	  ...
	  if(D){
	    if(D->getDist(*CGp)<(ir+d_ir))
	      out << D->getIndex() << " " << CGp->getIndex() << endl;
	  }
	  ...
	  if(typep==ILE||typep==THR||typep==VAL){
	    ...
	    if(D){
	      if(D->getDist(*CG2p)<(ir+d_ir))
		out << D->getIndex() << " " << CG2p->getIndex() << endl;
	    }
	  }
	  ...
	  if(typep==TRP||typep==ARG||typep==LYS){
	    ...
	    if(CB){
	      if(CB->getDist(*Dp)<(ir+cb_ir))
		out << CB->getIndex() << " " << Dp->getIndex() << endl;
	    }
	    if(CG){
	      if(CG->getDist(*Dp)<(ir+cg_ir))
		out << CG->getIndex() << " " << Dp->getIndex() << endl;
	    }
	    if(CG2){
	      if(CG2->getDist(*Dp)<(ir+cg2_ir))
		out << CG2->getIndex() << " " << Dp->getIndex() << endl;
	    }
	    if(D){
	      if(D->getDist(*Dp)<(ir+d_ir)){
		out << D->getIndex() << " " << Dp->getIndex() << endl;
	      }
	    }
	  }
	}
...
int main(int argc, char* argv[]){
  if(argc<3){
    cout << "usage: preRelax.linux PDB/seq boxSize [streched]" << endl;
    cout << " [streched]: set to start with streched conformation" << endl;
    cout << "   0: input a SEQ file         " << endl;
    cout << "   1: input a PDB file but generate a streched conf  " << endl;
    cout << "   2/3: input a PDB file and start with it, THE DEFAULT " << endl;
    exit(1);
  }
  
  int start_conf=3;
  if(argc==4) start_conf = atoi(argv[3]);
  pseudoPep p(argv[1], static_cast<input_t>(start_conf));
  ...
  for(int i=0; i<p.getLength();i++){
    ...
    else if(type==ILE||type==VAL||type==THR||
	    type==TRP||type==ARG||type==LYS) dummy+=3;
    else dummy+=2;
  }
