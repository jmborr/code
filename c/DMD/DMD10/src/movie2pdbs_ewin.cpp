...
int main(int argc, char* argv[]){
  if(argc < 9){
    cout << "usage: movie2pdbs_ewin.linux Emin Emax movie(bin) inputMethod inputFile startFrame nFrame output [chi_f]" << endl;
    cout << " inputMethod : 0/1" << endl;
    cout << "    0 --- input seq file only" << endl;
    cout << "    1 --- input PDB file but using SEQ only" << endl;
    exit(1);
  }
  pseudoPep p(argv[5], (input_t)atoi(argv[4]));
  int natoms = p.makeIndex();
  int isChi=0;
  ofstream chi_out;
  if(argc==10){
    isChi=1;
    chi_out.open(argv[9],ios::out);
  }
  ...
  double* para = m.getParameterValues();
