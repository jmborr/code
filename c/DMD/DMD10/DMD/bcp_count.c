...
int tball(int i,int j,int ct, double * t1)
{
  ...
  if (ab)
	...
	if(fabs(dd-coll[k].dd)>eps)
	  printf("big deviation into inner well of %ld %ld @ %ld: %lf %lf\n", i,j,get_ll(), dd, coll[k].dd);
...
//  deltaGID = atol(argv[1]);
  ...
  /*printf("%lf %lf\n",totaltime,tballtime);*/
  JUNK = fopen("junk_count","w");
  for(my_i=0;my_i<n1;my_i++){
    for(my_j=0;my_j<n1;my_j++){
      fprintf(JUNK,"%d %d %d\n",my_i,my_j,my_count[my_i][my_j]);
    }
  }
  writetext(text_name); 
   ...
   handling the events set the done variable to true. */
{
  printf("maxtime = %lf\n", maxtime); /* printf("n1=%d\n",n1);exit(1);*/
  int my_n=n+4;
  my_count=(long **)malloc(my_n*sizeof(long *));/*my_n number ats plus walls*/
  my_count[0]=(long *)malloc(my_n*my_n*sizeof(long));
  for(my_i=0;my_i<my_n*my_n;my_i++){my_count[0][my_i]=0;}
  for(my_i=1;my_i<my_n;my_i++){my_count[my_i]=my_count[my_i-1]+my_n;}
  while ((get_time()<maxtime)||coll_type) 
    {
      /*      int ticks=clock(); */
      coll_type=collision();

      my_count[p1][q1]++; my_count[q1][p1]++;
      /*  writetext(text_name);*/
