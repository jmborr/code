...
static double gr;
static int deltall;/*number of time steps between two updates of the temperature of the system by means of the thermal coefficient*/
...
extern void options_dialog (void)/*175 lines of code*/
{
  int reply;
  double old_temp;
  int must_open;
  fs_init();/*will we record story?*/
  coeff=get_coeff();
  ...
  frate=get_frate();
  mfrate=get_mfrate();	
  time1=get_time();/*total simulation time*/
  gr=get_gr();
  energy=countenergy();
  ...
  set_rate(rate);
/*when initial temperature is different than the temperature of the
  initial configuration file, set_temp is invoked. set_temp does not
  change the initial velocities to reflect the new
  temperature. Instead, it sets variable corr_2 to adjust time and
  potential wells*/
