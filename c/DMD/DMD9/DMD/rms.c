...
//    }
//  if(infile){
  fscanf(infile,"%ld %lf",&nn,&box);
  box2=box/2;
  n_index = (int *)malloc(nn*sizeof(int));
  if(!n_index)return 0;
...
from n_0 to n_0 + nn-1 */
  for(i=0;i<nn;i++)
      for(j=0;j<3;j++)
	y[i][j]=((moved_iatom *)a)[n0+n_index[i]].r[j];
...
{ 
  ...
  moveatoms();  
  {
    ...
    //else nucleus=nnat;
    nbyte=sprintf(&s[0],"%12.4lf\11%10.4lf\11%10.4lf\11%10.3lf\11%10.3lf\11%12.5lf\n"
		  ,time,temp,energy,rms,gr,T); 
    if(nbyte<=0){ fclose(echo_path);return 0;}
    if(fwrite(&s[0],1,nbyte,echo_path)!=nbyte){fclose(echo_path);return 0;}
    else 
      return 1;
