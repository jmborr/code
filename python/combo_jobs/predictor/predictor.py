...
inp=inpHand('Usage: predictor.py [options]',
            ...
            ' -e __co sequence cutoff for mkdat and mkpair(def:0.3)',            
            ' -f __wt walltime for tasser+spicker together, in days (def:7)',
            ' -g __co2 remove homologs above sequence cutoff for prospector  (def:1.0)'
            )
...
#    add_dir_for_backup(outdir,depth=2)
    ...
    jobID['dat']=job(exe='dat.py',exed=root,args='-a '+fastaf+' -b '+blastout+' -c '+inprosp+' -d '+prosptar+' -e '+outdir +' -f '+co).qsub('j5'+header,outdir,wallt='0.249',mem_limit='40',depend=dependencies,joblist=joblist)
    if jobID['dat']=='':Bye('dat job could not be submitted')
    dattar=outdir+'/'+header+'.dat.tar.bz2'
#    add_dir_for_backup(outdir,depth=2)
...
#    add_dir_for_backup(outdir,depth=2)
    ...
    job(exe='dukka_pulchra.py',exed=root,args='-a '+fastaf+' -b '+spktar+' -c '+outdir).qsub('j7'+header,outdir,wallt=0.249,mem_limit='40',depend=dependencies,joblist=joblist)
#    add_dir_for_backup(outdir,depth=2)
