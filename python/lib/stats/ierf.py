...
def erfc( x ):
    ...
    t = 1.0 / (1.0 + p*float(x))
    erfcx = ( (a1 + (a2 + (a3 + (a4 + a5*t)*t)*t)*t)*t ) * exp(-(x**2))
    return erfcx
...
def intnd(x): return erfc(x)/sqrt(8)

#integral from minus infinity to x of the standard normal distribution
def remnd(x): return intnd(-x)
