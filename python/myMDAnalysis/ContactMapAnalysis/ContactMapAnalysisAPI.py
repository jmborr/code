...
class ContactMapProtocol:
    ...
    def initialize(self,Universe):
        ...
        if self.byres:
            ...
            self.atom2res=numpy.zeros(N, dtype=int)
...
def GenerateContactMap(TimeStep,protocol):
    ...
    if protocol.byres:
        ...
        shape=(1+rowIndices.max(), 1+colIndices.max())
    else:
    ...
    data=numpy.ones( len(rowIndices), dtype='int32' ) #just signaling a contact map
...
def GenerateContactMapList(PSFfile,trajfile,protocol, startframe=0, endframe=-1, step=1):
    ...
    report_frames=max(1, (endframe-startframe)/(10*step))
    percent=0
    print 'Reading frames..0%', ; sys.stdout.flush()
    iframe=1
    for ts in u.trajectory[startframe:endframe:step]: # iterate through all frames
        cm=GenerateContactMap(ts,protocol)
        cml.append(cm)
        if not iframe%report_frames:
            percent += 10
            print '..{0}%'.format(percent), ; sys.stdout.flush()
        iframe +=1
    print '..finished!'
    return cml
