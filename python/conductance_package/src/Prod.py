...
from small_utilities import chomp,junkName,Bye
...
class Prod:
    ...
    def __init__(self,TYP,dir,module_name=None):
       ...
       ============Up to here is minimal Prod directory structure
        ...
        cmds={'crdlist':'cd '+dir+' && ls -1 prev|grep -P "'+TYP+'.+\.crd.*"',
              ...
              'outlist':'cd '+dir+' && ls -1 prev|grep -P "'+TYP+'\.\d+\.out.*"',
        ...
        for key,cmd in cmds.items():
            ...
            try:
                ...
                mssg='ERROR could not find '+key+' file types\n'
    ...
    def armonize_prev_outlist(self):
        ...
        #case when output files are compressed
        newlist=[]
        ...
        #case when we previously condensed the output files
        newlist=[]
        outl=os.popen('ls %s/prev/*.out'%self.simdir).readlines()
        if len(outl)!=len(self.prev['outlist']):
            self.prev['outlist']=[ 'prev/'+basename(x.strip()) for x in outl ]
        self.dump()
    ...
    def doStrip(self,maskl=[':WAT',':Na+']):
        ...
        if vellist:
            ...
            mssg='ERROR could not find velfiles\n'
            sys.stderr.write(mssg)
        #tar and compress files
        cmd="""TYP=_TYP_
simdir=_SIMDIR_
cd $simdir/prev
for d in crd vel top out rst;do
  echo \"processing $d ...\"
  mkdir $d
  ls -1|grep ${TYP}.*.$d > junk
  for f in `cat junk`;do
    mv $f $d
  done
  tar zcf $d.tar.gz $d
  /bin/rm -rf $d
        ...
        if dowegzip:
                cmd=cmd.replace('_TYP_',self.TYP)
                cmd=cmd.replace('_SIMDIR_',self.simdir)
    ...
    def getAvg(self,condense=True):
        ...
        for f in self.prev['outlist']: #cycle over simulations            
            ...
            while l:
                if 'A V E R A G E S   O V E R' in l:
                    l=pf.readline().strip()
...
if __name__=='__main__':
    ...
    ih=inpHand('Usage: Prod2.py',
               ...
               ' -b _R_TYP protein ID',
               ' -c __stripList strip list (def: "WAT Na+")',
               )
    ...
    #print 'doStrip'
    #if stripList: stripList=[ ':'+x for x in stripList.split(' ')]
    #else: stripList=[':WAT',':Na+']
    #pr.doStrip(maskl=stripList)
